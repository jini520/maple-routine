...
**폭은 248 이 상한이다**(사용자 지시, 정정 45). `ItemRevenuePopover` 와 같은 값이라 팝오버 셋이
한 폭으로 선다. 상한이 없으면 넓은 기기에서 두 문장이 한 줄씩으로 늘어져 상자가 화면을
가로지른다. 좁은 기기에서는 화면이 먼저 좁아 `창폭 − 24` 를 쓴다. 왼쪽은 카드 왼쪽에 붙인다.
제목이 고정 문구라 `?` 자리가 안 움직여서, 상한을 줘도 `?` 가 상자 위에 남는다.
