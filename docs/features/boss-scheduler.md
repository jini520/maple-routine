...
- **UI** ([[ADR-055]] 정정 1): 관리 화면의 잠긴 행은 **흐림(opacity-30) + 그 위에 얹는 한 줄**로 사유를 말한다 — `295레벨 달성 시 진행 가능` · `주간 12개를 모두 선택했어요`. 오른쪽 구석 뱃지는 쓰지 않는다(흐림만으로는 이유가 읽히지 않았다). 흐림은 `<li>` 가 아니라 안쪽 내용 래퍼에 걸어야 문구가 함께 흐려지지 않는다. 난이도 칩의 `{난이도} Lv.{요구레벨}` 표기는 유지(추적 중인 행에서만 보이고, 상태 뱃지가 아니라 그 난이도의 조건을 답한다). 보스 카드 표시 화면에서는 dim + `진행 불가` 칩이고, 잠긴 추적 항목을 **자동으로 제거하지 않는다**(결정 9).
