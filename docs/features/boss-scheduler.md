...
- **화면 진입 재조회 게이트도 컨텐츠 스케줄러와 동일**([[ADR-097]], 정책 원문은 [content-scheduler.md](./content-scheduler.md) 정책 절) — 마운트가 곧 조회가 아니고, 판정 근거는 세 화면이 공유하는 `scheduler-cache` 의 `syncedAt` 이다(10분). 이 화면의 `refresh` 는 `(ocids, onProgress?)` 지만 게이트는 `refresh` 가 아니라 `loadTrackedOcids()` 에 있어 시그니처 차이와 무관하다.
