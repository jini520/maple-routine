...
## 파티 관리 ([[ADR-019]])
캐릭터+보스+난이도 단위로 파티 인원을 미리 설정하는 상시 데이터(완료 여부·주차 무관). 화면 상단 "캐릭터 관리" 옆 **"보스 관리" 버튼**([[ADR-035]] 이후 두 모드 공통, 이전엔 "파티 관리")에서 편집(**RN 앱에는 그 버튼이 아예 없다** — 옆의 "캐릭터 관리"가 설정으로 갔고([[ADR-140]]), 남은 "보스 관리"도 하단바의 **스케줄 하위 탭**으로 올라갔다([[ADR-145]] 결정 1)). 저장은 `storage/` 에 `(ocid, boss, difficulty)` 유니크 키로 `boss_party_settings` upsert(1로 저장하면 솔로 취급, 별도 삭제 API 없이 1로 덮어씀). `boss_profit_records`(주차별 완료 기록)와 별도 테이블, 같은 SQLite DB. 이 값은 보스 카드 파티 배지·솔로/파티 필터·[보스 수익](./boss-profit.md) 자동 기록 기본값에 함께 쓰인다.

## UI

### 화면 헤더 ([[ADR-098]], 2026-08-06)

> **RN 앱은 캐릭터를 드롭다운이 아니라 초상화 레일로 고른다**([[ADR-142]], 2026-08-16) — 컨텐츠 스케줄러와 **같은 컴포넌트·같은 규칙**이고, 링은 **주간 하나뿐**이다(온전한 원 · 반시계 · 12시 틈 없음). **월간은 링에서 뺐다** — 종류가 하나뿐이라 «몇 개 중 몇 개» 가 1/1 밖에 없어 진행률이라는 표현이 과하다(표현 방법은 따로 정한다, [[ADR-142]] 정정 1·3). 세는 것은 표시 목록의 `isComplete` 이고 **솔로/파티 필터는 안 탄다**(필터는 «지금 보고 싶은 것» 이지 진행이 아니다). **레일의 순서는 `trackedCharacters` 저장 순서다**([[ADR-143]] 결정 3, 구현 완료 2026-08-17 — 컨텐츠 스케줄러와 같은 함수 `orderByTracked`, 상세는 [content-scheduler.md](./content-scheduler.md) 의 같은 절). 다만 **`/boss/manage` 의 레일은 아직 스토어 순서다**(아래 절).

> **RN 앱은 동기화 상태를 제목 줄에 둔다**([[ADR-141]], 2026-08-16) — 컨텐츠 스케줄러와 **같은 변경**이다(그 문서의 같은 절). ~~관리 버튼(`보스 관리`)은 제목 줄 오른쪽 끝을 지킨다.~~ → **그 버튼도 없어졌다**([[ADR-145]] 결정 1, 2026-08-17) — 보스 관리가 하단바의 하위 탭이 되면서 제목 줄에는 제목·동기화 상태·새로고침만 남는다.
...
### 빈 상태 ([[ADR-060]])
공용 `EmptyState`(inline, `Swords`). 수동 모드는 "추적할 주간/월간 보스가 없습니다" + CTA "보스 관리" → `/boss/manage`(**RN 앱은 하위 페이지가 아니라 같은 이름의 탭으로 보낸다** — [[ADR-145]] 결정 1), 자동 모드는 "등록된 주간/월간 보스가 없습니다"(CTA 없음 — 목적지가 앱 밖). 주간/월간 문구를 공유하지 않는다. 레시피는 [design-system.md](../foundation/design-system.md).
...
> **RN 앱에서는 하위 페이지가 아니라 스케줄 그룹의 세 번째 하위 탭이다**([[ADR-145]] 결정 1, 2026-08-17) — 바의 스케줄 하위가 «컨텐츠 · 보스 · 보스 관리» 셋이 되고, 진입 버튼도 화면의 `←` 도 함께 사라진다(← 는 하단바가 진다, [[ADR-132]] 결정 3). **그래서 주간/월간 탭은 승계가 아니라 공유다**(결정 2 — 탭 화면은 마운트된 채 남아 «진입 시점» 이라는 사건이 앱 실행당 한 번이 된다. 선택 캐릭터와 같은 규칙이 됐다). **자동 모드 상단 안내 한 줄은 없다**(결정 3), 토글은 「모든 보스 보기」(기본 꺼짐, 결정 4).
> **RN 앱은 그 드롭다운 자리에 초상화 레일을 둔다**([[ADR-142]] 정정 8) — 컨텐츠 관리 페이지와 같은 처리이고, **진행 링 없이** 레벨·닉네임만 싣는다. 링이 없는 만큼 **칸 사이 간격도 걷는다**(4 → 0, [[ADR-145]] 결정 5). 헤더 고정 방식과 스크롤 컨테이너도 스케줄러와 같다([[ADR-098]]·[[ADR-099]]). **순서만 아직 스케줄러와 다르다** — 관리 화면 둘(`/boss/manage`·`/content/manage`)의 레일은 스토어 순서(레벨 내림차순)이고, 스케줄러 쪽만 저장 순서로 옮겼다([[ADR-143]] 결정 3 구현 노트 — 다음 작업에서 맞춘다).
