...
- **화면 진입 재조회 게이트도 컨텐츠 스케줄러와 동일**([[ADR-097]], 정책 원문은 [content-scheduler.md](./content-scheduler.md) 정책 절) — 마운트가 곧 조회가 아니고, 판정 근거는 세 화면이 공유하는 `scheduler-cache` 의 `syncedAt` 이다(10분). 구현도 같은 모양이다 — `refresh` 가 내부 옵션(`options?: RefreshOptions{ auto? }`)을 받고 자동 진입 경로인 `loadTrackedOcids()` 만 `{ auto: true }` 를 넘긴다(헤더·당겨서 새로고침·재시도는 인자를 안 넘겨 강제 경로). **이 화면만의 예외는 파티원 수 조회(`loadPartySizes`)가 게이트보다 앞에 있다는 것** — `boss_party_settings` 는 로컬 SQLite 라 네트워크 TTL 의 대상이 아니고, 함께 건너뛰면 추적 목록이 바뀐 진입에서 파티 배지·솔로/파티 필터가 옛 값으로 남는다.
- 동기화 실패 표시도 컨텐츠 스케줄러와 **동일**([[ADR-063]]·[[ADR-083]], 정책 원문은 [content-scheduler.md](./content-scheduler.md) "동기화 실패 표시"): 전체 조회 실패도 **캐릭터별 실패(`selected.error`)도** 헤더 아래 인라인 문단이 아니라 **토스트**로 알리고, 원인별로 액션이 갈린다(`network` → 다시 시도 · `rateLimited`·`characterUnavailable` → 없음 · `invalidApiKey` → **토스트 자체가 없다** — 키 무효화 진입점이 자기 토스트와 함께 화면을 키 입력으로 옮긴다, [[ADR-115]] 결정 7, 2026-08-08).
...
- **완료 승격은 수동 모드 표시 목록에도 적용된다** ([[ADR-121]] 결정 5, 2026-08-10): `normalize.ts` 가 `isComplete = ownComplete || (isRegistered && 같은 content_name 이 완료)` 로 **보스 단위 승격**을 하지만([[ADR-031]]·[[ADR-032]]), 그 승격은 **`isRegistered` 인 행에만** 걸린다. 그리고 수동 모드 목록을 만드는 `mergeManualBossList` 는 **(보스명, 난이도) 정확 일치**로만 찾아 못 찾으면 미완료 폴백으로 떨어졌다 — 그래서 하드를 잡아 둔 뒤 익스트림으로 바꾸면 **완료 배지가 사라졌다**. 이제 정확 일치 행이 없거나 미완료면 **같은 보스명의 다른 난이도 완료를 승격**한다. 새 정책이 아니라 기존 승격 규칙을 수동 경로에도 적용하는 **누락 보완**이다. **`ownComplete` 는 승격하지 않는다** — 보스 수익이 "실제로 어느 난이도를 처치했는가"를 판정하는 근거라 원본이어야 한다([[ADR-033]] 결정 1).
...
### 화면 헤더 ([[ADR-098]], 2026-08-06)
제목~필터는 공용 셸 `PageHeader`([[ADR-094]])로 상단 고정하고 그 아래 목록만 스크롤한다. **`sticky` 가 아니라 `fixed` + 실측 spacer** 이고([[ADR-098]]), **스크롤은 문서가 아니라 화면이 소유한다**(`ScreenScroll`, [[ADR-099]]) — 근거와 레시피는 컨텐츠 스케줄러와 완전히 동일하다([content-scheduler.md](./content-scheduler.md) 'UI — 화면 헤더' · [foundation/design-system.md](../foundation/design-system.md) '스크롤 영역').
...
**카드 탭 → 파티 인원 모달** ([[ADR-121]] 결정 1, 2026-08-10, 이슈 #156): 카드 **전면(80px)이 버튼**이고 탭하면 아래 모달이 열린다. **어포던스 표식을 두지 않는다** — 셰브런·연필을 얹지 않고 눌림 피드백(`active:scale-[.985]` + 밝기)만 준다([[ADR-018]] 카드 규격 무변경과 맞바꾼 값이라, 발견 비용은 감수한다). 완료된 보스도 눌린다(파티 인원은 완료 여부와 무관한 상시 데이터).
> 이 방식은 [[ADR-019]] 결정 4가 **"카드 인터랙션 충돌 우려"** 로 폐기했던 것이다. 그 전제는 [[ADR-035]] 결정 18이 편집을 `/boss/manage` 로 옮기며 **카드의 인터랙션을 0개로 만들면서 사라졌다** — 충돌할 상대가 없는 자리에 단 하나의 탭 의미를 부여하는 것이다.

### 파티 인원 모달 ([[ADR-121]])
보스 카드를 탭하면 열린다. **난이도와 파티 인원을 함께** 다뤄 관리 페이지 왕복을 없앤다. 저장 경로는 그대로 `setPartySize(ocid, boss, difficulty, n)` 이고 스토어·SQLite 스키마 변경이 없다. 실패 토스트는 관리 페이지와 같은 문구(`파티원 수를 저장하지 못했습니다`).

**파티 인원은 (보스 + 난이도)에 붙어 있다** — 키가 `partySizeKey(ocid, boss, difficulty)` 이고 상한도 `getMaxPartySize(boss, difficulty)` 다(스우: 하드 6인 / **익스트림 2인**). 모달에서 난이도를 바꾸면 스테퍼가 **그 난이도의 값·상한**으로 갈아탄다 — 값을 잃은 것이 아니라 다른 칸을 보는 것이고, 라벨 옆 `n / max` 배지가 그 사실을 말한다.

**자동 모드에서도 난이도 세그먼트를 그린다**([[ADR-121]] 결정 3) — 단 **멤버십을 바꾸지 않는다.** 관리 페이지의 `autoDifficultyByBoss` 와 같이 "어느 난이도의 파티 인원을 편집할지" 고르는 스위치다(자동 모드의 카드 난이도는 게임 등록 기준이라 앱이 못 바꾼다). 닫아도 카드의 난이도 배지는 그대로다. 모드별로 숨기지 않는 이유는 **나중에 자동/수동 모드를 통합할 것**이라 그때 이 자리가 멤버십 전환으로 승격되면 되기 때문이다.

규격 전문은 [design-system.md](../foundation/design-system.md) '파티 인원 모달'. 요점만:
```
...
```
- **폭 288 은 하한이다** — 4난이도 보스(칼로스·카링·최초의 대적자)의 칩 한 줄이 약 225px 라 본문 폭 252(여유 27)에 겨우 들어간다. 한 단계 아래(`max-w-3xs` 256)는 220 이라 접히고, 접히면 **보스마다 모달 높이가 달라진다**. 288 은 **모든 기기에서 폭이 같다**는 성질도 갖는다(`max-w-sm` 은 오버레이 `px-4` 에 걸려 390 기기 358 / 360 기기 328 로 갈렸다).
- **일러스트 없는 보스**(`portraitSlug: null`)는 히어로를 **비운다** — `media-surface` 단색에 이름만. 폴백 디자인을 따로 만들지 않는다(데이터를 채울 예정이라 사실상 발생하지 않는다).
...
> **[[ADR-120]] 이 이 화면을 스택으로 옮긴다**(설계 확정·구현 전, 이슈 #166): 형제 최상위 라우트 → `/boss` 의 **중첩 라우트**, 화면 셸은 공용 `StackScreen`(포털 오버레이 + 푸시/팝 + 가장자리 스와이프 백), `←` 는 `navigateToScreen('/boss')` → `navigate(-1)` **진짜 pop**. 하위 페이지엔 탭바가 없다. 아래 목록·행 서술은 그대로 유효하다.
...
- **리디자인(2026-07-24) — 초상화 앵커 + 파티 스테퍼 상단 + 난이도 세그먼트**: 행 2줄 — 1줄 원형 `BossPortrait`(`size={44}`, `aria-hidden`) + 보스명(`flex-1 truncate`) + 파티 스테퍼(우상단 고정). 2줄(활성 시, `border-t`) 난이도 세그먼트. 선택 = 테두리·색(체크 원 없음, 미추적 `border-border bg-surface`, 추적 `border-primary bg-primary-tint`). 수동 토글은 초상화+보스명 영역이 버튼(`aria-label={보스명}`). 난이도 세그먼트: 선택은 `DifficultyBadge` 풀컬러, **미선택은 같은 뱃지 + `opacity-40`**([[ADR-121]] 결정 4 — 아래 '미선택 난이도 칩'). 파티 스테퍼: 보더 pill 안 `Users` 아이콘 + −/값/+, 1~`getMaxPartySize(boss, difficulty)` 경계 비활성화, 탭 즉시 저장(`size="compact"`, 28px — 모달은 40px `default`).
- **난이도 변경은 스토어 단일 액션이다** ([[ADR-121]] 결정 6): `setManualBossDifficulty(ocid, boss, to)` — 읽기 → 배열 계산 → **쓰기 1회**. `remove` → `add` 2단계는 커밋이 2회라 첫 커밋 직후 "그 보스가 목록에 없는" 상태가 저장소에 실재했고, 거기서 실패하면 보스가 통째로 사라졌다. 인자에 `from` 이 없는 것도 의도다 — 렌더 시점의 낡은 값이 넘어와 변경이 무음 유실되는 경로를 없앤다. 개수가 안 변하므로 **주간 12개 한도에 걸리지 않는다**(반환 `void`).

### 미선택 난이도 칩 ([[ADR-121]] 결정 4, 2026-08-10)
**미선택도 `DifficultyBadge` 풀컬러 그대로 두고 `opacity-40` 만 건다.** 선택/미선택은 선명도 차이로만 갈린다. 모달과 관리 페이지가 **같은 처리를 공유한다**(한쪽만 바꾸면 두 화면의 같은 칩이 다르게 보인다).

근거는 **"메이플 유저는 난이도를 색과 실루엣으로 안다"**(사용자 판단) — 카오스의 금테, 익스트림의 붉은 이중선, 하드의 분홍은 게임 안에서 반복 노출된 형태라 흐려져도 무엇인지 먼저 알아본다. **미선택 칩의 글자 가독성은 이 화면에서 결정적이지 않다.** 배지 **안** 글자 대비는 1.43~3.15로 4.5:1은커녕 3:1도 못 넘지만 이를 감수한다 — 단 **이 화면의 이 요소에 한정된 판단**이고 다른 자리의 흐림 처리로 일반화하지 않는다. `opacity` 는 스크린리더에 영향이 없어 칩의 글자와 `aria-pressed` 는 그대로 전달된다.

`opacity-40` 은 [[ADR-055]] 주간 한도 흐림이 이미 쓰는 값이라 **신규 색·신규 값이 0개**다. 대안(아웃라인 단일 잉크 / 아웃라인 모드별 잉크 2벌 / 틴트 채움)의 6테마 대비 실측과 탈락 사유는 [[ADR-121]] 결정 4 표.
...
- ~~수동 모드 표시 목록은 (보스명, 난이도) 정확 일치로만 완료를 판정~~ → **같은 보스명의 다른 난이도 완료를 승격**([[ADR-121]] 결정 5, 2026-08-10). 난이도를 바꾸면 완료 배지가 사라지던 누락 보완.
