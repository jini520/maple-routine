...
### 빈 상태 ([[ADR-060]])
공용 `EmptyState`(inline, `Swords`). 수동 모드는 "추적할 주간/월간 보스가 없습니다" + CTA "보스 관리" → **스케줄 그룹의 세 번째 하위 탭**([[ADR-145]] 결정 1 — 헤더 버튼이던 하위 페이지가 형제 탭이 됐다), 자동 모드는 "등록된 주간/월간 보스가 없습니다"(CTA 없음 — 목적지가 앱 밖). 주간/월간 문구를 공유하지 않는다. 레시피는 [design-system.md](../foundation/design-system.md).
...
> **하위 페이지가 아니라 스케줄 그룹의 세 번째 하위 탭이다**([[ADR-145]] 결정 1, 2026-08-17) — 바의 스케줄 하위가 «컨텐츠 · 보스 · 보스 관리» 셋이 되고, 진입 버튼도 화면의 `←` 도 함께 사라진다(← 는 하단바가 진다, [[ADR-132]] 결정 3). **그래서 주간/월간 탭은 승계가 아니라 공유다**(결정 2 — 탭 화면은 마운트된 채 남아 «진입 시점» 이라는 사건이 앱 실행당 한 번이 된다. 선택 캐릭터와 같은 규칙이 됐다). **자동 모드 상단 안내 한 줄은 없다**(결정 3), 토글은 「모든 보스 보기」(기본 꺼짐, 결정 4).
