...
> **그 드롭다운 자리에는 초상화 레일이 선다**([[ADR-142]] 정정 8) — 컨텐츠 관리 페이지와 같은 처리이고, **진행 링 없이** 레벨·닉네임만 싣는다. 링이 없는 만큼 **칸 사이 간격도 걷는다**(4 → 0, [[ADR-145]] 결정 5). 헤더 고정 방식과 스크롤 컨테이너도 스케줄러와 같다(ADR-098·[[ADR-099]]). **순서만 아직 스케줄러와 다르다** — 관리 화면 둘(`/boss/manage`·`/content/manage`)의 레일은 스토어 순서(레벨 내림차순)이고, 스케줄러 쪽만 저장 순서로 옮겼다([[ADR-143]] 결정 3 구현 노트 — 다음 작업에서 맞춘다).
