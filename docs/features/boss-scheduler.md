...
> **RN 앱은 캐릭터를 드롭다운이 아니라 초상화 레일로 고른다**([[ADR-142]], 2026-08-16) — 컨텐츠 스케줄러와 **같은 컴포넌트·같은 규칙**이고, 링은 **주간 하나뿐**이다(온전한 원 · 반시계 · 12시 틈 없음). **월간은 링에서 뺐다** — 종류가 하나뿐이라 «몇 개 중 몇 개» 가 1/1 밖에 없어 진행률이라는 표현이 과하다(표현 방법은 따로 정한다, [[ADR-142]] 정정 1·3). 세는 것은 표시 목록의 `isComplete` 이고 **솔로/파티 필터는 안 탄다**(필터는 «지금 보고 싶은 것» 이지 진행이 아니다).
...
> **RN 앱은 그 드롭다운 자리에 초상화 레일을 둔다**([[ADR-142]] 정정 8) — 컨텐츠 관리 페이지와 같은 처리이고, **진행 링 없이** 레벨·닉네임만 싣는다. 주간에서 들어오면 주간, 월간에서 들어오면 월간이고, 여기서 탭을 바꿔도 스케줄러는 그대로다. 헤더 고정 방식과 스크롤 컨테이너도 스케줄러와 같다([[ADR-098]]·[[ADR-099]]).
