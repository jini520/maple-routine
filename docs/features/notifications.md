...
**거부한 사람에게는 길을 안내한다.** 나중에 스위치를 켜면 스토어가 권한을 먼저 보고, 없으면
구독을 안 걸고 `blockedByPermission` 을 세운다. 화면이 그것을 받아 OS 설정으로 가는 행을
띄운다. 조용히 구독만 하면 **스위치는 켜져 있고 알림은 안 오는** 상태가 되는데 사용자는 그것을
