...
>
> **남은 것은 서버 조회 하나다.** 아래 **배경에서 도착만 한 공지**의 구멍이 그것이 와야 메워진다.
> 그 조회가 앱에서 서버로 가는 첫 요청이라 인프라가 함께 선다([[ADR-229]]).
...
그래서 **알림은 뜨는데 목록에는 없는** 공지가 생긴다. 사용자가 알림을 지우고 나중에 목록을 열면
그 공지를 못 본다. 서버 조회가 오면 목록이 그 구멍을 메운다.
