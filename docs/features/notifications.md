...
> **공지의 기준은 서버다** 절이 새 규칙이고, 열린 질문 셋이 구현을 막고 있다.
...
앱 공지가 든 조회에서만 한다. 게임 공지만 묻는 응답에는 앱 공지 id 가 없어서, 그 응답으로 정리하면
닫아 둔 배너가 전부 되살아난다.

**아직 안 정해진 것이 셋이다**(아래 열린 질문). 분류로 거른 목록과 사본의 관계 · 상세 본문의 사본 ·
상세 조회의 404.
...
- **공지 사본을 분류마다 두는가**([[ADR-277]] 열린 질문 1). 소식 행 다섯이 각자 한 분류로 20건을
  묻고 배너는 분류 없이 20건을 묻는다. 사본 하나를 마지막 조회가 통째로 바꾸면 다른 분류의 오프라인
  목록이 비고, 넥슨 공지가 몰린 날 배너의 앱 공지가 빠진다. **사용자 결정 대기.**
- **상세 본문(`blocks`)의 사본을 어디에 두는가**([[ADR-277]] 열린 질문 2). 목록 응답에는 `blocks` 가
  없어, 목록 사본을 통째로 바꾸면 오프라인 상세가 본문 블록을 잃는다. 20건 밖 공지를 알림으로 열었을
  때 무엇을 그리는지도 여기서 갈린다. **사용자 결정 대기.**
- **상세 조회의 404 를 `없다` 로 읽는가**([[ADR-277]] 열린 질문 3). 지금 앱은 404 와 장애를 둘 다
  실패로 접는다. **사용자 결정 대기.**
