...
## 구독은 넷으로 갈린다 ([[ADR-247]] 결정 1·2)

사용자가 정한 토글이 넷이고, 저장되는 분류는 다섯이다. 업데이트와 이벤트가 한 토글을 나눠 쓴다.

| 토글 | 토픽 | 담는 분류 | 켜지는 기본값 |
|---|---|---|---|
| 앱 공지사항 | `notice` | `app` | 권한 허용 시 **켬** |
| 게임 공지사항 | `notice-game` | `game` | 권한 허용 시 **켬** |
| 업데이트·이벤트 | `notice-update-event` | `update` · `event` | 꺼짐 |
| 캐시샵 | `notice-cashshop` | `cashshop` | 꺼짐 |
...
**넷을 다 켜 주지 않는 이유는 몰려 오기 때문이다**([[ADR-247]] 결정 10). 실측으로 8월 20일 패치
날에 이벤트 5건과 캐시샵 4건이 같은 8시 14분에 올라왔다. 묻지도 않고 켜면 그날 알림이 아홉 번
울리고, 그것이 알림을 통째로 끄게 만든다.

**이벤트는 전부 저장하되 알림은 썬데이만 나간다**(사용자 지정, [[ADR-247]] 결정 7). 목록에는 다
보이고 알림만 갈린다. 그래서 `업데이트·이벤트` 토글의 설명이 «패치 노트와 썬데이 메이플» 이다 -
화면이 그것을 말하지 않으면 사용자는 다른 이벤트 알림이 안 오는 것을 고장으로 읽는다.
