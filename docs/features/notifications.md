...
> 변경이 둘 다 커밋만 되어 있다.
>
> **공지의 기준을 서버로 옮긴다**(2026-09-13, [[ADR-277]], 이슈 #398, **구현 전**). 서버에서 지운
...
| 수신 | `src/features/notice/receive.ts` | **있다.** 페이로드를 공지로 읽는다. 쌓는 `receiveNotice` 는 [[ADR-277]] 결정 1 이 없앤다(구현 전) |
...
### 공지의 기준은 서버다. 기기에는 마지막 서버 응답의 사본만 둔다 ([[ADR-277]], 구현 전)
...
- ~~푸시 `data` 를 기기에 쌓고, 목록 · 상세 · 배너가 로컬을 먼저 그린 뒤 서버 응답을 `noticeId` 로
  ...
  기기에는 마지막 서버 응답의 사본만 둔다**([[ADR-277]], 2026-09-13, 이슈 #398, 구현 전). 합치기만
