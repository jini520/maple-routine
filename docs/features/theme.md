...
> **관련 ADR**: [[ADR-009]] [[ADR-064]] [[ADR-088]] [[ADR-089]] [[ADR-105]] [[ADR-006]] [[ADR-104]]. **관련 문서**: [../foundation/design-system.md](../foundation/design-system.md), [settings.md](./settings.md).
...
**혼테일**(보스 필드 맵 = 밤 수정 동굴)과 **검은마법사**(보스 맵, [[ADR-089]] 결정 3) 둘이고,
나머지 넷은 필드가 없어 CSS 자체가 나가지 않는다 — 배경 없는 테마의 그림은 한 픽셀도 안 바뀐다.
두 테마의 `dim` 이 다른 데서 보듯(0.82 / 0.78) 이 값은 그림마다 다시 잡는다.
...
- **`position` 이 항상 `center` 인 것은 아니다 — 그림의 대칭축이 그림 한가운데가 아닐 수 있다.**
  검은마법사 맵은 왕좌·눈의 대칭축이 x=413/800(**51.6%**)이라 `center` 로 두면 화면에서 오른쪽으로
  치우쳐 보였다([[ADR-105]] 결정 3). `cover` 가 세로로 긴 뷰포트에서 그림을 2.6배로 확대해 가운데
  150px 만 보여주므로 원본 13px 어긋남이 화면에서는 **35px(390 폭의 9%)** 가 된다 — 세로 뷰포트에서는
  이 오차가 배율만큼 증폭된다. `52% center` 로 잡았고 폭 360~430 에서 ±2px 다(확대 배율이 커서 퍼센트
  값이 뷰포트 비율에 거의 흔들리지 않는다). **그림을 다시 자르지 않는다** — 헤더 조각이 같은 에셋을
  같은 기하로 그리므로 에셋을 바꾸면 양쪽을 함께 맞춰야 한다.
