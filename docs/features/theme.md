...
- **`on-X`**: X의 **색상(H)을 물려받은** 짙은 색·옅은 색을 만든다. 흑/백은 방향(어두운 쪽이냐 밝은 쪽이냐)을 정하는 데만 쓰고 값으로는 쓰지 않는다 — 배경색마다 어울리는 전경색이 따로 있기 때문이다. 주황 채움 위에는 짙은 갈색(`#1F0C00`), 보라 위에는 짙은 보라(`#160D1D`), 파스텔 하늘 위에는 짙은 청록(`#0A1317`)이 온다. 채도는 원 색상보다 크게 낮춘다(전경은 글자라 색이 튀면 읽기를 방해한다). 밝은 파스텔 primary면 어두운 전경이, 어두운 primary면 밝은 전경이 자동으로 정해진다 — **어느 쪽도 전제하지 않는 것**이 요점이다.
...
현재 면제는 하나다 — **머쉬맘 `track/primary`(1.86:1)**. 파생된 어두운 트랙보다 기존 크림 트랙이 낫다는 사용자 판단(2026-07-30, [[ADR-064]] 결정 4 정정). 주황과 크림은 휘도 차가 작지만 색상 차가 커서 실제로는 구분된다.
