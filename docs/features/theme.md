...
`dim` 은 그림마다 다시 잡는다(옛 혼테일 0.82 · 옛 검은마법사 0.78 → 지금 **혼테일 0.8 · 검은마법사
0.65**). 옛 값과 비슷해 보여도 **물려받은 값이 아니라 새로 잡은 값**이다 — 그림이 다르면 `dim` 도
`position` 도 다시 재야 한다.
