...
> **관련 ADR**: [[ADR-009]] [[ADR-064]] [[ADR-088]] [[ADR-089]] [[ADR-105]] [[ADR-106]] [[ADR-108]] [[ADR-006]] [[ADR-104]]. **관련 문서**: [../foundation/design-system.md](../foundation/design-system.md), [settings.md](./settings.md).
...
**지금 값을 가진 테마는 검은마법사 하나다**([[ADR-108]], 2026-08-07). 혼테일은 [[ADR-106]] 으로 뗀 뒤
새 그림 대기 중이라 아직 없다 — 배경 없는 테마는 CSS 자체가 나가지 않아 그림이 한 픽셀도 안 바뀐다.
검은마법사가 붙으면서 "새 그림은 JSON 블록 하나로 다시 붙는다"는 [[ADR-106]] 의 전제가 실제로 확인됐다
(제품 코드·타입·CSS 변경 0).

`dim` 이 테마마다 다른 데서 보듯(혼테일 0.82 · 옛 검은마법사 0.78 · 지금 검은마법사 0.65) 이 값들은
그림마다 다시 잡는다. 옛 값을 그대로 물려받지 말 것 — 그림이 다르면 `dim` 도 `position` 도 다시 재야
한다. 검은마법사가 0.65 로 더 밝은 것은 **하늘이 투명이라 어두워지는 면적이 이미 작기** 때문이다.
