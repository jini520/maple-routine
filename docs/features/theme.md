...
> **관련 ADR**: [[ADR-009]] [[ADR-064]] [[ADR-088]] [[ADR-089]] [[ADR-006]] [[ADR-104]]. **관련 문서**: [../foundation/design-system.md](../foundation/design-system.md), [settings.md](./settings.md).
...
- **`X-tint`**: `color-mix(in oklab, var(--color-X) 15%, var(--color-surface))`. 테마당 손튜닝 값이 늘지 않는다. 다른 농도가 필요한 테마만 명시값으로 덮는다.
  ...
  **덮어야 하는 경우 — 시드와 표면의 색상이 멀 때.** 15% 규칙은 시드 색이 표면 색조와 가깝다는
  ...
  검은마법사가 **같은 자리에서 같은 처방**을 받았다([[ADR-089]] 결정 2) — 자주색 표면(`#1E1214`,
  휴 8)에 라벤더(`#A9A2E8`, 휴 288)를 15% 섞으면 `#30252F` 로 휴가 **41° 끌려간** 자두색이 되어
  `surface2`(`#302224`)와 구분되지 않는다. `#2D284A`(휴 이탈 1°)로 덮었다.
