...
> **관련 ADR**: [[ADR-009]] [[ADR-064]] [[ADR-088]] [[ADR-089]] [[ADR-105]] [[ADR-106]] [[ADR-006]] [[ADR-104]]. **관련 문서**: [../foundation/design-system.md](../foundation/design-system.md), [settings.md](./settings.md).
...
**지금 값을 가진 테마는 0개다**([[ADR-106]], 2026-08-06) — 혼테일·검은마법사가 갖고 있었으나 그림을
둘 다 바꾸기로 하면서 새 그림이 정해질 때까지 뗐다. 여섯 테마 전부 `bg` 단색이고 CSS 자체가 나가지
않는다. **구조는 그대로 살아 있다** — 새 그림은 아래 블록 하나로 다시 붙는다.

`dim` 이 테마마다 달랐던 데서 보듯(뗀 값은 0.82 / 0.78) 이 값들은 그림마다 다시 잡는다. 옛 값을
그대로 물려받지 말 것 — 그림이 다르면 `dim` 도 `position` 도 다시 재야 한다.
