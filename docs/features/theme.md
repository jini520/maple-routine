...
- **`on-X`**: X의 **색상(H)을 물려받은** 아이보리 또는 짙은 색. 순수 흑/백은 쓰지 않는다 — 배경색마다 어울리는 전경색이 따로 있다. 색 있는 채움 위에는 **아이보리가 기본**이고(주황 위 `#FFEEE2`, 보라 위 `#F7EEFF`), 채움이 아주 밝아 아이보리가 글자로서 사라지는 구간(`L > 0.75`)에서만 짙은 전경으로 넘어간다(파스텔 하늘 위 `#0A1317` 짙은 청록). 채도는 원 색상보다 크게 낮춘다 — 전경은 글자라 색이 튀면 읽기를 방해한다.

  이 자리만 **필수 하한이 3:1**이고 AA(4.5:1)는 권고다([[ADR-064]] 결정 1 재정정). 아이보리를 택한 대가이고, 3:1 아래는 취향이 아니라 글자가 안 보이는 구간이라 하한으로 남겼다. 대비를 못 맞춰도 흑/백으로 달아나지 않는다 — 밝은 쪽에서 명도를 끝까지 밀어봐야 대비는 거의 안 오르고(2.38→2.45) 색조만 씻긴다.
...
**규칙을 완화하는 대신 테마별로 면제한다.** `waivers: ['track/primary']` 처럼 `"토큰/바탕"` 식별자를 주면 그 항목만 실패에서 빠진다. 규칙 자체는 살아 있어 뒤에 올 테마는 계속 걸린다. **면제한 항목도 리포트에서 사라지지 않는다**(`waived` 로 계속 보고) — [[ADR-021]] 의 미달값이 코드에 아무 표시 없이 방치됐던 전례를 반복하지 않으려는 것이다.

현재 면제는 둘 다 머쉬맘이고, 둘 다 밝은 주황(`#F58B0F`)이 원인이다(2026-07-30 사용자 결정).

| 면제 | 대비 | 이유 |
|---|---|---|
| `track/primary` | 1.86:1 | 파생된 어두운 트랙보다 기존 크림 트랙이 낫다([[ADR-064]] 결정 4 정정). 주황과 크림은 휘도 차가 작지만 색상 차가 커서 실제로는 구분되고, 글자가 없는 자리다. |
| `onPrimary/primary` | 2.16:1 | 주황 위 짙은 갈색보다 아이보리가 낫다([[ADR-064]] 결정 1 재정정). `L=0.736` 이 아이보리의 3:1 한계(`L≈0.70`)를 넘는 구간이다. |
