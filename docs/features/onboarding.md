...
## 열린 질문
- **계정의 모든 캐릭터를 고를 수 없게 되는 두 경로를 어떻게 알릴지** — 지금은 둘 다 **설명 없는 빈 목록**이 된다.
  1. **전원 조회 불가**(400 `OPENAPI00003`, 실측 13/13 사례): 예열이 개별 실패를 삼켜([[ADR-016]]) 캐시가 0건이 되고, 피커는 콜드 스타트 경로에서 빈 배열을 "완성된 결과"로 방출한다([[ADR-053]] 결정 2). 온보딩은 그대로 완주된다. [[ADR-068]] 결정 4의 계정 선택 프로브가 첫 방어선이지만, 이미 그 계정으로 온보딩을 끝낸 사용자에게 알릴 자리는 아직 없다.
  2. **전원 장기 미접속**(`access_flag: false`): 조회는 정상(200, 실측)인데 **우리가 스스로 숨긴다** — [[ADR-053]] 결정 1이 `accessFlag: false` 를 목록에서 제외하기 때문이다. 비활성 캐릭터도 과거 기간 백필이 되고 당일 응답도 200이라는 사실([[ADR-067]])에 비추면 그 배제 규칙의 근거가 약해졌다 — **[[ADR-053]] 결정 1 재검토 대상**.
