...
- 모달이 아니라 페이지라 판정 분기는 `ContentCharacterStep` 안의 `RosterBody` 가 직접 그린다(피커와 동일한 우선순위·마크업). 실패도 피커와 같은 공용 `ErrorState` 를 쓰고([[ADR-062]]), 스탈 배너 분기도 같다. **다른 것은 액션뿐** — 온보딩 중에는 설정 화면이 없으므로 `invalidApiKey` 도 **다시 시도**이고 설명만 "API 키를 다시 확인해주세요"로 갈린다(피커는 "설정에서 키를 다시 등록해주세요" + 설정 열기). 빈 상태 문구("표시할 캐릭터가 없어요")는 피커와 같다.
...
## 열린 질문
- **이미 조회 불가 계정으로 온보딩을 끝낸 사용자에게 알릴 자리** — 계정 선택 프로브가 첫 방어선이지만, 그 전에 완주한 사용자에게는 아직 신호가 없다(예열이 개별 실패를 삼켜 캐시가 0건이 되고, 피커는 빈 배열을 "완성된 결과"로 방출한다 — [[ADR-053]] 결정 2). 설정의 계정 변경으로 안내할지.
- **계정의 모든 캐릭터를 고를 수 없게 되는 남은 경로**
  1. **전원 장기 미접속**(`access_flag: false`): 조회는 정상(200, 실측)인데 **우리가 스스로 숨긴다** — [[ADR-053]] 결정 1이 `accessFlag: false` 를 목록에서 제외하기 때문이다. 비활성 캐릭터도 과거 기간 백필이 되고 당일 응답도 200이라는 사실([[ADR-067]])에 비추면 그 배제 규칙의 근거가 약해졌다 — **[[ADR-053]] 결정 1 재검토 대상**.
