...
- 버튼을 빼는 이유는 처방이 재시도가 아니기 때문이다. 이 카드에서는 재시도가 **유일한 진행 수단**이었으므로([[ADR-063]] "남는 인라인" 표) 429 에서는 모달을 닫는 것 외에 진행 경로가 없어지는데, **실제로 지금 할 수 있는 일이 없는 것이 맞다**. 나머지 원인(`network`·`storageWriteFailed`)의 "다시 시도"는 그대로다.
...
## 열린 질문
- **설정에 "API 키 변경" 행을 되살릴지**([[ADR-115]] 결정 8, 이슈 #157 미정 3, 2026-08-08 신설) — **#135(설정 구조 개선)와 함께 본다.** 무효 키로 키를 바꾸는 경로는 [[ADR-115]] 가 안내 모달 + 이동으로 해결했으므로 남은 것은 *"무효화가 아닌데 키를 바꾸고 싶은"* 경우다(다른 넥슨 계정 키로 갈아타기, 서비스 단계 키로 승급 — 후자는 이슈 #158 의 429 안내가 사용자를 그리로 보낸다). `useSettingsStore.changeApiKey` 는 **로직·테스트가 살아 있고 배선만 없다** — 되살린다면 UI 만 붙이면 되지만, 그 경로의 401 은 "방금 나쁜 키를 입력한 것"이라 무효화와 성질이 달라 인라인 카드에 남는다는 것까지는 정해져 있다.
