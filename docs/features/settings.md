...
- 버튼을 빼는 이유는 처방이 재시도가 아니기 때문이다. 이 카드에서는 재시도가 **유일한 진행 수단**이었으므로([[ADR-063]] "남는 인라인" 표) 429 에서는 모달을 닫는 것 외에 진행 경로가 없었다. 나머지 원인(`network`·`storageWriteFailed`)의 "다시 시도"는 그대로다.
  - **이제 그 자리에도 진행 경로가 생긴다**([[ADR-116]] 결정 1, 2026-08-08, 이슈 #178) — 429 는 `invalidApiKey` 와 **같은 사슬**(닫을 수 없는 안내 모달 → `확인` → 키 입력 화면 + 저장된 `apiKey` 삭제)을 타고 문구만 갈린다(`호출 한도를 초과했습니다` / `서비스 단계 키로 다시 입력해주세요.`). 카드의 `rateLimited` 문구·버튼 없음은 **그대로 남는다** — 모달이 덮이기 직전 한 프레임이자 안전망이고, 그 자리에 재시도를 되살리지 않는다는 판단은 유효하다([[ADR-114]] 결정 2).
