...
- **스토어는 «보고 있는 것» 과 «지금 기간» 을 갈라 낸다**([[ADR-153]], 2026-08-19) — `rows`·`dropsByRowKey` 는 사용자가 보고 있는 (탭, 기간) 한 조각이고(`filterRowsForTab` 이 `cycle` 까지 건다), 그와 별도로 **`currentPeriodRows`**(지금 기간의 행 전부, 주기 무관)를 낸다. **이 화면은 전자만 쓰고 후자는 today 위젯이 읽는다** — 그 화면은 언제나 «이번 주» 를 그리므로 이 화면의 네비게이션에 흔들리면 안 된다(월간 탭으로 옮기면 today 의 주간 수익·결정석 한도가 함께 비던 결함). 내용은 `latestSyncSnapshot.rows` 와 같고 갱신 자리는 `commitLatestSyncSnapshot()` 하나뿐이다 — 사본이 아니라 **같은 값의 두 창구**라는 것이 이 배치의 계약이다. `dropsByRowKey` 는 사본을 만들지 않고 «보고 있는 기간 ∪ 지금 기간» 상위집합으로 둔다(키에 `periodKey` 가 있어 충돌 없음, 드롭 편집 경로가 갈리지 않는다).
...
- **셸이 자식을 카드 모양대로 잘라낸다**(⛔ ADR-049 결정 3 에서 살아남은 규칙): 클리핑은 **패딩 박스**(반경 13px = 14 − 테두리 1px)에서 일어나므로, 셸 안쪽에 붙는 장식은 그 곡선을 기준으로 맞춘다 — 골드 링은 펼침 상태에만 반경 13px(아래 "고가 드롭 강조"). 셸 **바깥**의 배지는 클리핑 대상이 아니다.
...
```
- **펼침 헤더는 고정되지 않는다** — 카드 헤더도 목록과 함께 흘러간다([[ADR-131]]). 웹의 중첩 sticky 는 **RN 에서 «안 만들기» 로 판정났다**(🗑 ADR-047). 근거와 되살리는 두 길은 `src/app/boss-profit/BossProfitScreen.contract.md` «못 옮긴 것» 에 있고, 요약하면 RN 의 `stickyHeaderIndices` 는 스크롤 뷰의 **직계 자식**만 붙일 수 있어 목록을 `[헤더, 본문, 헤더, …]` 로 펴야 하는데 그러면 [[ADR-045]] 의 카드 링이 두 조각으로 갈리고 셸 클리핑이 자를 상자를 잃는다.
  - **딸린 셋도 함께 없다** — 배지 sticky 레일 · stuck 헤더 하단 페이드 · 페이지 헤더 실측을 받던 `stickyTop` 프롭. **고가 드롭 배지는 펼침·접힘 모두 [[ADR-045]] 의 원래 구조**(카드 우상단)를 쓴다.
  - **살아남은 것은 카드 안 z 층 순서다**(⛔ ADR-047 결정 4): 드롭 아이콘 < 헤더 < 골드 회전 샤인 링 < 고가 드롭 배지. 링이 헤더보다 낮으면 테두리 상단·좌우가 헤더 배경에 끊긴다.
...
- **소계 footer 없음**(ADR-047 후속 · [[ADR-017]] 의 «합계는 헤더에만» 으로 되돌아간 것) — 캐릭터 합계는 카드 헤더에 있고 하단 중복 표시는 두지 않는다. 셸이 자식을 카드 모양대로 잘라주므로 요소별 하단 라운딩 보정도 필요 없다.
...
- **기간·탭이 바뀌면 최상단에서 시작한다**(⛔ ADR-080 에서 살아남은 **동작**) — 화면이 `ScreenScroll` 의 `ref` 로 직접 부른다. **이유는 바뀌었다**: 웹에서 그것은 *"문서 높이가 붕괴하며 sticky 헤더가 화면 밖에 그려지는 프레임"* 을 없애는 처방이었고 RN 에는 그 사슬이 아예 없다. 남는 것은 **관찰 가능한 동작**뿐이라, 없애면 웹과 다르게 동작하므로 그대로 둔다.
- **아코디언 접기는 상태만 바꾼다 — 스크롤을 건드리지 않는다**(ADR-102 결정 1). 사라지는 본문은 **탭한 헤더보다 항상 아래에** 있으므로, 스크롤을 그대로 두면 그 헤더와 위쪽 내용이 제자리에 남는다(손가락이 있던 자리가 유지된다). 내용이 화면보다 짧아지면 스크롤러가 한 번 클램프할 뿐이고, 그건 평범한 동작이다. **여기에 스크롤 조작을 다시 넣지 말 것** — 접기 전 사전 스크롤(🗑 ADR-085 결정 2)이 정확히 그것이었고, 두 단계가 **두 프레임으로 그려져** 접히기 직전에 페이지가 튀었다(아래 history).

### 화면 레이아웃 — 고정 영역이 없다 ([[ADR-131]])

**제목·탭·총 수익 헤드라인도 목록과 함께 스크롤한다.** 화면 루트는 공용 셸 `ScreenScroll` 이고
`PageHeader` 는 그 **첫 자식**이라 위로 흘러 올라간다. 카드 헤더를 고정하던 중첩 sticky 도 없다
(🗑 ADR-047 — RN 에서 «안 만들기» 로 판정났다).

- **이 «없음» 은 테스트가 지킨다** — `src/__tests__/sticky-policy.test.ts` 가 소스에
  `stickyHeaderIndices` 같은 sticky 수단이 **하나도 없음**을 단언한다. 회귀는 기능이 깨지는 모양이
  아니라 «없기로 한 것이 슬그머니 생기는» 모양으로 오기 때문이다. 카드 헤더를 고정하고 싶어지면
  이 테스트가 먼저 막는다.
- **그래서 딸린 장치들도 함께 사라졌다** — 실측 spacer 와 `ResizeObserver` 갱신 경로
  (🗑 ADR-085·ADR-112), stuck 카드 헤더 하단 페이드(🗑 ADR-047 후속), 페이지 헤더에
  경계 페이드를 쓰지 않는다는 예외 규칙까지. 지금은 **잰 높이에 의존하는 자리가 없다.**
- **살아남은 것은 z 층 순서와 셸 클리핑이다** — 고가 드롭 링이 카드 모서리에서 잘리지 않게 하는
  배치(⛔ ADR-047 결정 4 · ⛔ ADR-049 결정 3)는 지금도 그대로다(아래 「고가 드롭 강조」).
...
목록 최상단에서 아래로 당기면 헤더 새로고침 버튼과 **같은 재조회**(`refresh(trackedOcids ?? [])`)가 돈다 — 그 계약은 웹 시절 그대로다(⛔ ADR-072 결정 2). 제스처·인디케이터 배치는 RN `RefreshControl` 이 맡고, 웹에서 손으로 만들던 훅·배너는 사라졌다(🗑 ADR-072·ADR-073). 레시피는 [foundation/design-system.md](../foundation/design-system.md) 의 '당겨서 새로고침' 절.
...
이 조건 때문에 **`now`·기간 판정 선언이 빈 상태 조기 반환보다 위에 있다** — 훅은 조기 반환 아래에서 부를 수 없고, 훅의 `enabled` 가 그 값을 읽기 때문이다. 두 줄을 다시 아래로 내리지 말 것(`new Date()` 도 렌더당 1회 유지 — 두 번 만들면 "현재 기간 판정"과 "기간 라벨"이 기간 경계를 사이에 두고 갈릴 수 있다).

### 총 수익 헤드라인 (헤더 블록 최하단) — [[ADR-046]]
...
```
접힘 상태는 **월드가 몇 개든 칩 하나**로 고정한다([[ADR-054]] 결정 7·정정 2·3) — 헤더가 커지면 첫 화면에서 목록이 밀려난다. 결정석 아이콘은 `item-icons.json` 에 등록하지 않고 `getItemIconUrlByFile(fileName)` 로 파일명 직접 조회한다(주간/월간 각 1장) — 결정석은 드랍 테이블 항목이 아니라 UI 표시 전용이라 등록하면 `item-icons.test.ts` 의 "드랍 테이블 실재" 정합성 검사를 깬다.
...
### 고가 드롭 강조 — [[ADR-045]]
그 주차에 고가 아이템(`isValuableDrop`)을 먹은 항목을 네온 골드(`#f7d00d`)로 강조. 값은 `app/boss-profit/valuable-card-glow.ts`·`valuable-row-glow.ts` 에 있고(컴포넌트에서 export 하면 fast refresh 가 깨진다), 그리는 것은 `CharacterAccordion`·`ValuableRowBackground` 다.
- **접힘 캐릭터 카드**: 회전하는 골드 링(샤인 테두리) + 글로우 맥동. 우상단 획득 아이템 배지(`Sparkles`+아이템 아이콘). RN 은 그림자 속성을 보간하지 못해 **끝점 둘을 가진 겹을 반대 방향 `opacity` 로 교차**시킨다 — 끝점(`0%`·`50%`) 값은 웹과 같고 중간만 알파 교차다.
- **펼침 상태**: 글로우 맥동만 정지하고 회전 샤인은 유지한다. 링은 카드 안 z 층에서 위에 서고(⛔ ADR-047 결정 4), **링 반경을 13px 로 낮춘다**(⛔ ADR-049 결정 3) — 셸이 패딩 박스(반경 13px)에서 잘라내므로 14px 면 모서리 바깥이 깎인다.
- **펼침 시 고가 획득 보스 행**: 테두리/글로우가 아니라 **배경** — 아이템 쪽에 치우친 골드 글로우 + 미세 틴트 맥동(`ValuableRowBackground`). 가격 기록 화면의 행이 같은 값을 공유한다.
...
## SQLite 안정성
`applyDownloadedLiveUpdate()`(`Updates.reloadAsync()`)가 JS 컨텍스트를 파괴하는 재시작 전에 SQLite 커넥션을 정상 종료하지 않으면 stale 커넥션이 남아 과거 수익 데이터 로드가 조용히 멈춘다 → `storage/sqlite/db.ts` 의 `closeBossProfitDb()` 로 리로드 전 미리 닫음([[ADR-008]] 세 번째 정정). 읽기 조회는 `withSqliteFallback`(타임아웃 폴백), 쓰기는 `withSqliteTimeout`(타임아웃을 실패로 전파 — 성공 위장 시 영구 유실 위험).
...
깜빡임의 **존재**는 다른 데서 온다(아코디언 key에 `periodKey` 가 있어 매번 리마운트 · `periodKey` 를 로드 전에 동기 커밋 · `isPeriodLoading` 이 총 수익 헤드라인을 헤더에서 빼 헤더 높이를 바꿈). ADR-078은 그 상태가 **보이는 시간**만 줄인다.
