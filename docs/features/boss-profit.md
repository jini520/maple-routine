...
**TTL 로 건너뛴 화면 진입에서도 자동 기록하지 않는다**([[ADR-097]] 결정 6). 진입이 곧 조회가 아니게 되면서(`이번 실행에서 이미 동기화함 AND 가장 오래된 syncedAt 이 10분 안` 이면 `syncSchedules` 를 건너뛴다) **동기화 없이 캐시만으로 서는 진입**이 생겼다. 그때도 자동 기록은 하지 않는다 — "실제 재검증 이후에만 기록한다"는 위 [[ADR-017]]·[[ADR-067]] 결정 7의 규칙을 그대로 지킨다. **행 표시는 영향받지 않고**(캐시 우선 표시 경로가 그린다) 기록 생성만 최대 10분, 또는 다음 명시적 새로고침까지 미뤄진다. 편승 갱신된 `character/basic` 을 그 회차 화면에 반영하려면 이 화면만 **동기화 완료 분기에서 프로필을 다시 읽어야 한다** — `getSortedCharacterInfo` 를 `syncSchedules` 보다 먼저 부르는 유일한 화면이라, 다시 읽지 않으면 새 레벨·이미지가 다음 진입으로 밀린다(로컬 읽기라 네트워크 0회, [[ADR-097]] 결정 7).
