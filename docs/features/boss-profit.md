...
- **아코디언을 접으면 페인트 전에 문서 스크롤을 유효 범위로 자른다**([[ADR-084]]) — `useLayoutEffect` 로 펼침 → 접힘 전이에서 `window.scrollTo(0, min(scrollY, scrollHeight - innerHeight))`. 접기도 같은 높이 붕괴(주간 본문 ≈ 보스 행 89pt × 최대 12행)를 일으키는데 [[ADR-080]]의 방어는 `[tab, periodKey]` 에만 걸려 이 경로가 비어 있었다. 프레임의 얼굴은 남은 문서 길이가 정한다 — 뷰포트보다 짧으면 **빈 헤더**(stuck → 비-stuck 전환 시 미도색, [[ADR-077]]), 아직 길면 **헤더가 화면 밖**(스크린샷 계측 2026-08-02: 화면 y=0부터 보스 행 + 형제 카드 아바타가 회색 빈 원 = 레이어 재래스터). **스크롤이 이미 유효 범위면 건드리지 않는다**(고칠 프레임도 없이 위치만 튄다). 펼칠 때는 걸지 않는다 — 문서가 늘어날 때는 클램프가 없다. 목적지가 0이 아닌 경우는 [[ADR-082]]가 경고한 트랜스폼 지연에 걸릴 수 있어 실기기 검증이 남아 있다.
