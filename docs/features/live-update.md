...
- **설정의 `현재 버전` 은 도는 번들이 말한다**([[ADR-279]]). OTA 번들이면 매니페스트 `extra.appVersion`,
  ...
  바이너리를 1.0.7 로 보였다. `package.json` 은 둘 다 못 읽을 때의 마지막 폴백이다.
...
**옛 지문을 뺀 뒤에는 `publish-rn-ota.mjs` 로 그 기기를 더 못 고친다.**
`describePinMismatch` 가 못박은 값과 갱신된 판정값의 차이를 보고 중단시키고, 우회해서 발행하면
`latest-*.json` 이 옛 지문으로 되돌아가 **잠금이 조용히 풀린다.**
