...
- **그 판정은 값 하나가 아니라 목록이다**([[ADR-268]] 결정 2). `latest-*.json` 의
  `acceptedRuntimeVersions` 에 자기 지문이 있으면 통과다. 심사 기간에는 정당한 바이너리가 둘이라
  (스토어에 있는 옛 것 · 심사 중인 새 것) 값 하나로는 그 기간을 표현할 수 없다. 아래
...
## 지금 발행하려면 무엇이 필요한가 (2026-09-14 기준)
...
### 트리 지문이 곧 스토어 바이너리의 지문이다

두 스토어의 1.0.8 이 트리 계산값과 같은 지문으로 게시돼 `PINNED_RUNTIME_VERSIONS` 를 비웠다
(2026-09-14, [[ADR-190]] 결정 4). 이제 발행은 트리 계산값으로만 나간다.

그래서 `.gitignore` 나 `app.json` 처럼 지문 재료를 건드린 트리에서 발행하면 안 된다. 매니페스트
이름이 스토어 기기가 묻는 지문과 갈려 번들이 안 가고, 그 위에 `latest-<platform>.json` 이 새 지문으로
덮여 **스토어 사용자 전원이 잠긴다.** 발행 직전에 트리 지문이 스토어 바이너리와 같은지 본다
([../foundation/release.md](../foundation/release.md) 규칙 4-1).
...
> 네이티브가 낡은 기기를 스토어로 보내되 **닫을 수 없게** 보낸다. `나중에` 로 닫히던 동안 그
> 기기는 새 기능을 못 받으면서 문의는 똑같이 했다.

**두 플랫폼 다 켜졌다.** iOS 는 2026-09-13, 안드로이드는 2026-09-14 다. 둘 다 1.0.6 기기에 번들을
내지 않고 켰다([[ADR-268]] 정정 1).

`store-required` 모달에서 `나중에` 와 버전 배지를 빼고 `onClose` 를 no-op 으로 준다. 남는 버튼은
`스토어로 이동` 하나다. **앱 종료 버튼은 두 플랫폼 다 없다.** iOS 에 스스로 종료하는 길이 없어서
(RN 의 `BackHandler.exitApp` 은 iOS 구현이 빈 함수) 한쪽만 종료되면 같은 화면이 플랫폼마다 다른
말을 하게 된다.

### 판정은 목록이고, 스위치는 그 목록에서 빼는 것이다

```
심사 중   acceptedRuntimeVersions: [옛 지문, 새 지문]   1.0.6 기기 통과 · 심사 담당자 통과
출시 후   acceptedRuntimeVersions: [새 지문]           1.0.6 기기 잠김 · 새 스토어 앱 통과
```

**목록이 아니면 심사 담당자가 먼저 막힌다.** `latest-<platform>.json` 은 우리가 손으로 적는
파일이라 새 바이너리가 스토어에 올라간 **뒤에야** 갱신되는데, 심사는 그보다 앞이다. 그 시점에
심사 담당자 기기는 `자기 지문 ≠ latest.runtimeVersion` 이라 잠긴다. 지금까지는 `나중에` 가 있어
아무도 안 밟았다. 먼저 갱신하면 반대로 기존 사용자가 받을 것 없는 채로 며칠 잠긴다.

**선택 필드다.** `runtimeVersion` 은 그 자리에 그대로 두고 곁에 더한다. 옛 번들은 모르는 필드를
무시하므로 1.0.7·내장 1.0.6 으로 도는 기기의 동작이 안 바뀐다. 없거나 비면 `[runtimeVersion]`
으로 떨어지고, 그 폴백이 곧 지금 동작이라 이 필드가 사라져도 거짓 잠금이 안 생긴다.

**플랫폼마다 따로 친다.** 두 스토어의 게시 시점이 다르다. 되돌리기는 뺀 지문을 다시 넣는 것이다.

### 목록은 커밋되는 상수다

`scripts/ota-runtime-version.mjs` 의 `IN_REVIEW_RUNTIME_VERSIONS` 에 두고, 발행 스크립트가
`acceptedRuntimeVersions` 를 `[발행 지문, ...심사 중 지문]` 으로 쓴다.

**손으로 올린 파일을 다음 발행이 조용히 되돌리기 때문이다.** `publish-rn-ota.mjs` 는 발행마다
`latest-<platform>.json` 을 통째로 다시 쓴다. 심사 중에 발행이 한 번 끼면 목록이 옛 지문 하나로
돌아가 심사 담당자가 잠기고, 그 구간은 `describePinMismatch` 도 못 잡는다(그때는 아직 못박은 값과
발행된 판정값이 같아 가드가 정상으로 읽는다).

값은 [[ADR-190]] 의 못박은 지문과 같은 성질이라 같은 파일에 둔다. 지어내는 값이 아니라
**바이너리에서 읽어 온 사실**이다(`EXUpdates.bundle/fingerprint` · AAB `base/assets/fingerprint`).
...
옛 지문을 다시 못박아야 하는데, 발행 스크립트가 못박은 발행을 거부하고 `describePinMismatch` 도
갱신된 판정값과의 차이를 보고 멈춘다. 우회해서 발행하면 `latest-*.json` 이 옛 지문으로 되돌아가
**잠금이 조용히 풀린다.**

**대가 셋은 구조가 정한 것이다.** 잠금은 그 번들을 받은 기기에만 닿아 옛 번들에 머무는 사람은
`update-available` 을 한 번 거쳐야 판정에 들어온다([[ADR-154]] 의 ‘OTA 한 홉 뒤’ 와 같다) ·
오프라인이면 `checkStoreRequired` 가 조회 실패를 삼켜 안 잠긴다(확인 안 된 것을 근거로 앱을 막지
않는다) · 판정이 부팅 때뿐이라 앱을 계속 켜 두면 다음 콜드 스타트까지 안 잠긴다.
...
- ~~발행할 때마다 에셋 이름표(`OTA_LEGACY_ASSET_MAP`·`OTA_ASSET_REPORT`)를 줘야 한다~~ → **장치를
  걷었다**([[ADR-191]] 정정 1, 2026-09-14). 지문을 1.0.6 에 못박은 동안 안드로이드 이미지 리소스
  이름을 옛 바이너리에 맞추던 장치였다. 두 스토어 1.0.8 이 게시돼 못박기를 비웠고, 새 바이너리의
  드로어블 이름은 트리와 같다.
- ~~트리 지문이 달라져도 발행은 안 막힌다. 못박은 값이 트리 계산값을 이긴다~~ → **못박은 값이
  없다**(2026-09-14). 트리 계산값이 곧 발행 지문이라 지문 재료를 건드린 트리에서는 발행하면 안 된다.
- ~~`PINNED_RUNTIME_VERSIONS` 의 `binaryAppVersion` 이 1.0.6 인데 매니페스트 `appVersion` 이 1.0.7
  인 것이 정상이다~~ → 그 상수가 비었다(2026-09-14). 1.0.7 은 1.0.6 바이너리 위에 얹힌 OTA 였고,
  못박은 지문의 출처 바이너리를 적는 칸이 `binaryAppVersion` 이었다.
