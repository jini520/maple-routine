...
- **버전 형식은 `x.y.z` 또는 `x.y.z+n` 이다**([[ADR-292]], 사용자 지정 2026-09-17). 같은 버전의 버그 수정 OTA 는
  semver 빌드 메타데이터로 패치 번호를 붙이고(`1.0.10+1`), 화면은 `formatAppVersion` 으로 `1.0.10(1)` 을
  보인다. 첫 발행은 번호가 없고 그 뒤가 `+1` 이다. 새것 판정(`isNewerVersion`)은 패치 번호까지 본다.
  `1.0`(2단)은 여전히 안 된다. 그 형식은 OTA 가 한 번도 작동하지 않았던 적이 있다(⛔ ADR-024).
...
- **값의 출처는 `src/data/release-notes.ts`** 다. 배포 스크립트가 `package.json` version 의 기본 버전
  (`1.0.10+1` 이면 `1.0.10`) 노트에서 `highlights` 를 뽑아 싣는다. 패치 항목은 기본 버전 노트에 이어 붙인다
  ([[ADR-292]] 결정 4). 노트를 손으로 매니페스트에 적지 않는다. 같은 파일을 개발 노트 화면
...
- ~~버전 형식은 세 자리다. `native/live-update.ts` 의 파서가 그 형식을 강제한다~~ → **`x.y.z` 또는 `x.y.z+n`**
  ([[ADR-292]], 2026-09-17). 같은 버전의 버그 수정 OTA 에 패치 번호를 붙이려고 넓혔다.
