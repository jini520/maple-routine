...
- **탭 선택은 스토어가 소유한다**([[ADR-096]]) — `features/content-scheduler` 의 `activeTab`. 화면 로컬 state 가 아니므로 다른 탭에 다녀와도 유지되고, 관리 페이지(`/content/manage`)가 **같은 값을 읽어** 보던 탭 그대로 열린다. 앱 재시작 후에는 기본값(`'daily'`)으로 돌아간다(영속화하지 않음).
...
- **탭**: 일간/주간 pill 탭. 값은 스토어 소유라 스케줄러에서 보던 탭으로 열린다([[ADR-096]] 결정 2) — 일간에서 진입하면 일간, 주간에서 진입하면 주간. 여기서 탭을 바꾸면 스케줄러에도 반영된다.
