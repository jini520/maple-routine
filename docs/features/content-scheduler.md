...
**추적 목록의 순서와 대표 (RN 앱만 — [[ADR-143]] 결정 3·4, 2026-08-16, 구현 전)**: 저장된 `trackedCharacters` 배열의 **순서가 곧 표시 순서**다 — 두 스케줄러의 초상화 레일([[ADR-142]])·보스 수익 캐릭터 그룹이 그 순서로 선다. [[ADR-017]] 결정 2 의 레벨 내림차순 정렬은 «순서를 아직 정하지 않았을 때의 초기값» 으로 내려간다(그 결정이 없애려던 «단계마다 순서가 다르다» 는 진실이 배열 하나가 되면서 더 강하게 성립한다). 새로 들어온 캐릭터는 **배열 끝**이고, 전환 설치본을 레벨 순으로 정규화하지 않는다.
