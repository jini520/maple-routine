...
- 원인별 액션: `invalidApiKey` → **이 훅은 토스트를 띄우지 않는다**([[ADR-115]] 결정 7, 2026-08-08 — 키 무효화 진입점에 위임하고, 그쪽이 자기 문구 `API 키가 더 이상 유효하지 않습니다` 를 액션 없이 띄우며 화면을 키 입력으로 옮긴다) · `network` → **다시 시도**(`refresh`) · `rateLimited` → **없음**(지금 누르면 또 429).
  - **토스트의 429 문구는 원인만 말한다** — `호출 한도를 초과했습니다`([[ADR-114]] 결정 1·4). 처방("서비스 단계 키인지 확인해주세요")까지 넣으면 `Toast` 본문이 `truncate` 라 잘린다. 처방은 목록 자리(스탈 배너·`ErrorState`)와 설정 계정 카드가 준다.
