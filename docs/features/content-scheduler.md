...
- **선택 표시가 둘이 됐다** — 미선택 흐림 0.45 → **0.3**, 그리고 선택 칸의 **얼굴 둘레에 얇은
  테두리**(r = 20, 진행 링 r = 26 과 6 떨어져 겹치지 않는다). 테두리는 얼굴 `View` 의 `borderWidth`
  가 아니라 이미 있는 `<Svg>` 층에 그린다 — 그래야 [[ADR-145]] 결정 5 의 «레이아웃이 전혀 안
  움직인다» 가 지켜진다(`borderWidth` 를 주면 선택된 칸의 얼굴만 작아 보인다).
