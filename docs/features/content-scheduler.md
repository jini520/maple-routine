...
- **실패는 원인과 행동을 함께 준다**([[ADR-062]]). 호출부가 reject를 `toScheduleSyncError` 로 변환해 `loadError: ScheduleSyncError | null` 로 내려주고(옛 `loadFailed: boolean` 대체), 공용 `ErrorState` 가 원인별 문구·액션을 그린다 — `invalidApiKey` 만 **설정 열기**(재시도로 안 풀린다), `network` 만 **다시 시도**. 재시도는 피커를 여는 경로와 같은 초기화(`reloadRoster`)를 재사용하므로 열기와 재시도가 한 코드로 수렴한다.
...
- **보여줄 항목이 있는 채로 실패하면 목록을 지우지 않는다**([[ADR-062]] 결정 4). `loadError !== null` 이어도 `entries.length > 0` 이면 그리드를 그대로 두고 그 위에 스탈 배너를 얹는다. 캐시 stub이 네트워크보다 먼저 방출되므로([[ADR-017]] 결정 6) 예열이 끝난 정상 경로에서는 **이쪽이 기본 분기**다 — 배너가 없으면 실패의 대다수가 무음이 된다.
  - **배너 문구·액션도 원인별로 갈린다**([[ADR-114]] 결정 3, 2026-08-08, 이슈 #158). 전에는 원인과 무관하게 "목록이 최신이 아닙니다" + 다시 시도 하나였다 — 그런데 이쪽이 기본 분기이므로 **실패의 대다수가 원인을 잃는 자리**가 정확히 여기였다. `StaleBanner` 는 계층(molecule)을 지켜 `ScheduleSyncError` 를 받지 않고, 호출부가 `formatStaleRosterError(error, place)`(`features/schedule-sync/format.ts`)로 문구·액션을 뽑아 넘긴다.
