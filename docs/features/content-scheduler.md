...
> **여는 자리는 앱마다 다르다** — 웹뷰 앱은 컨텐츠·보스 헤더 버튼과 두 빈 상태 CTA, RN 앱은 `설정 → 캐릭터 관리` **하나**다([[ADR-140]], 2026-08-16). 아래 로딩 정책은 **여는 자리와 무관하게 두 앱에 동일하게** 적용된다.
>
> **후보의 범위도 앱마다 다르다**([[ADR-143]]·[[ADR-144]], 2026-08-16, **구현 전**) — 웹뷰 앱은 «고른 계정 하나» 이고, RN 앱은 «드롭다운이 고른 계정 하나» 다. **`getCharacterPickerRoster` 에게는 같은 일**이다(둘 다 `options.accountId` 로 들어온다, [[ADR-086]] 결정 6이 만든 계약). 갈리는 것은 그 인자를 **누가 정하는가**뿐이고, 그래서 아래 로딩·자격·실패 정책은 RN 의 계정 전환에도 그대로 적용된다 — 계정을 바꾸면 **아래 층(후보 목록) 자리만** 이 정책대로 다시 돈다.
>
...
**추적 목록의 순서와 대표 (RN 앱만 — [[ADR-143]] 결정 3·4, 2026-08-16, **구현 완료 2026-08-17 · 실기기 미검증**)**: 저장된 `trackedCharacters` 배열의 **순서가 곧 표시 순서**다 — 두 스케줄러의 초상화 레일([[ADR-142]])·보스 수익 캐릭터 그룹이 그 순서로 선다. [[ADR-017]] 결정 2 의 레벨 내림차순 정렬은 «순서를 아직 정하지 않았을 때의 초기값» 으로 내려간다(그 결정이 없애려던 «단계마다 순서가 다르다» 는 진실이 배열 하나가 되면서 더 강하게 성립한다). 새로 들어온 캐릭터는 **배열 끝**이고, 전환 설치본을 레벨 순으로 정규화하지 않는다.
- **순서를 얹는 자리는 core 가 아니라 RN 화면이다.** 스토어의 `sortByCachedLevel` 은 웹뷰 앱이 그 정렬로 살아 손대지 않고, RN 이 순수 함수 `orderByTracked(items, orderedOcids)`(`app-rn/src/lib/tracked-order.ts`)를 세 자리에서 통과시킨다 — 컨텐츠 레일 · 보스 레일 · 보스 수익 캐릭터 그룹. 순서 인자는 세 스토어가 이미 들고 있는 `trackedOcids` 라 저장소 조회가 늘지 않는다. **모르는 항목은 버리지 않고 뒤에 남긴다** — 두 목록이 한순간 어긋날 때(저장 직후·동기화 중간 커밋) 캐릭터가 화면에서 사라지지 않게 하려는 것이고, 순서를 정하는 함수가 목록의 크기를 바꾸면 안 된다는 뜻이다.
...
> **RN 앱은 캐릭터를 드롭다운이 아니라 초상화 레일로 고른다**([[ADR-142]], 2026-08-16) — 둘째 줄이 원형 초상화 가로 목록이 되고, 각 초상화의 **테두리 링**이 **가운데를 기준으로 좌·우 반원**으로 갈려 일간(왼쪽)·주간(오른쪽) 진행률을, 아래 호의 곡선 글자가 레벨(왼쪽)과 닉네임(오른쪽)을 말한다. 링이 세는 것은 **화면의 카드 목록과 같은 것**이다(같은 함수에서 나온다). 레시피는 [foundation/design-system.md](../foundation/design-system.md) 「캐릭터 초상화 레일」.

> **RN 앱은 동기화 상태를 제목 줄에 둔다**([[ADR-141]], 2026-08-16) — `n분 전`/`조회 중...` + 새로고침 아이콘이 캐릭터 드롭다운 줄 오른쪽(`ml-auto`)에서 **제목 바로 옆**으로 올라갔다. 줄 수는 2로 그대로이고, 관리 버튼(`컨텐츠 관리`)은 제목 줄 오른쪽 끝을 지킨다. 좁은 폭에서는 시각 텍스트만 한 줄로 잘린다(제목·관리 버튼은 `shrink-0`). 아래 서술은 **웹뷰 앱의 현행 동작**이다.
...
- **헤더 진입점**: 수동 모드에서만 "컨텐츠 관리" 텍스트 버튼이 "캐릭터 관리" 왼쪽에 나타남(자동은 "캐릭터 관리" 하나). 버튼 `text-sm font-medium text-text-muted hover:text-text`.
  > **RN 앱에는 "캐릭터 관리" 버튼이 없다**([[ADR-140]], 2026-08-16) — 진입점이 `설정 → 캐릭터 관리` 한 곳으로 모였다. 그래서 자동 모드에서는 헤더 오른쪽이 비고, 수동 모드에는 "컨텐츠 관리" 하나만 선다. 위 서술은 **웹뷰 앱의 현행 동작**이다.
...
- **캐릭터 미선택 빈 상태의 판정은 `trackedOcids !== null && trackedOcids.length === 0`**([[ADR-101]] 결정 1) — `null` 은 "0명"이 아니라 "저장소를 아직 안 읽었다"다. 둘을 `||` 로 묶으면 콜드 스타트 첫 페인트가 "표시할 캐릭터가 없습니다"를 한 프레임 그린다. 세 화면 공통 규칙이며 원칙은 [error-resilience.md](../foundation/error-resilience.md) 제품 원칙 2, 남는 로딩 프레임을 치우는 부팅 선하이드레이션은 [architecture.md](../foundation/architecture.md) '상태 관리'.
  - 그 빈 상태의 CTA("캐릭터 선택하기")는 웹뷰 앱에서는 **이 화면 위에 피커를 띄우고**, RN 앱에서는 **설정 탭을 피커가 열린 채로 연다**([[ADR-140]] 결정 1·2).
...
- **헤더**: 뒤로(`ArrowLeft`) + "컨텐츠 관리" + 대상 캐릭터 **드롭다운**(`CharacterSelectDropdown size="compact"`, [[ADR-096]] 결정 4·5). 이 화면에서 캐릭터를 바꿀 수 있고, 스케줄러와 **같은 `selectCharacter`** 를 호출하므로 돌아갔을 때 그쪽도 같은 캐릭터다.
  > **RN 앱은 그 자리에 초상화 레일을 둔다**([[ADR-142]] 정정 8, 2026-08-16) — 제목 줄이 아니라 **그 아래 줄**이고, **진행 링은 없다**(레벨·닉네임만). 같은 `selectCharacter` 를 부르는 것은 그대로다.
