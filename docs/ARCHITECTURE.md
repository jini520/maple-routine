...
"파티 관리" 버튼 탭(캐릭터 관리 버튼 옆)
  → 현재 선택된 캐릭터의 등록된 보스 전체(주간+월간 통합 한 목록) 모달(`PartyManagementModal`), 보스별 숫자 입력(1인~해당 보스 `maxPartySize`, 새 게임 데이터 아님 — 기존 `boss-crystal-prices.json`의 검증 범위 그대로 재사용, [[ADR-006]]), 입력 포커스 아웃 시 개별 저장
