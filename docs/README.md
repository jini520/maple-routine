...
| 사냥 수익 · 지출 · 유틸리티 | **기능 문서 없음** — 사냥 수익·유틸리티는 [[ADR-132]] 뿐, **지출은 [[ADR-166]]** 이 정책을 갖는다 | `app/hunting-profit/` · `app/spend/` · `app/utility/` — 전부 «개발 진행중» 껍데기다. 내용을 만들 때 이 표에 문서를 함께 추가할 것. 지출의 기능 문서는 화면 자리가 정해질 때(#187) 만든다 |
