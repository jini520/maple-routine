...
| 가계부 (수익·지출 캘린더) | [features/cashbook.md](./features/cashbook.md) | `app/cashbook/`(`CashbookScreen`) · `lib/calendar-month`(격자·열지도) · `lib/cashbook/meso-compact`(좁은 칸 표기) · `components/molecules/CalendarMonth/` · `navigation/`(하위 탭 둘 — [[ADR-169]] 결정 1). **전부 섰다** — 캘린더([[ADR-169]]) · 손입력과 주간 보기([[ADR-170]]·[[ADR-166]], 참조 데이터 `src/data/spend-catalog.json`) · 고치기·지우기([[ADR-171]]) · **보스 수익의 날짜**([[ADR-172]], #239 — `defeated_on` · `features/boss-profit/defeat-dates`). **사냥 갈래는 계산기다**([[ADR-175]] — `src/data/hunting-grounds.json` · `lib/cashbook/hunting-grounds` · `lib/cashbook/hunting-meso` · `lib/assets/asset-lookup` · `src/assets/force/`) 이고 **메소 획득량은 캐릭터에서 읽는다**([[ADR-177]] — `lib/cashbook/meso-rate` · `nexon/meso-rate` · `storage/meso-rate-cache`). 남은 원천은 사냥 타이머 자동 수익 하나다. 사냥 수익·지출 껍데기 탭 둘은 삭제됐다([[ADR-169]] 결정 2) |
