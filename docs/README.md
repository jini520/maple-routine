...
| 설정 | [features/settings.md](./features/settings.md) | `app/settings/`(`SettingsScreen` + 하위 화면 `SettingsReleaseNotesScreen`/`SettingsAccountDataScreen`/`SettingsAboutScreen` — 라우트 `/settings/release-notes`·`/settings/account-data`·`/settings/about`, `/settings` 의 **형제**) · 행 프리미티브 `SettingsRow`/`SettingsLinkRow`/`row-class.ts` · `src/data/release-notes.ts`(+`src/types/release-notes.ts`) · `features/settings/`(`cache-data`) · `storage/api-key` · `features/tracking-mode` |
