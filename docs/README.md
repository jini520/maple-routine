...
| 보스 스케줄러 (파티 관리 포함) | [features/boss-scheduler.md](./features/boss-scheduler.md) | `app/boss-scheduler/` · `features/boss-scheduler/` · `storage/boss-party-settings` · `lib/artwork` · `lib/boss-matching` |
...
| 아이템 드랍 | [features/item-drop.md](./features/item-drop.md) | `app/item-drop/` · `features/item-drop/` · `lib/artwork` · `storage/boss-drop-records` · 전 기간 히스토리: `app/boss-profit/DropHistoryScreen`(`/profit/drops`) · `features/boss-profit/drop-history-store` · `lib/drop-history` · 잎 램프(RN, today 와 공유) `constants/style/drought-tier-styles` |
...
| 가계부 (수익·지출 캘린더) | [features/cashbook.md](./features/cashbook.md) | `app/cashbook/`(`CashbookScreen`) · `lib/calendar-month`(격자·열지도) · `lib/meso-compact`(좁은 칸 표기) · `components/molecules/CalendarMonth/` · `navigation/`(하위 탭 둘 — [[ADR-169]] 결정 1). **전부 섰다** — 캘린더([[ADR-169]]) · 손입력과 주간 보기([[ADR-170]]·[[ADR-166]], 참조 데이터 `src/data/spend-catalog.json`) · 고치기·지우기([[ADR-171]]) · **보스 수익의 날짜**([[ADR-172]], #239 — `defeated_on` · `features/boss-profit/defeat-dates`). **사냥 갈래는 계산기다**([[ADR-175]] — `src/data/hunting-grounds.json` · `lib/hunting-grounds` · `lib/hunting-meso` · `lib/artwork` · `src/assets/force/`) 이고 **메소 획득량은 캐릭터에서 읽는다**([[ADR-177]] — `lib/meso-rate` · `nexon/meso-rate` · `storage/meso-rate-cache`). 남은 원천은 사냥 타이머 자동 수익 하나다. 사냥 수익·지출 껍데기 탭 둘은 삭제됐다([[ADR-169]] 결정 2) |
...
| 테마 시스템 | [features/theme.md](./features/theme.md) | `features/theme/` · `storage/theme` · `src/theme/theme-vars.ts`(NativeWind `vars()`) · `global.css` · `src/data/job-themes.json` · `lib/theme-derive` · `lib/artwork` · `src/assets/themes/`(+ `src/assets/generated/themes.ts`) · `lib/color` |
