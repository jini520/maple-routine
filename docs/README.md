...
| 이어서 하는 작업 (기존 기록을 다시 쓰는 작업의 진행률 · 앱이 중간에 닫힌 뒤의 재진행 확인. **설계만, 구현 전**, [[ADR-307]]) | [foundation/error-resilience.md](./foundation/error-resilience.md) | `features/resumable-task/` · `app/resumable-task/` · `components/organisms/TaskProgressModal/` · `components/organisms/TaskResumeModal/` · `storage/pending-tasks` |
