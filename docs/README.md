...
| 설정 | [features/settings.md](./features/settings.md) | `app/settings/`(`SettingsScreen` + 하위 화면 `ReleaseNotesScreen`/`AccountDataScreen`/`AboutScreen` — 라우트 `/settings/release-notes`·`/settings/account-data`·`/settings/about`) · `src/data/release-notes.ts` · `features/settings/` · `storage/api-key` · `features/tracking-mode` |
