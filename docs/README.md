...
| 유틸리티 (도구 목록 + 아이템 분배 계산기) | [features/utility.md](./features/utility.md) | `app/utility/`(`UtilityScreen` 목록 · `ItemSplitScreen` 계산기) · `lib/item-split` · `navigation/routes`(`UtilityItemSplit` — 도구는 루트 스택 push, [[ADR-168]] 결정 6) |
| 사냥 수익 · 지출 | **기능 문서 없음** — 사냥 수익은 [[ADR-132]] 뿐, **지출은 [[ADR-166]]** 이 정책을 갖는다 | `app/hunting-profit/` · `app/spend/` — 둘 다 «개발 진행중» 껍데기다. 내용을 만들 때 이 표에 문서를 함께 추가할 것. 지출의 기능 문서는 화면 자리가 정해질 때(#187) 만든다 |
