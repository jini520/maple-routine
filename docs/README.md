...
| MVP 등급 (메이플 ID 마다 등급을 시작 날짜와 함께 받아 수수료 기본값과 스타포스 할인에 쓴다. **설계만, 구현 전**, [[ADR-306]]) | [features/mvp-grade.md](./features/mvp-grade.md) | `src/data/mvp-grades.json` · `lib/mvp/` · `features/mvp-grade/` · `app/mvp-grade/` · `storage/mvp-grades` · `storage/character-accounts` · 명패 `src/assets/mvp/` · 비용 식 `lib/enhancement/cost` · 드롭 몫 `lib/drop/drop-price` |
