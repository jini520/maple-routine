...
- **`scheduler-content-catalog.json`** ([[ADR-030]]): character 범위(기본값)가 아닌 예외만 등록: `worldShared`/`accountShared` 배열(`{ name, section }`), `maxCountOverrides`(`{ 항목명: 고정값 }`). `section` 은 daily/weekly 어디서 오는지(빈 응답 stale 시 원장 복원용). `maxCountOverrides` 는 API 알려진 오류 필드 덮어쓰기 전용. 조회는 `lib/scheduler/scheduler-content-scope.ts`. [[ADR-006]] 취지 준용.
