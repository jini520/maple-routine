...
- **여백은 변형이 아니라 `size` 가 정한다**([[ADR-230]] 정정 5). 여섯 변형이 전부 같은 `px-5 py-2.5` 를 적고 있었으므로 그것은 변형의 성질이 아니었다. 표가 둘로 갈린다.

  | `size` | 상자 | 라벨 | 높이 | 어디에 |
  |---|---|---|---|---|
  | `default` | `px-5 py-2.5` | 변형 표 그대로 | 40 | 그 밖 전부 |
  | `compact` | `px-3.5 py-1.5` | `text-13` | 32 | 카드나 배너 **안에** 끼는 버튼 |

  **`className` 으로 여백을 덮으려 하지 말 것.** `px-3.5` 와 `px-5` 는 같은 속성이라 이어 붙인 순서가 아니라 **생성된 CSS 순서**가 이기고, 그 순서는 테일윈드가 정한다. 조용히 안 덮인다.
  **글자 크기도 마찬가지다.** `text-13` 은 `text-sm` 을 못 덮었고(실측 `fontSize` 14), `text-sm` 은 `text-base` 를 덮는다. 이름 쌍마다 갈리므로 예측하지 말 것. 그래서 라벨 크기는 `buttonTextSizeClass(variant, size)` 가 **한 벌만** 낸다 - 겹치는 클래스가 없으면 이기고 지는 문제가 없다.
  **`compact` 는 44px 권장 타깃 아래다.** 시각 크기를 줄이는 대신 `hitSlop` 으로 손가락 자리를 되돌려 준다. 좌우로 넓히면 나란히 선 버튼끼리 겹치므로 **위아래로만** 준다.
- **변형이 일곱이다**([[ADR-217]] 정정 4 · [[ADR-230]] 결정 6 · 정정 6). `primaryOutline` 은 **테마색 테두리 pill** 이다 - `outline` 과 같은 자리에 서되 선이 중립이 아니라 `primary` 다.
  ⚠️ **글자가 `primary-ink` 라 밝은 테마에서 대비가 낮다**(사용자 결정). 머쉬맘 2.38 · 엔젤릭버스터 2.26 · 검은마법사 3.96 으로 AA(4.5) 미달이다. 색을 맞추는 쪽을 택한 것이므로 알고 둔 값이고, 고칠 자리는 이 변형이 아니라 `primary-ink` 파생 규칙이다([[ADR-064]] 결정 1).
- **`tint` 는 지금 소비자가 0 이다**([[ADR-230]] 정정 6) - 만들게 한 today 배너 버튼이 `primaryOutline` 로 옮겨 갔다. 표에 남긴 것은 채운 알약보다 한 단 약한 자리가 다음에도 생기기 때문이지, 지금 쓰이고 있어서가 아니다. `danger` 옆에 `dangerText` 가 있다. 상자는 `text` 와 같고
  글자색만 `error-ink` 인 것으로, **되돌릴 수 없는 동작을 작게 내리는 자리**다(파괴적 확인 모달의
  `삭제` · `연결 해제`). 테두리를 두르면 그것대로 눈길을 끌어 안전한 기본값인 취소와 비중이 붙는다.
  파괴 동작이 취소보다 커 보이면 안 되고, 위험 신호는 색 하나로 충분하다.
- **`tint` 는 채운 알약보다 한 단 약한 주 동작이다**([[ADR-230]] 결정 6). 상자가
  `bg-primary-tint` 이고 글자는 `text-text` 다. `outline` 과 같은 자리에 서되 테두리 대신 옅은
  면을 쓴다. 첫 사용처는 today 공지 배너의 `자세히 보기` 이고, 그 자리가 요구한 것이 **주 동작인데
  주변보다 세면 안 된다** 였다. 글자색을 `primary-ink` 로 두지 않는 것이 이 변형의 핵심이다.
  옅은 틴트 위에 같은 계열 잉크를 얹으면 테마에 따라 대비가 1.6 까지 떨어진다(머쉬맘 실측).
- **색을 `textClassName` 으로 덮지 말 것.** 변형의 글자색과 같은 특이도라 생성된 CSS 순서가 이긴다.
  크기(`text-xs`)는 덮이는데 색은 안 덮여 조용히 회색으로 남는다(실측). 색이 필요하면 변형을 만든다.
...
**제스처 뿌리를 모달 안에 한 번 더 세운다** (2026-09-23, 실기기 계측). 안드로이드에서 RN `Modal` 은
**별도 네이티브 창**이라 앱 루트의 `GestureHandlerRootView` 밖이고, 그 창 안의 `react-native-gesture-handler`
제스처는 이벤트를 하나도 못 받는다. 탭은 RN 응답자 시스템이라 멀쩡해서 증상이 **눌리는데 끌리지 않는다**
로 온다(파티 관리 모달의 비율 슬라이더를 끌 수 없다는 보고). 갤럭시 Z Flip3 에서 같은 스와이프를 넣어
잰 값이다.

| `ShareField` 가 선 자리 | `onChange` 건수 |
| --- | --- |
| 일반 트리 | 17 |
| RN `Modal` 안 | **0** |
| RN `Modal` 안 + 제스처 뿌리 | **22** |

`Modal` 이 `RNModal` 바로 안에서 `GestureHandlerRootView`(`flex: 1`)로 내용을 감싼다. iOS 에서도 같이
두는 것은 그 중첩이 해롭지 않고, 플랫폼마다 트리가 갈리면 이 자리를 만질 때마다 둘을 따로 확인해야
하기 때문이다. **모달 안에 제스처를 쓰는 부품을 새로 넣을 때 이 래퍼가 있는지 먼저 볼 것** - 없으면
그 부품은 안드로이드에서 조용히 죽는다.
...
[옵션]        머리 밖의 자유 블록 (선택). 콜아웃 · 펼침판
  ↕ 20
[주 버튼]     variant="primary" · 전폭. **되돌릴 수 없는 확인에서는 이 자리가 `취소`**
  ↕ 4        부 버튼일 때. 링크는 12
[부 버튼 또는 링크]   danger 는 크기가 같고 글자색만 error-ink
```
...
- **되돌릴 수 없는 확인에서는 주 버튼이 `취소` 다**([[ADR-217]] 정정 4). 안전한 기본값이 무르는
  쪽이고 채운 알약이 그것을 가리켜야 한다. 파괴 동작은 `secondaryAction` 의 `danger` 로 내린다.
  크기는 `나중에` 와 같고 글자색만 갈린다.
- 쓰는 곳 다섯: `ApiKeyNoticeModal` · `DevelopmentStageKeyModal` · `UpdatePromptModal`(아홉 상태 중
  일곱) · `DisconnectConfirm` · `CacheClearConfirm`.
...
- **배지도 버튼도 없는 상태.** 업데이트 모달의 `downloading` · `applying` 이다. `action` 이 필수라
  ...
  글자끼리의 8 보다 숨이 필요하다.
- **고르는 화면.** `ThemeModal` · `TrackingModeModal` 은 배지가 없고 제목이 왼쪽 정렬 `text-lg`
  이며, 본문이 문장이 아니라 선택기다. 취소가 안전한 기본값도 아니라 주 버튼이 될 수 없다.
  `Modal.Card` 를 직접 쓴다(사용자 지정, 2026-09-05).

### 갱신 시각 한 줄 (`components/molecules/DataFreshness`): [[ADR-231]] (2026-09-08)

화면이 그리는 데이터가 **언제 것인가**를 말하는 한 줄. 제목 줄 바로 아래에 선다.

```
<DataFreshness fetchedAt={fetchedAt} />   →  14:03:22 기준
                                             text-11 text-text-disabled
```

- **값을 프롭으로 받는다.** 스토어를 이 안에서 읽으면 `components/` 가 `features/` 를 알게 되고
  그 방향은 계층 테스트가 막는다.
- **화면이 그 값을 재지 않는다**([[ADR-231]] 정정 1·2). **실시간 조회 하나**가 움직이는 값을
  다섯 화면이 함께 읽는다(`features/refresh/freshness`). 화면마다 재면 같은 한 번의 조회로 그린
  데이터인데 값이 갈리고(실사용에서 1초 차이로 드러났다), 확정된 과거 기록을 열기만 해도
  시각이 갱신된다.
- **받은 적이 없어도 자리를 지킨다**([[ADR-231]] 정정 3). 글자만 비고 높이는 남는다. 값이
  들어오는 순간 줄이 생기면 헤더가 16 만큼 내려앉고 그 아래 화면 전체가 밀린다.
- **그 줄은 `PageHeaderTitleRow` 가 그린다**([[ADR-231]] 정정 4). 화면이 자리를 고르지 않는다.
  제목 줄과 갱신 시각 줄이 한 덩어리이고 그 덩어리가 곧 프리미티브다. 제목을 그리는 24 자리가
  전부 그것을 쓰므로 예외가 없다 - 헤더 셸을 쓰든 손으로 그리든, 목록 가지든 빈 상태 가지든
  같은 높이다.
- **덩어리 안에 `gap-*` 이 없다**(사용자 지정 2026-09-08). `min-h-8` 이 제목 글자 아래에 이미
  여백을 남긴다. 거기에 간격을 더하면 둘이 떨어져 보인다.
- **값이 없는 화면도 그 줄을 그린다.** 글자만 비고 자리는 남는다. 없는 시각을 지어 적지 않는다.
- **절대 시각이다.** `12분 전` 같은 상대 표기는 화면을 보고 있는 동안 계속 틀려지는데 그 줄을
  다시 그리지 않는다.
- **값을 싣는 곳 다섯**: today · 컨텐츠 스케줄러 · 보스 스케줄러 · 보스 수익 · 가계부. **당김을
  가진 화면 전부**다. 나머지 헤더는 같은 줄을 빈 채로 그려 높이만 맞춘다.
...
- **단계 전환 흐림의 재질도 `mode` 가 고른다**([[ADR-244]] 정정 5). `expo-blur` 의 기본
  `tint="default"` 는 iOS 의 적응형 `.regular` 이라 **OS 외형**을 따라가, 다크 OS + 라이트
  테마에서 흰 시트 위에 검은 재질이 얹혔다(사용자 보고). `useSheetBlurTint()` 가 고정 재질
  (`systemMaterialLight`·`systemMaterialDark`)을 낸다. 유리 판의 `colorScheme` 과 같은 규칙이다.
...
**시트는 키보드를 안 본다**([[ADR-302]] 결정 1 · 배선 삭제 완료 2026-09-21). 상한은
`frame.height * 0.82` 하나이고 키보드 높이를 빼지 않는다([[ADR-240]] 폐기). 시트에 치는 칸이
하나도 없으므로 **키보드 리스너도, 그것을 시트에 알리던 길도 없다**. 자리를 옮기는 시간은 380ms
한 갈래다(키보드에 맞출 것이 없어졌다).

**바닥 줄은 스크롤 내용의 마지막 줄**이다. 떼어 세우던 층(`SheetFooterLayer`)은 그 조건이
「키보드가 떠 있을 때」 하나였어서 함께 걷혔다. 머리만 흐름 밖이고, 그 몫은 스크롤 내용의
`paddingTop` 이 되돌려 준다.
...
### 입력 카드 (`components/organisms/InputCard`): [[ADR-302]] (2026-09-20)

**치는 칸은 시트가 아니라 키보드 위에 뜬 카드가 받는다.** 시트는 키보드에 반응하지 않고, 시트의
칸은 값을 보여 주는 누르개가 된다. 실제 `TextInput` 은 카드에만 산다.

```
스크림 둘 (시트의 것 + 카드의 것)
카드   left 12 · right 12 · bottom 키보드 높이 + 12
  머리   표식 28 + (칸 이름 14/700 · 맥락 줄 11/400) + 닫기 44
  입력   h-56 · 테두리 1.5 primary
           숫자  왼쪽 한국어 읽기(1200만) · 오른쪽 값 24/700 + 단위
           글자  왼쪽 정렬 값 20/600
  칩     오른쪽 정렬로 감싸 내림
  확인   h-44 · 폭 전부
```

- **칸 하나를 받고 닫힌다.** 다음·이전 버튼이 없어 버튼은 `확인` 하나다. 다음 칸은 시트의 그
  줄을 다시 눌러 새 카드를 연다. 폼마다 치는 칸의 차례를 들 일이 없어지는 것이 이 모양의 값이다.
  조건에 따라 섰다 말았다 하는 줄이 셋이라(메소 획득량 · 시세 · 수량) 그 목록은 고정으로 둘 수
  없는 것이었다.
- **닫는 것은 `✕` 뿐이다.** `✕` 와 안드로이드 뒤로가기가 카드를 닫고 친 값을 버린다. 스크림
  탭은 **안 닫는다** — 판 밖을 실수로 눌러 치던 값이 날아가던 것을 없앴다.
- **판의 빈 자리도 스크림도, 누르면 키보드만 내린다.** 카드 안팎이 한 규칙이다. 카드는 제자리에
  남고(자리는 그 카드가 본 가장 큰 키보드 높이를 붙들고 있다) 다시 치려면 입력 칸을 한 번 더
  누른다. 터치는 스크림에서 멈춰, 카드가 열린 동안 뒤의 시트는 안 눌린다.
- **셈한 결과를 안 그린다.** 합계 · 실수령 · 1인당은 확인을 누르고 시트로 돌아가면 큰 숫자
  자리(`AmountFigure`)에 이미 서 있다. 그래서 카드가 받는 것은 여섯이고 전부 시트가 이미 아는
  값이다. 칸 이름 · 맥락 줄 · 현재 값 · 단위 · 칩 눈금 · 글자인가 숫자인가.
- **빠른 칩 눈금은 칸이 세는 단위를 따른다.** 메소는 `MESO_QUICK_ADDS` 다섯, 시세(메포)는
  `+1000`~`+1` 넷, 개수와 %는 `+10`·`+1` 둘, 글자는 없다. **조각 개당 값만 예외**로 같은 10배
  눈금을 두 자리 내려 쓴다(`FRAGMENT_PRICE_QUICK_ADDS` · `+1만`~`+100만`). 칸이 세는 단위는
  메소가 맞지만 그 칸에 들어오는 수의 자릿수가 다르다.
- **글자는 왼쪽 정렬이다.** 시트의 오른쪽 정렬은 값을 읽는 자리의 규칙이고, 치는 동안에는
  커서가 글자를 따라가는 쪽이 읽힌다.
- **머리 표식은 셋으로 갈린다.** 메소를 받는 칸은 `meso_pouch`, 조각 개수는
  `sol_erda_fragment`, 나머지는 없다. 메소 칸에 아이템 그림을 달지 않는 것은 그 카드가 받는
  값이 아이템이 아니라 그 아이템의 값이어서다.
- **`organisms/BottomSheet/` 안에 두지 않는다.** 카드는 시트 밖 오버레이이고, 드롭 판매가처럼
  시트가 아닌 자리에서도 뜬다.
- **시트가 카드를 직접 그리지 않는다.** 앱 셸(`App.tsx`)이 `BottomSheetModalProvider` **뒤에**
  `InputCardHost` 를 한 번만 세우고, 시트는 `features/input-card/store` 의 `openInputCard` 로
  칸을 맡긴다. 뒤에 세우는 것이 핵심이다 — 시트는 그 프로바이더가 자기 자식들 뒤에 그리므로,
  시트 안에서 그린 것은 무엇으로 감싸도 시트 위로 못 올라간다.
- **판을 `transform` 으로 올린다.** 레이아웃 속성(`marginBottom`)을 Reanimated 로 움직이면 그림만
...
시안 캔버스는 https://claude.ai/artifact/6LH1TTLskASBHMxjGRnCcN 의 둘째 줄이다.
...
### 파티 분배 요약: `PartyShareSummary`, [[ADR-312]] 결정 3 (2026-09-23)

`변경` 글자 버튼 옆에 **강조색 배지 하나**가 선다. 셋 중 하나이고 `66.7%` 는 결정석 몫이다.

```
균등 · 혼자     균등 · 둘 이상   비율
솔로            파티 3인         66.7%

크기 default  배지 default · gap 2       보스 관리 행
크기 compact  배지 mini · gap [7px]      보스 수익 카드
```

- **라벨이 없다.** 비율이 결정석에만 있어 무엇의 비율인가를 물을 일이 없다([[ADR-312]] 결정 1).
- **모양이 하나라 행 높이가 안 갈린다.** 균등과 비율이 다른 모양이던 때는 같은 목록의 보스 카드
  높이가 분배 방식마다 달랐다.
- 값은 **백분율**이다(`formatSharePercent`). `내 비율 : 나머지` 로 적으면 `2:1` 과 `4:2` 가 다른
  값처럼 보이는데 둘은 같은 약속이다.
- **두 크기가 같은 수를 적는다**(소수 첫째 자리까지). 크기만 갈린다 — 보스 수익 카드는 이 줄을 금액과 나눠 쓰므로
  배지가 작다. 카드만 반올림했더니 같은 비율이 화면마다 다른 수로 보였다(사용자 지정).
- 혼자면 `솔로` 라 적고 인원을 세지 않는다(`파티 1인` 은 파티가 아니다).
...
합        오른쪽 w-7 세로: PlusIcon / 수(text-15) / MinusIcon · 상한 10 하한 2 에서 흐림
...
- **합의 상한 10 은 입력 편의에서 온 값이다**(9 에서 올림, 사용자 지정 2026-09-23). 게임 규칙이 아니고
  [[ADR-305]] 의 열린 질문이다. 쓰는 곳 셋이 한 상한을 나눠 쓴다.
- 쓰는 곳 셋이 같은 부품이다. 파티 인원 모달 · 드롭 가격 입력 카드 · 판매 분배금 계산기.
  세 자리가 같은 것을 묻는데 모양이 갈리면 값이 무엇을 뜻하는지 매번 다시 읽어야 한다.
- **트랙 높이는 벌마다 다르다.** `wide` 44 · `row` 36 · `stacked` 24(사용자 지정 2026-09-23). `stacked` 은 칸이
  판의 반쪽이라 트랙을 키운 만큼 카드가 키보드 쪽으로 자란다. 24 로 줄여도 **끌기는 6px 막대 위에서 잡혀
  그대로 되고**, 잃는 것은 잡는 자리의 여유와 칸 탭의 정확도다.
- **트랙에 `flex-1` 을 주는 것은 가로로 설 때뿐이다**(2026-09-23 실측). RN 의 `flex: 1` 은 `flexBasis: 0` 이라
  `stacked`(트랙과 합이 위아래로 서는 벌)에서는 주축인 **높이를 0 으로 덮는다**. 그러면 6px 막대만 상자 밖으로
  넘쳐 그려져 멀쩡해 보이는데, 칸 누르개(`absolute inset-0`)가 함께 0 이 돼 **눌러 옮기기가 죽는다**(끌기는
  막대 위에서 잡혀 남는다). 드롭 가격 카드에서 그렇게 서 있었고 칸이 44 만큼 짧았다.
- **`stacked` · `wide` 의 머리 줄은 `items-start` 다**(사용자 지정 2026-09-23). 라벨이 11px, 백분율이 23px 라
  `items-baseline` 이면 라벨이 백분율의 아랫변에 붙어 7px 내려앉는다. 카드 좌상단에 서야 옆 칸의 라벨과
  윗변이 맞는다(드롭 가격 카드의 `파티 인원`). 세로로 서는 `row` 는 백분율이 작아 지금도 `items-center` 다.

### 캐릭터 카드 그리드(다중 선택): 옛 `CharacterTrackingPicker`, [[ADR-015]] — **기록**

> **이 모양은 떠났고 코드도 없다.** 3열 그리드 모달이 **두 층의 페이지**가 됐고([[ADR-144]], 2026-08-16), `CharacterTrackingPicker/` 디렉터리는 2026-09-03 에 지웠다([[ADR-144]] 정정 3). **아래에 나오는 `CharacterTrackingPicker`·`CharacterTrackingGrid`·`ROSTER_BODY_MIN_H` 는 `src/` 에 없다.** 지금 이 자리를 그리는 것은 `organisms/CharacterManage` 의 `CharacterManageBody` 이고, 얼굴 원은 `molecules/CharacterAvatar` 다(위 절). 아래 그리드 서술은 기록이다. 두 층 다 **가로 행 카드**(얼굴 + 1줄 ‘[월드 엠블럼] 이름’ + 2줄 ‘Lv.294 아크메이지(썬, 콜)’)이고 갈리는 것은 좌우 슬롯뿐이다. 위(계정을 넘는 ‘선택됨’)는 왼쪽에 **가로 3줄 끌기 핸들**, 오른쪽에 `★`(대표)·`✕`, 아래(계정 하나의 후보)는 오른쪽에 `＋` 하나. **직업이 카드에 선다**. [[ADR-015]] 결정 2의 ‘직업 미표시’는 3열 칸이 좁다는 근거였고, 행 카드에는 그 제약이 없다([[ADR-144]] 결정 2). **선택이 ‘별 토글’이 아니라 ‘카드 이동’** 이라 후보 카드에는 별이 없다. 아래 명세는 **웹뷰 앱**의 것이고, 살아남는 것은 얼굴 크롭 기법([[ADR-015]])·엠블럼 규칙·레벨 내림차순 정렬이다.
...
**스켈레톤 (`atoms/Skeleton`)**: [[ADR-287]] 결정 1·2 와 정정 1, 2026-09-16. **셸 승계 카드가 못 덮는 자리에만 쓴다** — 결과가 들어올 껍데기가 **자리마다 다를 때**다. 지금 그 자리는 소식 둘이다: 더보기 화면의 갈래 다섯(배너 둘은 높이가 `폭 ÷ noticeBannerRatio(kind)` 이고 글 셋은 줄 수가 셋·셋·둘이라, 승계할 카드가 하나가 아니다)과 `전체` 목록 화면(`SettingsNoticesScreen`). **결과의 실제 치수를 그대로 그리는 것**이 요점이라 도착해도 아래가 안 밀린다.
```
칠:       bg-surface-2 (배너의 빈 자리가 이미 그 색이고, 모든 테마에서 track 과 같은 값이다)
더보기 배너: NoticeBannerArt 와 같은 상자 — w-full overflow-hidden + aspectRatio
더보기 글:   NoticeLines 와 같은 Card 안에 같은 줄 수 · 같은 줄 높이(제목 20 · 날짜 17) · 같은 구분선
목록 배너:   NoticeBannerCard 와 같은 카드 셋 — 그림 비율 · 제목 19 · 구분선 · 기간 17. 제목은 한 줄로 잡는다
목록 글:     NoticeLines 줄 여섯. 개수는 한 화면을 덮을 만큼이고 스크롤 아래까지 채우지 않는다
모션:      opacity 펄스. useReducedMotion 이면 정지한 채로 자리만 잡는다
```
- **캐시가 있으면 쓰지 않는다.** 셸 승계 카드와 같은 규칙이고, 소식 갈래는 기기 사본에 글이 있으면 그것을 먼저 그린다.
- **당겨서 새로고침에는 안 세운다.** 이미 그린 것이 있고 당김 인디케이터가 대기를 말한다([[ADR-286]]).
- **카드 하나가 들어올 자리에는 쓰지 않는다.** 그쪽은 셸 승계 카드다.

**쓰지 않는 것**: 점선 박스(빈 상태 전용) · 비-브랜드 CSS 링 스피너 · `MapleWaveProgress`(폐기) · 진행률 바 `h-2` 변형(폐기). 새로고침 아이콘(`RefreshCw`)의 회전은 스피너가 아니라 **기능 신호**라 교체 대상이 아니다.
...
- **못 읽히는 것이 캐릭터일 때만 이 알약이다**([[ADR-254]] 결정 3). 기간이 못 읽히는 것(롤링 창 밖,
  원장이 400 으로 굳힌 주, 추적하지 않는 캐릭터의 앞으로의 주)은 회색 글자 + 행 `opacity-40` 이다.
  같은 말이지만 앞의 것은 사용자가 할 일이 있고(캐릭터 관리에서 해제하거나 갈아끼운다) 뒤의 것은
  없다. 월간 탭 주차 줄이 둘을 함께 그리는 자리다(`boss-profit/AccordionBody`).
...
- **손잡이는 미끄러진다**([[ADR-260]]). `transform` 트랜지션 200ms 이고 곡선은 세그먼트의 미끄러지는
  상자와 같다. 처음 서는 자리와 움직임 줄이기에서는 안 미끄러지고, 트랙 색은 누르는 즉시 바뀐다.
...
- **적용 화면**: `today` · 컨텐츠 · 보스 · 보스 수익 · 가계부. 하위 페이지·설정에는 없다.
- **배선은 `ScreenScroll` 이 갖는다**([[ADR-226]] 결정 1). 화면은 `onRefresh` 로 자기 재조회
  함수만 준다. 셸이 `usePullRefresh` 를 부르고 테마 색을 붙여 `RefreshControl` 을 조립한다.
  안 주면 당김이 없는 화면이다. **내비게이터에는 못 둔다** — `RefreshControl` 은 컴포넌트가
  아니라 `ScrollView` 의 프롭이라 스크롤 컨테이너가 있는 자리에만 산다.
- **끄는 조건은 빈 상태 하나다.** 새로고침이 의미 없는 기간에서 끄던 게이트
  (`isPeriodRefreshable`)는 [[ADR-222]] 가 지웠다. 지금은 다섯 화면이 무조건 넘긴다.
...
- **안전영역 페이드: RN 은 ‘덮지 않고 깎는다’** ([[ADR-134]], 2026-08-14). 콘텐츠가 크롬과 겹치는 자리(상단 상태바 밑 · 하단 홈 인디케이터)에서 **알파가 0으로 간다.** 배경색을 덮는 스크림이 아니다. 그러면 벽지 테마에서 정지 상태에도 띠가 보여 [[ADR-133]] 이 걷어낸 상태로 돌아간다.
  ...
  ```
- **헤더에 담는 것은 셋뿐이다** ([[ADR-233]], 2026-09-08 사용자 지시). 제목 · 데이터 기준 시각 ·
  다른 페이지로 가는 것(뒤로 · 이동 링크). 가르는 기준은 **여기를 떠나는가** 다. 떠나면 헤더,
...
- **바닥 줄의 자리는 `animatedSheetHeight` 에서 잰다**([[ADR-241]] 정정 3). 시트가 미끄러지는
  동안 그 값은 안 바뀌므로 줄이 시트에 붙어 함께 움직인다. 컨테이너 기준으로 재면 키보드가
  뜰 때 그 줄만 먼저 튀고 시트를 끌어내릴 때 그 줄만 제자리에 남는다. 내용 상자(흐름)에
  얹어도 안 된다. 그 상자는 시트보다 느리게 자라서 열 때 줄이 위로 떴다가 내려앉는다.
- **시트가 옮겨 앉는 시간은 250ms** 다(`animationConfigs`). 라이브러리 iOS 기본값은 과감쇠
  스프링이라 530ms 가 걸리는데, 키보드는 265ms 만에 다 올라와 시트 아랫변이 낮은 채로 덮인다.
...
- **키보드 배선은 다섯이 한 벌이다**(`BottomSheet` 조직체가 쥔다). 초점 채우기 ·
  ...
  **상한에서 키보드 높이 빼기**([[ADR-240]] — 라이브러리가 시트를 통째로 밀어 올리므로, 안 빼면
  윗변이 82% 선보다 키보드 높이만큼 더 올라간다).
  하나만 빠져도 증상이 각각 다르다: **안 올라간다 / 두 번 올라간다 / 안 내려온다 / 시트 끝과
  키보드 사이가 벌어진다 / 시트가 82% 선 위로 넘어간다.**
...
- **칸에 묶인 글자는 `fixed` 를 준다**. today 위젯 전부 · 하단바 라벨 · 캐릭터 레일 초상의 폴백
  ...
  (글자가 커지면 상자도 커진다).
- **누가 그 값을 정하는지는 셋으로 갈린다**([[ADR-252]] 결정 1). ① 그 파일이 직접 글자를 그리면
  호출부가 `<Text fixed>` 를 단다. ② **어느 호출부에서도** 상자가 고정인 부품은 컴포넌트가
  스스로 진다(`Badge`). ③ **상자가 자리마다 갈리는 부품**은 `fixed` 를 **프롭으로 열어 호출부가
  고른다**. ③의 첫 소비자가 `molecules/Segment` 다 - 폼 안·설정에서는 글자를 따라 커지고
  `남은 스케줄` 위젯 안에서는 타일 높이가 76 으로 못박혀 있다. 기본값은 거짓이라 **무시하는 쪽을
  호출부가 적는다.** `TabSegment` 는 헤더에만 서므로 안 열었다.
- **정책 테스트가 그 셋을 다 본다**(`src/__tests__/font-scaling-policy.test.ts`). 고정칸이 글자
  그리는 부품을 가져오면, 그 부품이 **고를 수 있는가**(`fixed?: boolean`)와 이 자리에서 실제로
  **골랐는가**(`<T …>` 마다 `fixed` 가 붙었는가)를 함께 묻는다([[ADR-252]] 결정 2).
...
- **떠 있는 원은 탭 화면마다 한 자리다.** 오른쪽 아래, 지름 56, 하단바 위 12. 값은
  ...
  **콘텐츠는 그 몫을 바닥에 갚는다** - `FAB_SPACE_PX`(84)를 `paddingBottom` 으로 주고 `pb-4` 를
  같이 주지 않는다. 그 상수가 숨돌림 16 을 이미 품고 있어 함께 주면 여백이 두 번 붙는다.
- **떠 있는 원은 그림자를 든다**([[ADR-263]]). 값은 `lib/fab-metrics.ts` 의 `FAB_SHADOW`
  (불투명도 0.65 · 반경 12 · y 4) 하나이고 원 둘이 같은 것을 쓴다. 같은 층에 떠 있는 물건이라
  같은 높이로 보여야 한다. 불투명도 0.65 는 테마 `shadowColor` 의 알파 0.35 와 곱해져 실효 0.23 이다.
  **`boxShadow` 로 쓴다.** `shadowOpacity`·`shadowRadius`·`shadowOffset` 은 iOS 전용이라
  안드로이드에 안 닿고, `elevation` 은 같이 주지 않는다(두 번 그려진다). 번역기는
  `lib/shadow.ts` 의 `boxShadowOf` 하나이고 하단바도 그것을 쓴다(블러는 `shadowRadius` 의 두 배,
  알파는 테마 `shadowColor` 와 미리 곱한다).
  **그림자는 원을 감싸는 바깥 뷰가 든다.** `DropPriceFab` 의 원이 `overflow-hidden` 이라 같은 뷰에
  달면 플랫폼에 따라 잘린다. 둘이 같은 물건으로 보여야 해서 `SpeedDial` 도 같은 자리에 단다.
  **다크에서는 흰 헤어라인**(`rgba(255,255,255,0.22)`)이 경계를 낸다. 어두운 바탕에서는 그림자가
...
- ~~스위치 손잡이는 순간이동한다. 미끄러지게 하는 것은 #378 이다~~ → **200ms 에 미끄러진다**([[ADR-260]], 2026-09-12, 사용자 지시). #378 은 누름을 막지 않는 일이라 손잡이를 다루지 않았고, 따로 정했다. 트랙 색은 그대로 누르는 즉시 바뀐다.
- ~~누름이 아무 일도 안 하는 동안은 `Switch` 에 `disabled` 를 준다. 알림 설정이 FCM 왕복 중에 쓴다~~ → **프롭째 걷었다**([[ADR-259]] 결정 7, 2026-09-12, 이슈 #378). 쓰던 곳이 알림 설정의 왕복 중 잠금 하나였고, 그 화면이 왕복 중에도 누름을 받게 되어 쓰는 곳이 없다.
...
- ~~스켈레톤은 도입하지 않는다(셸 승계 카드가 같은 효용을 낸다)~~ → 더보기의 소식 갈래 다섯에서만 `atoms/Skeleton` 을 쓴다([[ADR-287]] 결정 1, 2026-09-16). 옛 판단의 전제가 **결과가 들어올 자리가 카드 하나**라는 것이었는데 그 갈래들은 배너 둘·글 셋으로 껍데기가 갈려 승계할 카드가 하나가 아니다. 카드 하나가 들어올 자리는 그대로 `LoadingState` 다.
