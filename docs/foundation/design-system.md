...
**시트는 키보드를 안 본다**([[ADR-302]] 결정 1 · 배선 삭제 완료 2026-09-21). 상한에서 키보드
높이를 빼지 않는다([[ADR-240]] 폐기). 시트에 치는 칸이 하나도 없으므로 **키보드 리스너도,
그것을 시트에 알리던 길도 없다**. 자리를 옮기는 시간은 380ms 한 갈래다(키보드에 맞출 것이
없어졌다).

**상한은 `frame.height − 상단 인셋`이다**(사용자 지정 2026-09-24. 옛 값은 `frame.height * 0.82`).
비율은 화면이 높을 때만 숨돌림이고, 화면이 낮아지면 **남는 18%가 내용에서 먼저 깎인다** —
360×640dp 에서 0.82 는 525dp 이고 머리와 저장 줄을 빼면 스크롤에 400dp 안팎이 남는다. 같은
화면에서 안전영역만 피하면 609dp 라 84dp 가 돌아온다. 피해야 하는 것은 **노치와 상태바**이지
화면의 몇 퍼센트가 아니다.

**머리와 바닥 줄이 흐름 밖에 고정되고 가운데만 구른다**(사용자 지정 2026-09-24). 내용이 길면
스크롤이 생기는 것은 어쩔 수 없지만, 그때도 제목과 저장 줄은 제자리에 남는다. 흐름 밖에 선
둘의 몫은 스크롤 내용의 `paddingTop`·`paddingBottom` 이 비운다 - 라이브러리가 시트 키를 스크롤
**내용** 높이 하나로 재므로 그 여백이 곧 둘이 설 자리다.

**바닥 줄은 시트 상자에 `bottom: 0` 으로 붙는다.** 그 몫을 따로 애니메이션하지 않는다. 상자가
움직이면 줄이 같은 곡선으로 함께 미끄러지고, 따로 옮기면 줄이 상자보다 **앞서 도착해** 단계를
오갈 때 빈 띠가 보인다([[ADR-241]] 정정 6 이 그렇게 났던 기록이다). 층에는
`pointerEvents="box-none"` 이 필수다 - 층이 자기 상자로 터치를 먹으면 바로 위 내용이 안 눌린다.

**셀 것이 없는 단계에는 바닥 줄이 없다**(사용자 지정 2026-09-25). 가계부 1차(갈래 고르기)와
지출 카탈로그의 항목 격자가 그렇다. 전에는 같은 높이의 빈 상자를 세워 단계를 오갈 때 바닥의
기하를 붙들어 뒀는데, **버튼이 없는데 87이 비어 있는 것**이 더 크게 읽혔다. 갈래 고르개의
`닫기` 버튼도 함께 걷었다 - 닫는 길은 아래로 쓸어내리기와 스크림 탭 둘이 이미 있고, 스크림은
`accessibilityLabel="닫기"` 누르개라 읽어 주는 길도 남는다.

**페이드는 가릴 것이 있을 때만 뜬다.** 늘 그려 두면 안 구르는 시트에서도 위아래 끝의 내용이
옅어져, 가릴 것이 없는데 가린 것처럼 보인다. 위는 구른 적이 있을 때, 아래는 내용이 구르는 자리
보다 클 때만 선다. 판정에 쓰는 두 키(구르는 자리 · 내용)는 **ref** 다 - 같은 배치에서 잇따라
도착해(`onLayout` 다음에 `onContentSizeChange`) state 로 두면 뒤에 오는 쪽이 앞의 값을 못 본다.

**구르는 자리의 위아래 끝은 페이드다**(16px). 내용이 고정된 줄 밑으로 선명한 채로 사라지면
잘린 것으로 읽히고, 옅어지며 들어가면 그 밑에 더 있다는 것이 읽힌다. 화면 셸(`ScreenScroll`)은
같은 일을 **마스크**로 하는데 거기는 뒤에 벽지가 있어 덮으면 띠가 남기 때문이다. 시트는 바탕이
불투명하므로 **같은 색으로 덮는다** - 결과가 같고 마스킹이 시트의 동적 높이 계산을 안 건드린다.

**고정된 줄과 구르는 자리 사이에 안 구르는 띠를 두지 않는다.** 바닥 슬롯이 주는 위 여백(12)
위에 구분선을 그으면 그 12가 죽은 띠로 남는다. 선은 줄의 **맨 윗변**에 서야 하고, 그 자리는
호출부가 `-mt-3` 로 슬롯 여백을 되돌려 만든다(드롭 시트가 그렇게 한다).

옛 정책은 「바닥 줄은 스크롤 내용의 마지막 줄」이었다. 떼어 세우던 층(`SheetFooterLayer`)을
[[ADR-302]] 가 걷은 것은 그 조건이 「키보드가 떠 있을 때」 하나였기 때문인데, **지금 이유는
키보드가 아니라 내용이 길어서**다.
