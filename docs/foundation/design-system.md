...
- **하단바 — RN 은 «떠 있는 캡슐 + 두 층 + 뒤로가기»** ([[ADR-132]], 2026-08-13). 웹(`app-capacitor`)은 아래 «폐기된 정책» 의 고정 탭바를 그대로 쓴다 — 교체될 앱이라 옮기지 않는다.
  ```
  바      좌우 14 · 안전영역 위 12 · h 60 · radius 999 · 화면에 떠 있다(콘텐츠가 아래로 지나간다)
  안쪽여백 **3** (알약 = 60 − 3×2 = 54, 비율 0.90) · 가로는 칸에서 2 — 레퍼런스 실측 0.89 에 맞춘
          값이다. 넓히면 항목이 «바 안에 떠 있는» 것처럼 보인다
  ...
  폴백    iOS 26 미만·안드로이드: 불투명 캡슐 + 테두리. 그 자체로 완성된 모습이어야 하므로 아래
          색 관계는 유리와 무관하게 성립한다
  테두리   유리 위 `rgba(255,255,255,.5/.16)` 헤어라인 · 폴백은 `border`(라이트) / `border`→`text`(다크)
  알약    h 54(= 바 − 여백×2) · 칸 − 2 · 언제나 «바보다 밝은» 쪽 토큰
  입체감   장치 **셋이 함께** 진다 — ① 바 그림자(약하게: 0.22 · r14 · y5) ② **알약 자체 그림자**
          (0.2 · r5 · y2 — 이게 빠지면 바 전체가 납작한 판으로 보인다) ③ 밝은 테두리. 하나만 세게
          주면 «두껍다» 가 되고, 하나라도 빼면 «평평하다» 가 된다
  ←       원 지름 **48 고정** — 여백을 줄여도 그대로라 알약(54)보다 작다(레퍼런스 둘째 장과 같다)
  ...
  구동    **전부 네이티브 드라이버** — 전환에 레이아웃 값이 하나도 없다. ← 자리는 상자를 넓히는
          ...
          JS 를 막아 꼬리가 떨린다
  측정    **바 루트 하나만** 잰다 — 애니메이션이 바꾸는 값(트랙 폭)을 재면 그 값이 다시 애니메이션
          범위를 정하는 고리가 되고, 잘못된 배치로 **굳는다**(실제 관측)
  ...
  ←       48dp 원 · 하위 행에만 · 활성 <text> / 비활성 <text-muted>
  인셋    콘텐츠 paddingBottom = 72 + 안전영역   (스크롤포트는 줄이지 않는다)
  ```
...
- **RN 하단바의 활성 아이콘은 «면»이다 — 다만 가려서 채운다**([[ADR-132]] 정정 25). fill 과 stroke 가 같은 색이라, **안쪽에 선이 있는 그림은 채우는 순간 그 선이 사라진다**(조준경 → 원판, 달력 → 체크 소실). 통째로 채우는 것은 안쪽에 의미가 없는 셋뿐이고(`LayoutDashboard`·`Wrench`·`ShoppingCart`), 톱니와 수익은 **커스텀이라 채울 자리를 고른다** — `GearIcon` 은 lucide `settings` 와 같은 좌표를 한 패스로 다시 그려 `fillRule="evenodd"` 로 가운데를 비우고(설정 화면들은 계속 lucide `Settings` 를 쓴다), `ProfitIcon` 은 동전 두 개만 채우고 단을 그리는 호는 선으로 남긴다. **채운 그림에서는 구멍을 키운다**(톱니 r 3 → 4.5) — 둘레의 획이 구멍 안쪽을 먹어 원래 크기로는 «덩어리 속 점» 이 된다.
