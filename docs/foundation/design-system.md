...
- **바닥 줄의 자리를 계산하지 말 것**([[ADR-241]] 정정 2). 스크롤 뒤에 오는 흐름의 마지막
  자식으로 두고 자기 높이만큼 음수 마진을 준다. 시트가 크고 줄고 미끄러지는 것은 전부
  라이브러리가 한 벌의 애니메이션으로 돌리는 값이라, 흐름에 얹혀 있어야 한 몸으로 움직인다.
  좌표로 얹으면 키보드가 뜰 때 그 줄만 먼저 튀고 시트를 끌어내릴 때 그 줄만 제자리에 남는다.
