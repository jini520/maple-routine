...
**스켈레톤 (`atoms/Skeleton`)**: [[ADR-287]] 결정 1·2, 2026-09-16. **셸 승계 카드가 못 덮는 자리 하나에만 쓴다** — 결과가 들어올 껍데기가 **자리마다 다를 때**다. 지금 그 자리는 더보기 화면의 소식 갈래 다섯뿐이다(배너 둘은 높이가 `폭 ÷ noticeBannerRatio(kind)` 이고 글 셋은 줄 수가 셋·셋·둘이라, 승계할 카드가 하나가 아니다). **결과의 실제 치수를 그대로 그리는 것**이 요점이라 도착해도 아래가 안 밀린다.
```
칠:       bg-surface-2 (배너의 빈 자리가 이미 그 색이고, 모든 테마에서 track 과 같은 값이다)
배너 갈래: NoticeBannerArt 와 같은 상자 — w-full overflow-hidden + aspectRatio
글 갈래:   NoticeLines 와 같은 Card 안에 같은 줄 수 · 같은 줄 높이(제목 20 · 날짜 17) · 같은 구분선
