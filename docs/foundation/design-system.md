...
- **하단바 — RN 은 «떠 있는 캡슐 + 두 층 + 뒤로가기»** ([[ADR-132]], 2026-08-13). 웹(`app-capacitor`)은 아래 «폐기된 정책» 의 고정 탭바를 그대로 쓴다 — 교체될 앱이라 옮기지 않는다.
  ```
  바      좌우 14 · 안전영역에 바로 붙는다(들어올림 0 — 그 12 는 바 높이로 옮겼다) · h 72 ·
          radius 999 · 화면에 떠 있다(콘텐츠가 아래로 지나간다)
  안쪽여백 **3** (알약 = 72 − 3×2 = 66, 비율 0.92) · 레퍼런스 실측 0.89 에 맞춘 값이다.
  ...
  알약    h 66(= 바 − 여백×2) · 폭은 칸 + 오버행 23 · **폴백도 «무채색 판» 이다**([[ADR-132]] 정정 28)
