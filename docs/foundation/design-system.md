...
- **값을 프롭으로 받는다.** 스토어를 이 안에서 읽으면 `components/` 가 `features/` 를 알게 되고
  그 방향은 계층 테스트가 막는다.
- **화면은 그 값을 적지 않고 읽는다**([[ADR-231]] 정정 1). 그리는 데이터의 `syncedAt` 이 그 값이다.
  화면이 자기 시계로 지금 을 적으면 조회가 TTL 에 막혀 한 번도 안 나간 진입에도 시각이 갱신되고,
  같은 한 번의 조회로 그린 데이터인데 페이지마다 값이 갈린다(실사용에서 1초 차이로 드러났다).
  여럿을 읽는 화면은 `lib/data-freshness` 의 `latestSyncedAt` 으로 가장 최근 것을 고른다.
