...
- **그 줄을 안 그리는 화면은 `PageHeader` 가 같은 16 을 바닥에 비운다**(`ownsFreshnessLine` 이
  거짓일 때). 안 비우면 탭을 오갈 때 제목이 16 뛴다.
- **제목 줄과 한 덩어리로 묶는다.** 둘을 뷰 하나에 넣어 `PageHeader` 의 `gap-4` 흐름에 자식
  하나로 들여보낸다. 따로 넣으면 16 이 벌어져 제목에 딸린 글씨로 안 읽힌다.
- **그 뷰에 `gap-*` 을 주지 않는다**(사용자 지정 2026-09-08). `PageHeaderTitleRow` 의 `min-h-8`
  이 제목 글자 아래에 이미 여백을 남긴다. 거기에 간격을 더하면 둘이 떨어져 보인다.
- **절대 시각이다.** `12분 전` 같은 상대 표기는 화면을 보고 있는 동안 계속 틀려지는데 그 줄을
  다시 그리지 않는다.
- 쓰는 곳 다섯: today · 컨텐츠 스케줄러 · 보스 스케줄러 · 보스 수익 · 가계부. **당김을 가진
  화면 전부**다.
