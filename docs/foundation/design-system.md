...
**하위 페이지는 스크롤 끝에 하단 안전영역만큼 여백을 남긴다**([[ADR-120]] 결정 16) — 탭바가 없어 끝까지 내리면 마지막 줄이 홈 인디케이터에 닿는다. 콘텐츠가 그 영역을 **지나가는 것은 막지 않고** 끝에만 여백을 둔다. 그래서 스크롤포트를 줄이지 않고 **콘텐츠의 `padding-bottom`** 으로 넣는다(`ScreenScroll` 이 `hasTabBar === false` 일 때 안쪽 래퍼에). **스크롤 컨테이너 자신에 걸지 말 것** — 스크롤 요소의 `padding-bottom` 은 브라우저에 따라 넘치는 콘텐츠의 스크롤 영역에 안 잡힌다.
