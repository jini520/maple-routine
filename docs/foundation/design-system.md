...
**시트는 키보드를 안 본다**([[ADR-302]] 결정 1, **걷는 것은 마지막 폼이 옮겨간 뒤**). 상한은 `frame.height * 0.82` 하나이고
키보드 높이를 빼지 않는다([[ADR-240]] 폐기). 걷는 배선은 `keyboardHeight` 상태 · `SheetFooterLayer`
의 저장 줄 떼어 붙이기 · `scrollToEndOnKeyboard` · `android_keyboardInputMode` ·
`keyboardBlurBehavior` · `molecules/SheetTextInput` · `hooks/useSheetKeyboardTarget` 이다.
라이브러리는 `animatedKeyboardState` 의 `target` 이 비면 키보드 이벤트를 받고도 무시하므로,
그 훅을 안 쓰는 것만으로 시트가 제자리에 선다. **걷는 것은 그 시트의 치는 칸이 전부 입력 카드로
옮겨간 뒤다**. 글자 칸이 하나라도 남으면 그 칸이 키보드를 부르고 시트가 가린다.
