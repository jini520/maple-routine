...
| 데이터 안전 | ❌ | `AD_ID` 권한을 선언했으므로 **광고 ID 수집 신고 필수**. 넥슨 API 키의 취급 분류도 함께. **알림을 넣는 릴리스부터 한 항목 더**. FCM 등록 토큰은 앱이 서버로 안 보내도(토픽 방식, [[ADR-146]] 결정 2) **Firebase 가 갖는 기기 식별자**라 신고 대상이다. **[[ADR-227]] 결정 4 의 2차 가공 데이터 수집부터 또 한 항목** - 식별자를 떼고 옵트인으로 보내도 **모으는 것 자체가 신고 대상**이다. [[ADR-299]] 가 그 수집을 넥슨 로그인 uid·API 키 해시에 묶고 끌 수 없게 바꾼다(설계). 그 릴리스부터는 **선택 항목이 아니라 필수 수집**으로 신고한다 |
...
### 규칙 4-1: 한 플랫폼만 발행할 수 있다 (`--platform`)

```bash
node scripts/publish-rn-ota.mjs --platform ios
```
...
그 기기가 켜자마자 죽는다. 둘을 한 번에만 낼 수 있으면 iOS 도 안드로이드 게시를 기다려야 했다.

**못박기가 비어 있는 플랫폼은 발행 직전에 트리 지문이 스토어 바이너리와 같은지 사람이 본다.**
스크립트가 그 플랫폼의 `latest-<platform>.json` 을 트리 계산값으로 덮기 때문이다. 다르면 그 스토어
사용자 전원이 닫을 수 없는 스토어 업데이트 창에 막힌다.
