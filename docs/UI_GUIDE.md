...
콘텐츠 행: flex items-center justify-between, padding 0 14px(좌우 동일 — 일러스트 위에 바로 얹히므로 별도 좌측 여백 없이 카드 가장자리에 붙임, 2026-07-13 확정)
  ...
  오른쪽: flex items-center gap-1.5 — 파티 인원 설정 진입 버튼(항상) → 파티 배지(설정된 경우) → 완료 배지(완료된 경우)를 이 순서로 나란히 배치. 파티 배지·완료 배지 둘 다 없으면 진입 버튼만 남는다(2026-07-13, [[ADR-019]] 반영 — 설계만·구현 전, 실제 화면 검증 전까지 잠정안)
```
...
**파티 인원 설정 진입 버튼 — 확정, 2026-07-13, [[ADR-019]]**: 카드 전체를 탭하는 대신, 오른쪽 배지 영역 맨 앞(파티 배지·완료 배지보다 왼쪽)에 별도 아이콘 버튼을 둔다(카드 전체 탭은 향후 다른 카드 인터랙션과 충돌할 수 있어 사용자가 명시적으로 기각). 아이콘은 파티 배지의 `Users`와 혼동되지 않도록 `lucide-react`의 `SlidersHorizontal`(size 16, strokeWidth 2)을 쓴다. 다른 아이콘과 마찬가지로 배경 원 없이 단독으로 표시하고("아이콘 컨테이너로 감싸지 않는다" 원칙 재사용), 카드가 항상 레테 고정 배색이므로 아이콘 색도 테마 토큰이 아니라 레테 `text-muted`(`#B89CBD`) → hover 시 `text`(`#E8DFEC`) 리터럴을 쓴다. 탭하면 파티 인원 입력 모달(`PartySizeModal`, `components/Modal/Modal` 재사용)이 열린다.
