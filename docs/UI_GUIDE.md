...
**정정(2026-07-22, 레테 `info-tint` 대비 수정)**: 레테의 `info-tint`(`#C9D6F2`, 밝은 파스텔 블루)가 같은 테마의 `text`(`#E8DFEC`, 거의 흰색)와 대비 1.13:1로 사실상 안 읽히는 문제 발견(Toast 컴포넌트 설계 중 확인). 원인은 위 118번째 줄에 적힌 대로 `info-tint`가 레테 기존 팔레트에 대응값이 없어 즉석으로 채운 5개 토큰 중 하나였기 때문 — 혼테일(다른 다크 테마)의 `info-tint`(`#3A3235`)처럼 나머지 팔레트와 같은 "어두운 배경" 방향을 따르지 않고 밝은 값이 들어가 있었다. `#262A3A`(레테의 `surface-2`~`border` 사이 톤, 차갑게 살짝 틀어 기존 블루 계열 인상은 유지)로 교체 — `text` 대비 약 11:1로 상향. `UpdatePromptModal`의 `InfoNote`도 동일 토큰을 쓰므로 함께 해소됨.
...
### 수동 트래킹 체크리스트 — 보스 스케줄러 (구현 완료, 2026-07-24, [[ADR-035]])

컨텐츠와 동일한 원칙을 보스 스케줄러에 적용한다 — 트래킹 모드가 `manual`일 때만 추가/삭제 진입점이 붙고(auto 모드는 기존과 완전히 동일), 표시 카드는 기존 `BossCard`를 **그대로 재사용**하며 완료 여부는 동기화 결과에서 조회한다(멤버십에 값을 복제하지 않음, [[ADR-035]] 결정 6·12). 보스는 카운트형 진행값이 없어 컨텐츠의 템플릿 기본값 파일이 필요 없다 — 한 번도 동기화된 적 없는 보스는 "미완료"가 기본값이고, cycle만 `weekly-bosses.json`에서 조회한다(`lib/manual-boss-merge.ts`).
- **보스 추가 버튼**: 화면 상단 "캐릭터 관리"/"파티 관리" 버튼 옆에 텍스트 버튼("보스 추가", 두 버튼과 동일한 `text-sm font-medium text-text-muted hover:text-text`). 컨텐츠처럼 탭별 하단 버튼이 아니라 화면당 하나다 — 탭하면 `ManualBossPickerModal`이 열려 **보스 드롭다운 → 난이도 뱃지** 2단 폼(PartyManagementModal과 동일 패턴)으로 고르고, 난이도 뱃지를 누르면 즉시 추가되고 모달이 닫힌다. 후보는 항상 전체 보스 목록(`weekly-bosses.json`의 weekly+eventWeekly+monthly)이고, 이미 추적 중인 (보스,난이도)의 뱃지는 `opacity-40`으로 비활성화된다(자유 텍스트 없음, [[ADR-035]] 결정 11). 추가된 보스는 자기 cycle에 맞는 탭(주간/월간)에 나타난다.
- **삭제 진입점**: 컨텐츠와 동일한 원형 X 버튼을 각 보스 카드 우측 상단 모서리에 얹는다(`aria-label`은 "{보스명} {난이도} 삭제" — 같은 보스를 여러 난이도로 추적할 수 있어 난이도까지 포함).
- **빈 상태**: 해당 탭에 추적 보스가 없으면 "추적할 보스가 없습니다 — \"보스 추가\"로 추가해주세요"를 점선 박스로 보여준다.
- **솔로/파티 서브 필터([[ADR-019]])는 트래킹 모드와 무관하게 그대로 동작한다** — 수동 모드의 보스 목록도 `partySizes` 맵 기반 필터를 동일하게 통과시킨다. n/12 카운트·시즌 배지·파티 관리 모달은 모두 store의 auto 목록을 그대로 읽어 변경되지 않는다.
