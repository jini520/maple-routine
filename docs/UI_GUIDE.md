...
콘텐츠 행: flex items-center justify-between, padding 0 14px(좌우 동일 — 일러스트 위에 바로 얹히므로 별도 좌측 여백 없이 카드 가장자리에 붙임, 2026-07-13 확정)
  왼쪽: 난이도 뱃지 + 보스명 + 파티 인원 설정 진입 버튼(순서: 뱃지 → 이름 → 버튼, 버튼은 이름 바로 오른쪽), 이름에는 text-shadow(0 1px 3px rgba(0,0,0,.9), 0 0 10px rgba(0,0,0,.6))로 일러스트 위에서도 대비 확보(**정정, 2026-07-13** — 진입 버튼은 원래 오른쪽 배지 영역 맨 앞이었으나 보스명 바로 옆으로 이동)
...
**파티 인원 설정 진입 버튼 — 확정, 2026-07-13, [[ADR-019]] (정정 반영)**: 카드 전체를 탭하는 대신 별도 아이콘 버튼을 둔다(카드 전체 탭은 향후 다른 카드 인터랙션과 충돌할 수 있어 사용자가 명시적으로 기각). **정정(2026-07-13)** — 위치는 오른쪽 배지 영역이 아니라 **왼쪽, 보스명 바로 오른쪽 옆**이다. 아이콘도 `SlidersHorizontal`에서 `lucide-react`의 `Cog`(톱니바퀴, size 16, strokeWidth 2)로 교체했다. 다른 아이콘과 마찬가지로 배경 원 없이 단독으로 표시하고("아이콘 컨테이너로 감싸지 않는다" 원칙 재사용), 카드가 항상 레테 고정 배색이므로 아이콘 색도 테마 토큰이 아니라 레테 `text-muted`(`#B89CBD`) → hover 시 `text`(`#E8DFEC`) 리터럴을 쓴다. 탭하면 파티 인원 입력 모달(`PartySizeModal`, `components/Modal/Modal` 재사용)이 열린다.
