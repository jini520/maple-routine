...
| 무엇 | 상태 | 판정 기준 |
|---|---|---|
| [[ADR-047]] **중첩 sticky** | 없다. 배지 레일·stuck 헤더 페이드·`stickyTop` 도 함께 | **육안 대조 1순위.** 보스 행을 스크롤하는 동안 캐릭터 합계가 사라지는 것이 견딜 만한가 — 거슬리면 `ScreenScroll` 을 `Animated.ScrollView` 로 바꾸는 것이 선행 작업이다(4-7단계 «두 길») |
...
| [[ADR-048]]·[[ADR-103]] **드롭 연출 재생 엔진** | 프레임 에셋은 [[ADR-129]] 로 들어왔고([[ADR-048]] origin 을 해석할 크기를 이제 읽을 수 있다) **엔진·팝인·`mix-blend-screen` 이 없다** | [[ADR-103]] 의 1.5배는 **눈으로 정해진 값**이라 구조(단계별 fps 표 + 한 배율)를 세우고 실기기에서 확정해야 한다 |
