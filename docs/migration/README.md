...
**판정을 기다렸다.** 사용자가 UI 정책을 다시 잡으며 판정을 내렸다: **sticky 는 화면 최상단
헤더에만 둔다**([[ADR-131]]).

그래서 «못 옮긴 것» 이 «안 옮기기로 한 것» 이 된다. RN 은 이미 그 정책을 만족하므로(페이지 헤더가
sticky 가 아니라 스크롤 뷰의 형제다) **코드 변경이 없다.** 대신 정책이 조용히 뒤집히지 않도록
...
**대가는 그대로 남는다.** [[ADR-047]] 후속 3 이 소계 footer 를 지운 근거가 *"헤더가 sticky 라
합계가 스크롤 내내 보인다"* 였으므로, 긴 카드에서 캐릭터 합계가 화면 밖으로 나간다. [[ADR-131]] 은
그 자리를 되메우지 않는다 — footer 복원은 [[ADR-023]]·[[ADR-017]] 을 다시 여는 제품 결정이고,
정책이 바뀌는 중이라 지금 굳힐 값이 아니다.
