...
`CharacterSelectDropdown`(엠블럼·목록).~~ → **아이콘·엠블럼은 다 붙었다**(2026-08-14). 남은 것은
`CharacterSelectDropdown` 의 **목록(열린 상태)** 하나뿐이고, 그것은 에셋이 아니라 **목록 UI 결정**을
...
| `ValuableDropBadge` | 045, 046, 071 | 045 결정 2 골드 pill + Sparkles + 아이콘 최대 3 + `+N`·결정 3 **전 테마 공통 고정 골드** ✅ / 046 결정 4 배치·라벨은 호출부 ✅ / 071 결정 4 는 호출 자리(히스토리 요약 줄)라 컴포넌트 무관 ✅ / **아이콘 그림이 붙었다**(2026-08-14) — `getItemIconUrl(itemName, slot)` 결과를 `source` 에 그대로 넣고 `object-contain`→`resizeMode`. 바탕색이 갈리는 것까지 웹과 같다(그림 `bg-surface` · 폴백 원 `bg-surface-2`)이고 매핑에 없는 이름은 **여전히 폴백**이다. 스택 규칙은 두 갈래가 같은 `stackStyle` 을 쓴다 / **모션은 이 배지에 없다**(step 사양 정정 — `@keyframes` 셋은 전부 카드·행 쪽이다) |
...
| 컴포넌트 | ADR 계약 | 확인 |
|---|---|---|
| `PageHeader` | 047, 077, 085, 088, 094, 098, 112, 123 | 094 결정 1 취약 구조를 한 셸에 ✅ / 088 결정 5-1 배경 조각 자리·**첫 자식** 순서(RN 은 형제 순서가 곧 그리는 순서라 `z-index:-1` 이 필요 없다) ✅ / 123 블러 없음 — RN 엔 `backdrop-filter` 자체가 없어 **구조로** 지켜진다 ✅ / `below` 슬롯 계약 그대로 ✅ / **085 결정 1 · 098 결정 2 · 112 는 형태가 바뀐다**: `fixed` + 실측 spacer 가 **흐름 안의 보통 뷰**가 되어 spacer 도 `useMeasuredHeight` 도 없다(회귀 가드 테스트로 고정) — *"spacer 가 헤더와 같은 프레임에 맞는다"* 는 맞출 것이 없어 만족되고, *"헤더 위치가 스크롤 오프셋의 함수가 아니다"* 는 [[ADR-131]] 이 **일부러 버렸다**(헤더가 스크롤 뷰의 첫 자식이라 함께 움직인다 — 085 의 결함은 `sticky` 의 매 프레임 재계산이 원인이라 돌아오지 않는다) / **047 결정 3 은 폐기됐다** — [[ADR-131]] 이 중첩 sticky 와 `stickyTop` 오프셋을 통째로 버려 계산할 대상이 없다([[ADR-100]] 결정 3 도 함께) / **상단 패딩은 일부러 갈랐다** — `pt-[calc(1rem+var(--sa-top))]` 의 상수 몫 `1rem` 을 옮기지 않는다([[ADR-139]], 사용자 판정 2026-08-16). 웹에서 그 16 이 하던 일(불투명 판의 안쪽 여백 · 고정 헤더와 상태바의 분리) 둘 다 RN 에 해당이 없다. **웹 쪽 값은 그대로 유효하다** ⚠️ |
