...
| 컴포넌트 | ADR 계약 | 확인 |
|---|---|---|
| `PageHeader` | 047, 077, 085, 088, 094, 098, 112, 123 | 094 결정 1 취약 구조를 한 셸에 ✅ / 088 결정 5-1 배경 조각 자리·**첫 자식** 순서(RN 은 형제 순서가 곧 그리는 순서라 `z-index:-1` 이 필요 없다) ✅ / 123 블러 없음 — RN 엔 `backdrop-filter` 자체가 없어 **구조로** 지켜진다 ✅ / `below` 슬롯 계약 그대로 ✅ / **085 결정 1 · 098 결정 2 · 112 는 형태가 바뀐다**: `fixed` + 실측 spacer 가 **흐름 안의 보통 뷰**가 되어 spacer 도 `useMeasuredHeight` 도 없다(회귀 가드 테스트로 고정) — *"spacer 가 헤더와 같은 프레임에 맞는다"* 는 맞출 것이 없어 만족되고, *"헤더 위치가 스크롤 오프셋의 함수가 아니다"* 는 [[ADR-131]] 이 **일부러 버렸다**(헤더가 스크롤 뷰의 첫 자식이라 함께 움직인다 — 085 의 결함은 `sticky` 의 매 프레임 재계산이 원인이라 돌아오지 않는다) / **047 결정 3 은 폐기됐다** — [[ADR-131]] 이 중첩 sticky 와 `stickyTop` 오프셋을 통째로 버려 계산할 대상이 없다([[ADR-100]] 결정 3 도 함께) |
