...
| `/boss` | `BossScreen` | **탭 2** | |
| `/boss/manage` | `BossManageScreen` | ~~탭 2 위 push~~ → **탭**(스케줄 그룹 하위 셋째) | [[ADR-145]] 결정 1 (2026-08-17) — 웹의 하위 페이지가 RN 에서 형제 탭이 됐다. 17행의 유일한 예외이고, `routes.ts` 의 `target` 이 `push` 가 아니라 `tab` 인 것이 그 기록이다 |
...
| `content-scheduler/DailyContentCards.tsx` | 018, 020, 094 | 018 카드 규격(`rounded-[14px]`·80px·bleed 레시피)과 **`.media-scope` 재선언**([[ADR-064]] 결정 5)을 `IllustratedCard` 가 진다 ✅ / 020 접두어 제거·지역 배경 매칭·`quest_state` 3단 배지·몬스터파크 112px 예외 ✅ / 094 결정 7 화면에서 분리된 상태 유지 ✅ — **bleed 네 줄은 `FadedIllustration` 로 접혔다**(CSS 배경 → RN 기하, README) |
...
**열둘째가 생겼다 — `FadedIllustration`.** 4단계 step 4 가 `app/content-scheduler/` 에 만들며
...
| 컴포넌트 | ADR 계약 | 확인 |
|---|---|---|
| `BossPortrait` | — | (계약 없음) **네 케이스가 다 선다**(4단계 step 5). 3단계는 플레이스홀더만 그렸고 막던 것이 둘이었다 — 에셋은 [[ADR-129]] 가, 크롭 기하는 step 4 의 `resolveImageCropLayout` 이 풀었다. **변환을 여기서 다시 하지 않는다**(카드 bleed 와 표만 다르고 값의 형태가 같아, 두 벌이 되면 한쪽만 고쳐지는 사고가 열린다) / **원형 클리핑은 우리가 명시한다** — 웹은 `background-image` 라 둥근 모서리가 배경을 저절로 잘랐지만 RN 의 `<Image>` 는 자식이다(`overflow-hidden`) / 접근성 역할은 웹과 **같은 `img`** |
| `FadedIllustration` (+ `lib/image-crop.ts`) | 018, 020, 021, 064, 094, 129 | **웹에 짝이 없다** — CSS `background-size`/`position`/`filter`/`mask-image` 네 줄이 RN 에서 기하 계산이 되어, 일곱 호출부에 복붙하면 한 곳만 어긋나도 그림이 조용히 다르게 잘린다([[ADR-094]] 결정 1 의 두 조건을 넘긴다) / 018·020·021 의 bleed 레시피와 `IllustratedCard` 껍데기(= 웹 `<Card className="media-scope …">`) ✅ / 064 결정 5 스코프 재선언은 `buildMediaScopeVariables` 한 곳에서 온다 ✅ / 129 로 들어온 에셋의 **고유 크기**가 기하의 유일한 실측 입력이고, 못 읽으면 `cover` 로 떨어진다(`Number.isFinite` — step 5 가 `undefined <= 0` 이 false 라 `aspectRatio: NaN` 이 나가던 것을 고쳤다) / 페이드 끝점은 카드(38%/76%)와 모달 히어로(42%/82%)가 달라 `variant` 로 가른다(core 의 두 마스크가 그렇게 갈라져 있다) |
