...
앱을 완전히 종료했다 다시 열면 Zustand 스토어는 비어 있는 상태로 시작한다. `AppShell`(`src/App.tsx`)이 마운트되며 **몇 가지만** 즉시 복원한다.

인증과 진입 단계가 **각각** 저장소를 읽는다. 둘 다 `apiKey` 를 보지만 묻는 것이 달라서(로그인했는가 · 어느 화면이 서는가) 한쪽이 다른 쪽에서 파생되지 않는다([[ADR-215]] 결정 2).

```mermaid
flowchart LR
    Start([앱 실행]) --> Shell[AppShell 마운트]
    Shell --> A["useAuthStore().restoreFromStorage()"]
    Shell --> E["useAppEntryStore().resolveFromStorage()"]
    Shell --> B["useThemeStore().restoreFromStorage()"]
    Shell --> C["useTrackingModeStore().restoreFromStorage()"]
    A --> P1[("apiKey")]
    E --> P4[("apiKey / trackedCharacters")]
    B --> P2[("theme")]
    C --> P3[("trackingMode")]
...
- **`character_profiles`가 기록 그룹에 있는 이유**([[ADR-219]] 결정 1). 캐릭터 이름·초상의 스냅샷이다. 값 자체는 재조회로 복구되지만, `general`에 뒀다면 "기록은 남기고 이름은 지우는" 조합이 가능해진다. 기록에서 행을 만드는 자리가 이름을 모르는 `ocid`를 건너뛰므로(`ocid`는 사용자에게 아무 뜻도 없어 대신 안 적는다), 그 상태에서는 **지운 적 없는 과거 수익이 화면에서 통째로 사라진다.** 재조회로도 못 되살리는 경우가 있다. 추적에서 뺐거나 삭제된 캐릭터는 `character/basic`이 다시 안 불린다.
- **`character_world_leaps`가 기록 그룹에 있는 이유**([[ADR-274]] 결정 4). 월드 리프로 갈린 옛 ocid 와 새 ocid 의 연결이고, 리프한 기간에 같은 처치가 두 ocid 로 한 번씩 선 기록을 짝지어 옛 기록을 지우는 데만 쓴다. 옛 ocid 는 `character/list` 에서 빠져 이 연결을 재조회로 되살릴 수 없다. `general`에 뒀다면 "기록은 남기고 연결은 지우는" 조합이 생기고, 그때 중복은 두 번 센 채로 굳는다.
