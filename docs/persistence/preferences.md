...
| `notices` | `Notice[]` (JSON) | `storage/notices.ts` | 삭제 | 받은 공지 최근 50건([[ADR-228]] 결정 4). 푸시로 온 것과 서버에서 받은 것이 섞이고 같은 `id` 는 나중 것이 이긴다. **보존 안 하는 이유**: 서버가 다시 준다 |
| `dismissedNotices` | `string[]` (JSON) | `storage/notice-banner.ts` | **보존** | today 배너에서 닫은 공지 id 최근 50건([[ADR-230]] 결정 4). 닫은 것은 배너이지 공지가 아니라 목록·상세에는 그대로 보인다. **보존하는 이유**: `notices` 는 지워도 서버가 다시 주지만 **닫았다는 사실은 기기에만 있어** 아무도 복원해 주지 않는다. 지워지면 이미 닫은 공지가 되살아난다 |
