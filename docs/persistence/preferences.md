...
| `representativeCharacter` | `string` (ocid, 평문) | `storage/character-selection.ts` | 삭제 | **대표 캐릭터**([[ADR-143]] 결정 4, RN 앱만). 쓰는 자리는 캐릭터 설정 · 캐릭터 관리 저장(둘 다 목록 저장 뒤) · 월드 이전 확인(`replaceTrackedCharacter`)이다. 읽는 자리는 today(대표 카드 · 머리 버튼) · 캐릭터 관리 초안(`useSelectionDraft`) · 동기화 회차(대표 하나의 5분 가드 건너뛰기, [[ADR-218]] 결정 1)다. **앱 메모리에서는 대표 표식 스토어 하나가 든다**([[ADR-275]], 설계 · 구현 전). 화면이 이 키를 바로 쓰면 기기 값만 바뀌고 today 가 옛 대표를 그린다(이슈 #395). 미지정이면 키가 **없고**(첫 번째가 임시 대표. 그 파생값은 저장하지 않는다), 저장 시점에 대표가 목록에 없으면 지운다. 그 판정은 `setTrackedCharacterOcids` 안에 있어 목록을 어느 경로로 저장하든 매달린 대표가 남지 않는다. 목록과 대표를 한 호출로 확정하는 `setCharacterSelection(ocids, representative)` 도 있지만 제품 코드에서는 안 불린다(화면 둘은 목록을 저장한 뒤 대표를 따로 쓴다) |
