...
| `noticeSubscribed` | `'on' \| 'off'` | `storage/notice-settings.ts` | **보존** | 공지 토픽 구독 여부([[ADR-228]] 결정 2). 값이 없으면 꺼짐. 켠 사람만 받는다. 지워지면 구독은 FCM 쪽에 남아 있는데 앱은 껐다고 믿어, **스위치가 꺼져 있는데 알림이 오는** 상태가 된다 |
| `notificationPermissionAsked` | `'yes'` | `storage/notice-settings.ts` | **보존** | 알림 권한을 물어본 적 있는가([[ADR-228]] 결정 3). OS 에 물어서는 알 수 없다 - 안드로이드의 `denied` 가 거부와 안 물음을 안 가른다. 지워지면 iOS 에서 다시 묻게 되는데 그 팝업은 이미 답한 사용자에게 **아예 안 뜬다**. 되돌리는 함수를 두지 않는다 |
| `notices` | `Notice[]` (JSON) | `storage/notices.ts` | 삭제 | **마지막 서버 응답의 사본**([[ADR-277]]). 조회가 성공하면 통째로 바뀌고 실패하면 그대로 남는다. 푸시 내용은 여기 안 들어온다. **분류마다 사본 하나**라 한 분류의 조회는 그 분류의 항목만 갈아 끼운다(최대 100건, [[ADR-277]] 결정 6). 상세 본문(`blocks`)은 안 들어온다(결정 7). ~~받은 공지 최근 50건. 푸시로 온 것과 서버에서 받은 것이 섞이고 같은 `id` 는 나중 것이 이긴다([[ADR-228]] 결정 4)~~. **보존 안 하는 이유**: 서버가 다시 준다 |
| `dismissedNotices` | `string[]` (JSON) | `storage/notice-banner.ts` | **보존** | today 배너에서 닫은 공지 id 최근 50건([[ADR-230]] 결정 4). 앱 공지 사본을 바꾸는 조회(`kind=app`)가 성공하면 서버 목록에 없는 id 를 빼고, 상세 조회가 404 를 받으면 그 id 를 뺀다([[ADR-230]] 정정 9 · [[ADR-277]] 결정 8). 닫은 것은 배너이지 공지가 아니라 목록·상세에는 그대로 보인다. **보존하는 이유**: `notices` 는 지워도 서버가 다시 주지만 **닫았다는 사실은 기기에만 있어** 아무도 복원해 주지 않는다. 지워지면 이미 닫은 공지가 되살아난다 |
| `dataFetchedAt` | ISO 8601 | `storage/data-freshness.ts` | 삭제 | **실시간 데이터를 마지막으로 받은 시각 하나**([[ADR-231]] 정정 2). 값 하나를 화면 다섯이 함께 읽는다. 움직이는 것은 스케줄러 데이터 · 오늘 날짜의 강화 데이터 · `character/basic` 셋뿐이고, 기기 DB 읽기와 과거 기간 조회는 Open API 를 부르더라도 안 움직인다(확정된 기록이라 실시간이 아니다). **보존 안 하는 이유**: 이 값은 **캐시가 언제 것인가** 라서 캐시와 함께 지워지는 것이 맞다. 캐시를 비웠는데 시각만 남으면 새로 받은 데이터에 옛 시각이 붙는다 |
| `schedulerCache:{ocid}` | `{ state: SchedulerCharacterState, syncedAt: string, version: 3 }` (JSON) | `storage/scheduler-cache.ts` | 삭제 | 캐릭터별 마지막 동기화 스냅샷(일간/주간/보스 콘텐츠). 보스 항목은 `bossKey` · `apiName` · 난이도 key 를, 컨텐츠 항목은 `contentKey` · `apiName` 을 든다. **`version` 이 3 이 아닌 옛 값은 없는 것으로 읽는다**([[ADR-280]] 결정 12 · 13). 옮기지 않고 다음 동기화가 덮어쓴다 |
...
| `characterBasicCache:index:{accountId}` | `string[]` (ocid 목록, JSON) | `storage/character-basic-cache.ts` | 삭제 | "이 **계정**의 **마지막으로 받은 `character/list` 에 있고** 캐시 항목도 있는 캐릭터" 역인덱스([[ADR-017]] 결정 6 + [[ADR-086]] 결정 9. 뜻은 2026-09-13 정정으로 바뀌었다, 이슈 #408, **구현 완료** 2026-09-14). 읽는 곳은 캐릭터 관리 피커의 stub 단계 하나다(`getCharacterPickerRoster`). **붙이는 곳**은 `setCachedCharacterBasic` 이다. **목록에 맞추는 곳**은 `getCharacterPickerRoster` 가 `character/list` 를 받은 직후다(`reconcileCachedCharacterBasicOcids`). 지금 연 계정의 인덱스에서 목록에 없는 ocid 를 빼고, 목록에 있고 캐시 항목도 있는 ocid 는 없으면 다시 붙인다. 붙이기만 하면 목록에서 빠진 캐릭터가 인덱스에 영영 남아, 캐릭터 관리를 열 때마다 후보 목록에 떴다가 사라진다. 빠지는 경우로 확인된 것은 월드 리프로 남은 옛 ocid 하나다. 다시 붙이는 이유는 5분 TTL 이다. 목록에 돌아온 캐릭터의 캐시가 TTL 안이면 `fetchCharacterBasicCached` 가 캐시를 다시 쓰지 않아 `setCachedCharacterBasic` 이 안 불린다. 돌아오는 경우는 확인된 사례가 없는 가정이다. **맞추는 것은 인덱스뿐이다.** 캐시 항목(`characterBasicCache:{ocid}`)은 today · 보스 수익 · 가계부 · 리프 판정 · `schedule-window` 기록이 읽어서 남긴다. 목록 조회가 실패하거나 계정을 응답에서 못 찾으면(`resolveRegisteredCharacters` 가 던지면) 안 건드린다. 캐릭터가 0명인 계정은 `normalizeCharacterList` 가 걸러 계정을 못 찾은 경우가 되고, 메이플 ID 가 여럿인 상황이라 범위 밖이다. 붙이기와 맞추기는 둘 다 read-modify-write 라 같은 프로미스 체인(`withIndexLock`) 안에서 차례로 돈다. 잠금 밖이면 한쪽이 붙인 ocid 가 다른 쪽의 쓰기에 덮여 유실된다. 계정별로 나뉘기 전(`characterBasicCache:index`)에는 피커 stub 단계가 **이전 계정 캐릭터까지 그렸다**. 마이그레이션(`runLegacyIndexMigration`)은 전역 인덱스를 레거시 `selectedAccountId` 것으로 이관하고(예열이 채운 계정은 그것 하나뿐이라 정확) 전역 키를 지운다 |
| `scheduleProbe:{ocid}` | `{ unavailable?: true, dates: Record<'YYYY-MM-DD', ProbeRecord>, version: 2 }` (JSON). 관측의 `bosses` 는 `보스 key\|난이도 key` 이고 표에 없는 보스는 안 적는다. `version` 이 2 가 아닌 옛 원장은 빈 원장으로 읽는다([[ADR-280]] 결정 12) | `storage/schedule-probe-ledger.ts` | 삭제 | **(ocid, 날짜) 조회 원장**([[ADR-086]] 결정 4 = [[ADR-067]] 결정 5). "이 캐릭터를 이 날짜로 이미 조회했고 결과가 이랬다"를 기록해 ① 후보 자격 스윕과 ② 선채움([[ADR-034]])이 같은 날짜를 다시 부르지 않게 한다. 성공·`OPENAPI00003`·`OPENAPI00004` 만 기록하고 `OPENAPI00009`·네트워크 실패는 **기록하지 않는다**(나중에 풀린다). 읽을 때 14일 윈도우 밖 날짜를 prune. 재조회로 복구되는 파생 데이터라 `KEEP_KEYS` 에 넣지 않는다. **소비자가 셋이 됐다** — ③ 처치 날짜 캐기([[ADR-172]])가 `observed` 기록의 `bosses`(그날 `ownComplete` 였던 «보스|난이도» 목록)를 읽고 쓴다. 그 필드가 **없는**(`undefined`) 옛 기록은 «보스 정보를 안 남긴 관측» 이라 미조회로 취급해 다시 부른다 — 빈 배열(«그날 완료 0건»)과 섞으면 관측을 잃는다. **`unavailable` 은 앱 전체에서 «조회 불가» 의 유일한 출처다**([[ADR-253]] 결정 8) — 400 `OPENAPI00003` 뿐 아니라 **동기화가 한 번도 답하지 않은 추적 ocid**(월드 이전으로 `character/list` 에서 빠진 경우)도 여기 쓰고, 캐릭터 관리 배지·스케줄러 첫 페인트·보스 수익 카드가 모두 이 값을 읽는다. 쓰는 자리는 `syncSchedules`(`persistUnavailable`)와 `fetchCharacterBasicCached`(성공하면 내린다) 둘이다. **Preferences 에 두는 것이 맞다** — 캐시 비우기 `일반` 그룹은 `KEEP_KEYS` 빼고 Preferences 를 통째로 지우는데 그때 `trackedCharacters` 도 함께 날아가, 표식만 남고 목록이 사라지는 상태가 존재하지 않는다 |
...
| `representativeCharacter` | `string` (ocid, 평문) | `storage/character-selection.ts` | 삭제 | **대표 캐릭터**([[ADR-143]] 결정 4, RN 앱만). 쓰는 자리는 캐릭터 설정 · 캐릭터 관리 저장(둘 다 목록 저장 뒤) · 월드 이전 확인(`replaceTrackedCharacter`)이다. 읽는 자리는 today(대표 카드 · 머리 버튼) · 캐릭터 관리 초안(`useSelectionDraft`) · 동기화 회차(대표 하나의 5분 가드 건너뛰기, [[ADR-218]] 결정 1)다. **앱 메모리에서는 선택 스토어(`features/character-selection/store.ts`)의 `representativeOcid` 가 든다**([[ADR-275]]). 화면이 이 키를 바로 쓰면 기기 값만 바뀌고 today 가 옛 대표를 그린다(이슈 #395). 미지정이면 키가 **없고**(첫 번째가 임시 대표. 그 파생값은 저장하지 않는다), 저장 시점에 대표가 목록에 없으면 지운다. 그 판정은 `setTrackedCharacterOcids` 안에 있어 목록을 어느 경로로 저장하든 매달린 대표가 남지 않는다. 목록과 대표를 한 호출로 확정하는 `setCharacterSelection(ocids, representative)` 도 있지만 제품 코드에서는 안 불린다(화면 둘은 목록을 저장한 뒤 대표를 따로 쓴다) |
...
| `lastHuntToggles` | `{ boosts: string[] }` (JSON) | `storage/last-hunt-toggles.ts` | 삭제 | 사냥 계산기에서 켠 메소 획득률 아이템(`유니온의 부` · `소형 재물 획득의 비약`)을 다음 기록의 첫 값으로 남긴다([[ADR-288]] 결정 2). **계산기로 저장이 성공한 뒤에만** 적고 새 기록에만 세운다. 수동 폼으로 적은 사냥은 안 적는다. 그 폼에 아이템 줄이 없어 빈 값을 적으면 계산기에서 켜 두던 것이 지워진다. 옛 값에 남은 `fragmentsDeferred` 는 읽을 때 무시한다([[ADR-290]] 결정 5). 어댑터는 **모양만** 본다(상한 JSON 이면 `null`). 그 `boosts` id 가 `MESO_BOOSTS` 에 아직 있는지는 되살리는 쪽이 판정한다. **보존 안 하는 이유**: `lastHuntSelection` 과 같다. 한 번 적으면 다시 생기는 값이고, 지워지면 시트가 전부 꺼진 채 열릴 뿐이다 |
...
- ~~`lastHuntToggles` 는 `{ fragmentsDeferred, boosts }` 이고 수동 폼 저장은 `fragmentsDeferred` 만 바꾼다~~ → `{ boosts }` 이고 계산기 저장만 적는다([[ADR-290]] 결정 5, 2026-09-17). `조각 가격 나중에 입력` 체크박스가 걷혔다.

- ~~수동 추적 보스 항목 · 스케줄 캐시 · 조회 원장이 보스 이름과 한글 난이도를 든다~~ → 보스 key 와 난이도 key 를 든다([[ADR-280]] 결정 12, 2026-09-15, 이슈 #445). 캐시와 원장은 옮기지 않고 다시 받는다.

- ~~`lastHuntSelection` 은 `{ ocid, ground }` 이고 사냥터 이름을 든다~~ → `{ ocid, groundKey }`([[ADR-280]] 결정 10,
  2026-09-15, 이슈 #443). 옛 모양은 읽을 때 key 로 옮긴다.

- ~~`characterBasicCache:index:{accountId}` 는 이 계정에서 **지금까지 캐싱된 적 있는** 캐릭터 전부다. 붙이기만 하고 빼지 않는다~~([[ADR-017]] 결정 6) → 마지막으로 받은 `character/list` 에 맞춘다(2026-09-13 정정, 이슈 #408). 옛 뜻에서는 목록에서 빠진 캐릭터가 인덱스에 영영 남았다. 그래서 캐릭터 관리를 열 때마다 stub 단계가 그 캐릭터를 후보 목록에 그리고, `character/list` 응답 뒤의 방출이 목록을 갈아끼우며 지웠다. [[ADR-253]] 정정 6 이 조회 원장으로 거른 것은 추적 중인 캐릭터뿐이었다.
