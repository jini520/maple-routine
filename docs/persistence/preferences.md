...
| `dataFetchedAt` | `{ cashbook: ISO }` (JSON) | `storage/data-freshness.ts` | 삭제 | 마지막 데이터 호출 시각([[ADR-231]] 결정 4 · 정정 1). **여기 적는 페이지는 가계부 하나다** - 나머지 넷은 그리는 데이터의 `syncedAt` 을 읽는다(그 값은 조회가 실제로 돈 회차에만 적히고 스케줄러 캐시에 이미 영속된다). 가계부만 남는 것은 그 데이터가 오는 원장 층이 조회 시각을 안 들어서다. **보존 안 하는 이유**: 이 값은 **캐시가 언제 것인가** 라서 캐시와 함께 지워지는 것이 맞다. 캐시를 비웠는데 시각만 남으면 새로 받은 데이터에 옛 시각이 붙는다 |
