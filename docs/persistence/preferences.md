...
| `dataFetchedAt` | `{ [page]: ISO }` (JSON) | `storage/data-freshness.ts` | 삭제 | 페이지별 마지막 데이터 호출 시각([[ADR-231]] 결정 4). 페이지 다섯(`today`·`content`·`boss`·`profit`·`cashbook`)이 당김을 가진 화면 전부다. 키를 쪼개지 않는 것은 읽는 쪽이 언제나 다섯을 한 번에 필요로 해서다. **보존 안 하는 이유**: 이 값은 **캐시가 언제 것인가** 라서 캐시와 함께 지워지는 것이 맞다. 캐시를 비웠는데 시각만 남으면 새로 받은 데이터에 옛 시각이 붙는다 |
