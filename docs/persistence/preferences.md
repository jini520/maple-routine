...
| `characterBasicCache:index:{accountId}` | `string[]` (ocid 목록, JSON) | `storage/character-basic-cache.ts` | 삭제 | "이 **계정**의 **마지막으로 받은 `character/list` 에 있던** 캐릭터 중 캐시 항목이 쓰인 것" 역인덱스([[ADR-017]] 결정 6 + [[ADR-086]] 결정 9. 뜻은 2026-09-13 정정으로 바뀌었다, 이슈 #408, **설계 · 구현 전**). 읽는 곳은 캐릭터 관리 피커의 stub 단계 하나다(`getCharacterPickerRoster`). **붙이는 곳**은 `setCachedCharacterBasic` 이다. **줄이는 곳**은 `getCharacterPickerRoster` 가 `character/list` 를 받은 직후이고, 지금 연 계정의 인덱스를 그 계정 목록에 있는 ocid 로 줄인다. 붙이기만 하면 목록에서 빠진 캐릭터(삭제 · 월드 리프로 남은 옛 ocid)가 인덱스에 영영 남아, 캐릭터 관리를 열 때마다 후보 목록에 떴다가 사라진다. **줄이는 것은 인덱스뿐이다.** 캐시 항목(`characterBasicCache:{ocid}`)은 today · 보스 수익 · 가계부 · 리프 판정 · `schedule-window` 기록이 읽어서 남긴다. 목록 조회가 실패하거나 계정을 응답에서 못 찾으면(`resolveRegisteredCharacters` 가 던지면) 안 건드린다. 붙이기와 줄이기는 둘 다 read-modify-write 라 같은 프로미스 체인(`withIndexLock`) 안에서 차례로 돈다. 잠금 밖이면 한쪽이 붙인 ocid 가 다른 쪽의 쓰기에 덮여 유실된다. 계정별로 나뉘기 전(`characterBasicCache:index`)에는 피커 stub 단계가 **이전 계정 캐릭터까지 그렸다**. 마이그레이션(`runLegacyIndexMigration`)은 전역 인덱스를 레거시 `selectedAccountId` 것으로 이관하고(예열이 채운 계정은 그것 하나뿐이라 정확) 전역 키를 지운다 |
...
- ~~`characterBasicCache:index:{accountId}` 는 이 계정에서 **지금까지 캐싱된 적 있는** 캐릭터 전부다. 붙이기만 하고 빼지 않는다~~([[ADR-017]] 결정 6) → 마지막으로 받은 `character/list` 에 있던 캐릭터로 줄인다(2026-09-13 정정, 이슈 #408). 옛 뜻에서는 목록에서 빠진 캐릭터가 인덱스에 영영 남았다. 그래서 캐릭터 관리를 열 때마다 stub 단계가 그 캐릭터를 후보 목록에 그리고, `character/list` 응답 뒤의 방출이 목록을 갈아끼우며 지웠다. [[ADR-253]] 정정 6 이 조회 원장으로 거른 것은 추적 중인 캐릭터뿐이었다.
