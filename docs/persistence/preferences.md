...
| `notices` | `Notice[]` (JSON) | `storage/notices.ts` | 삭제 | **마지막 서버 응답의 사본**([[ADR-277]], 구현 전). 조회가 성공하면 통째로 바뀌고 실패하면 그대로 남는다. 푸시 내용은 여기 안 들어온다. 사본을 분류마다 나눌지는 [[ADR-277]] 열린 질문 1 이다. ~~받은 공지 최근 50건. 푸시로 온 것과 서버에서 받은 것이 섞이고 같은 `id` 는 나중 것이 이긴다([[ADR-228]] 결정 4)~~. **보존 안 하는 이유**: 서버가 다시 준다 |
| `dismissedNotices` | `string[]` (JSON) | `storage/notice-banner.ts` | **보존** | today 배너에서 닫은 공지 id 최근 50건([[ADR-230]] 결정 4). 앱 공지가 든 조회가 성공하면 서버 목록에 없는 id 를 뺀다([[ADR-230]] 정정 9, 구현 전). 닫은 것은 배너이지 공지가 아니라 목록·상세에는 그대로 보인다. **보존하는 이유**: `notices` 는 지워도 서버가 다시 주지만 **닫았다는 사실은 기기에만 있어** 아무도 복원해 주지 않는다. 지워지면 이미 닫은 공지가 되살아난다 |
