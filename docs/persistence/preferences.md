...
| `lastHuntToggles` | `{ fragmentsDeferred: boolean, boosts: string[] }` (JSON) | `storage/last-hunt-toggles.ts` | 삭제 | 사냥 시트의 체크 셋 셋을 다음 기록의 첫 값으로 남긴다([[ADR-288]] 결정 2). `조각 가격 나중에 입력` 하나가 `fragmentsDeferred` 이고, 켠 메소 획득률 아이템(`유니온의 부` · `소형 재물 획득의 비약`)이 `boosts` 다. **저장이 성공한 뒤에만** 적고 새 기록에만 세운다. 키를 셋으로 안 쪼개는 것은 읽는 쪽이 언제나 셋을 한 번에 쓰기 때문이다(시트를 열 때 한 번 읽는다). 수동 폼으로 적은 사냥은 `fragmentsDeferred` 만 바꾼다 — 그 폼에 아이템 줄이 없어 빈 값을 적으면 계산기에서 켜 두던 것이 지워진다. 어댑터는 **모양만** 본다(상한 JSON 이면 `null`). 그 `boosts` id 가 `MESO_BOOSTS` 에 아직 있는지는 되살리는 쪽이 판정한다. **보존 안 하는 이유**: `lastHuntSelection` 과 같다. 한 번 적으면 다시 생기는 값이고, 지워지면 시트가 전부 꺼진 채 열릴 뿐이다 |
