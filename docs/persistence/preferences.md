...
| `schedulerCache:{ocid}` | `{ state: SchedulerCharacterState, syncedAt: string, version: 2 }` (JSON) | `storage/scheduler-cache.ts` | 삭제 | 캐릭터별 마지막 동기화 스냅샷(일간/주간/보스 콘텐츠). 보스 항목은 `bossKey` · `apiName` · 난이도 key 를 든다. **`version` 이 2 가 아닌 옛 값은 없는 것으로 읽는다**([[ADR-280]] 결정 12). 옮기지 않고 다음 동기화가 덮어쓴다 |
...
| `manualTrackedContent:{ocid}` | `ManualTrackedItem[]` (JSON). 보스 항목은 `{ kind: 'boss', bossKey, difficulty }` 다. 보스 이름 · 한글 난이도를 든 옛 항목은 읽을 때 key 로 옮기고 못 찾으면 뺀다([[ADR-280]] 결정 12) | `storage/manual-tracked-content.ts` | 삭제 | 수동 모드에서 그 캐릭터가 추적할 항목의 **멤버십 + 사용자 입력 `maxCount`만**([[ADR-035]] 결정 6). 진행값·체크 상태는 여기 없고 `schedulerCache`가 단일 진실 공급원 |
