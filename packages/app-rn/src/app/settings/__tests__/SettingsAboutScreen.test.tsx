...
// ③ **OTA 상태를 이제 목으로 만들 수 있다**([[ADR-137]]). 예전에는 스토어를 값으로 import 하는
//    것만으로 죽어([[ADR-128]] 결정 7) 이 화면이 상태를 **상수로 심었고**, 그래서 웹의 「최신이면
...
/**
 * 이 화면이 마운트에서 `loadCurrentVersion()` 을 부르므로 포트가 있어야 한다([[ADR-137]] 배선).
 *
...
describe('SettingsAboutScreen', () => {
  ...
  // [[ADR-137]] 배선의 요점 — 화면이 **실행 중인 번들 버전**을 묻고 그리는가. 이것이 [[ADR-026]] 이
