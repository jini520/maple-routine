...
// `refreshAccounts()` 를 트리거하고, 그 결과(`AccountFlowStatus`)만 보여준다([[ADR-086]] 결정 6).
//
// **RN 으로 오며 로직은 한 줄도 안 바뀐다** — 상태 머신과 닫힘 판정은 core 스토어와 이 파일의
// 두 이펙트가 갖고 있고, 갈린 것은 감싸는 껍데기(`Modal`)뿐이다.
    ...
    <Modal onClose={props.onClose} testId="account-modal-overlay">
      <Modal.Panel>
        <AccountFlowStatus
          ...
          onCommitCharacters={(ocids) => {
            void handleCommit(ocids)
          }}
          // ADR-086 결정 6: 취소는 아무것도 되돌릴 필요가 없다 — 아직 아무것도 쓰지 않았다.
          // status가 idle로 돌아가면 위 effect가 모달을 닫는다.
          onCancel={reset}
          // 이슈 #78 D: `reset` 이었다 — status를 'idle'로 되돌리므로 위 effect의 닫힘 판정
