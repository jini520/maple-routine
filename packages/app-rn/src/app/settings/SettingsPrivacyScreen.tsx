...
//
// 웹이 8초 타임아웃을 **유일한 신호**로 삼은 이유는 교차 출처 `iframe` 이 네트워크 실패에 `error`
// 를 신뢰성 있게 발화하지 않기 때문이었다. `WebView` 는 발화한다(`onError`) — 그래서 그 타임아웃은
// **유일한 신호에서 보조 신호로 내려간다**: 즉시 실패는 `onError` 가 잡고, 타임아웃은 "받아 놓고
// 응답하지 않는" 매달림만 맡는다. 값을 그대로 8초로 두는 것은 그 자리가 여전히 있어서이지 같은
// 일을 해서가 아니다.
//
// **`navigator.onLine` 사전 검사는 함께 사라진다.** 그것은 웹 API 이고, 이 앱에서 네트워크 종류를
// 묻는 유일한 통로(`getNetworkType`)는 `LiveUpdatePort` 를 타서 지금 던진다([[ADR-128]] 결정 7).
