/**
 * 기본 배치 — **손으로 적은 좌표**([[ADR-147]] 결정 2 · 정정 13, 사용자 확정 2026-08-17).
 *
 * ```
 * (0,0)  4x1     대표 캐릭터
 ...
 * ([[ADR-147]] 정정 32, 사용자 지정).
 *
 * **`row` 는 auto 타일의 nominal `h = 1`**(최소 높이) 기준으로 적는다. 그래서 좌표가 `(0,4)` 에서
 * 바로 이어지고, 렌더러가 실측 초과분만큼 아래를 민다([[ADR-147]] 정정 1) — **`row` 를 재계산해
 * 다시 채우지 않는다.** auto 타일이 둘이어도 규칙은 그대로다: `w === 4` 라 옆 칸이 없어 초과분이
 * 그냥 **누적**된다.
 *
...
export const TILE_LAYOUT: readonly TilePlacement[] = [
  ...
  { id: 'weekly-boss-profit', col: 0, row: 4, w: 4, h: 3 },
  { id: 'top-valuable-item', col: 0, row: 7, w: 2, h: 1 },
  { id: 'unpriced-drops', col: 2, row: 7, w: 2, h: 1 },
  { id: 'valuable-drought', col: 0, row: 8, w: 4, h: 1 },
]
