/**
 * 위젯 1 — **대표 캐릭터**([[ADR-146]] 결정 6 · 정정 2·7·8·13).
 *
 * ## 세 크기가 같은 구조다
 *
 ...
 * ## 없는 것을 그리지 않는다
 *
 * - **«대표 없음» 상태는 없다**([[ADR-146]] 정정 2) — `resolveDisplayRepresentative` 가 미지정이면
 *   목록의 첫 번째를 세운다. `representative === null` 은 **추적 캐릭터가 하나도 없을 때뿐**이라
 *   그때만 한 줄로 그 사실을 말한다. «임시 대표» 라는 표시도 하지 않는다([[ADR-144]] 결정 4).
 * - **`expRate` 가 없으면 EXP 줄 자체를 안 그린다**([[ADR-146]] 정정 8) — 0% 바를 그리면 «경험치가
...
const VARIANT = {
  /** 4x1(기본) — 내부 높이 52 에 초상화 44 가 들어간다([[ADR-146]] 정정 7). */
  row: {
    portraitPx: 44,
    emblemPx: 15,
    name: 'text-[13px]',
  ...
  /** 2x2 — 가운데 정렬. 158 폭이라 **직업이 잘린다**([[ADR-146]] 열린 질문). */
  compact: {
    portraitPx: 56,
    emblemPx: 16,
    name: 'text-sm',
