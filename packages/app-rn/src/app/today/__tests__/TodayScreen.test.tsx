// 첫 화면 「today」의 **배선**([[ADR-146]] 결정 4 · [[ADR-132]] 결정 7·8).
//
...
//    우회하는 `refresh` 는 진입에서 부르지 않는다([[ADR-097]] 결정 2·4).
//    **게이트 자체는 여기서 못 본다** — 그것은 스토어 안의 계약이라 core 테스트가 지킨다. 화면이
//    보장할 수 있는 것은 «어느 문을 쓰는가» 이고, 그 문이 바뀌면 TTL 이 통째로 사라진다.
// ② **당김과 헤더 버튼이 같은 재조회인가**([[ADR-072]] 결정 2 · [[ADR-130]] 결정 1).
// ③ **스토어가 비어도 서는가** — 콜드 스타트에 위젯 여덟이 전부 빈 상태로 선다([[ADR-146]] 결정 5).
 ...
 * ([[ADR-072]] 결정 2) — 어느 스토어를 어떤 인자로 몇 번 불렀는지가 전부 같아야 «같은» 이다.
 */
...
describe('TodayScreen — 진입 조회', () => {
  // [[ADR-132]] 결정 8 «today 는 그 순차 밖의 **네 번째 트리거**» — 예열이 셋을 돌고 이 화면이
  ...
  // 회귀 가드 — 게이트는 `loadTrackedOcids` **안**에 있다([[ADR-097]] 결정 4). 진입이 `refresh` 를
  ...
  // 드롭 히스토리는 예열 목록 밖이고 네트워크도 안 탄다([[ADR-071]]) — 동기화 트리거 수에 들지
  // 않으므로 위 케이스와 갈라 둔다.
...
describe('TodayScreen — 명시적 재조회', () => {
  ...
  // 명시적 재조회는 TTL 을 무시하고([[ADR-097]] 결정 2) **이 화면이 그리는 스토어 셋을 모두** 읽는다
...
describe('TodayScreen — 격자', () => {
  // [[ADR-146]] 결정 5 — 데이터가 없다고 타일을 빼지 않는다. 콜드 스타트(스토어 전부 빈 값)에서도
  ...
  it('캐릭터 넷의 기본 배치', async () => {
    // 가뭄 문구는 마운트마다 풀에서 무작위로 뽑는다([[ADR-146]] 정정 6) — 그대로 찍으면 스냅샷이
