...
// ## 왜 «없음» 을 테스트하나
//
// 이 정책은 [[ADR-047]] 의 중첩 sticky 를 **안 만들기로 한** 결정이라, 지켜야 할 코드가 없다.
...
//
// 이 저장소가 [[ADR-092]] 디버그 도구·[[ADR-119]] 배포 가드에서 쓴 방식과 같다: **결정을 문서가
// 아니라 실패하는 테스트로 지킨다.**
//
// ## 무엇을 잡나
//
...
// - `stickyTop` — [[ADR-047]] 이 쓰던 오프셋 프롭 이름. 되살아나면 그 구조가 함께 온다는 신호다
