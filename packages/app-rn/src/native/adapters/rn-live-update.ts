...
/**
 * `LiveUpdatePort` 의 `expo-updates` 구현 ([[ADR-137]]).
 *
 * [[ADR-128]] 결정 7 이 *"별도 ADR"* 로 미뤄 두고 `not-implemented.ts` 가 던지던 자리다.
 *
 * ## 이 파일이 «번역기» 인 이유
 *
 * 포트가 말하는 다섯 갈래(`unsupported`·`error`·`up-to-date`·`store-required`·`update-available`)는
 * **프로토콜과 무관한 «앱이 무엇을 할 수 있나»** 다([[ADR-137]] 결정 6). `expo-updates` 는 그중 넷을
...
/** 매니페스트 URL 의 형제 — 같은 Worker 의 다른 경로다([[ADR-137]] 결정 4). */
const LATEST_URL = `${appJson.expo.updates.url.replace(/\/manifest$/, '')}/latest`

/**
 * 매니페스트 `extra` 에서 우리 축의 값을 꺼낸다([[ADR-137]] 결정 5).
 *
 * **전부 선택 필드로 읽는다.** [[ADR-119]] 가 `highlights` 를 필수 검사에 안 넣은 것과 같은 이유이고,
 * 이번에는 더 세다 — 매니페스트를 만든 스크립트와 그것을 읽는 앱이 서로 다른 시점의 것일 수 있다.
 * 없으면 모달은 그 부분을 안 그린다(버튼째 없다 — [[ADR-126]] 결정 6).
 */
...
async function checkStoreRequired(): Promise<LiveUpdateCheckResult | null> {
  try {
    ...
    // 이름이 아니다([[ADR-137]] 결정 3). 모달은 그 줄을 안 그린다.
...
export const rnLiveUpdatePort: LiveUpdatePort = {
   ...
   * 직전 번들로 되돌렸다([[ADR-022]] 결정 6 → [[ADR-117]] 결정 2 가 그 호출 자리를 `AppShell`
   ...
   *
   * 그래서 [[ADR-117]] 결정 2 가 **지키려던 것**(*"렌더가 던지는 번들이 SUCCESS 로 찍혀 영구히
   ...
   * 버전은 매니페스트 `extra.appVersion` 이고([[ADR-137]] 결정 5), 내장 번들로 돌 때는 그 값이
  ...
  // 채널이 하나로 줄었다([[ADR-137]] 결정 7) — [[ADR-024]] 의 빌드 시점 분리는 사이드로딩 베타를
  // 위한 것이었고 App Store 출시로 용도가 끝났다. 표시값은 남는다([[ADR-026]] 관찰용 UI).
  getChannel() {
   ...
   * 사용자 동의 후 받는다.
   *
   * 진행률은 **네이티브 상태 변경 이벤트**로 온다(`downloadProgress` 는 0~1) — [[ADR-061]] 결정 1 의
