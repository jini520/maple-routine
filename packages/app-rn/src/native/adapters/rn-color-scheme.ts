...
/**
 * `ColorSchemePort` 의 RN 구현([[ADR-127]] 결정 4 — 밖으로 나가는 시그니처는 Capacitor 구현과 한
 * 글자도 다르지 않다). 정책은 [[ADR-009]](2026-07-14 시스템 다크 모드 연동) · [[ADR-104]].
 *
 ...
 * 판정**에만 쓰이고 실행 중 OS 설정 변경은 따라가지 않는다([[ADR-104]]) — `Appearance` 에
