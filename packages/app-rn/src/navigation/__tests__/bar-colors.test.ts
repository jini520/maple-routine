// 바 색의 **관계**를 등록된 테마 전부에 대고 검사한다([[ADR-132]] 결정 11 · 정정 12).
//
...
import { hexToOklch, parseHex, relativeLuminance } from '@core/lib/color'
...
describe('떠 있는 바의 색 ([[ADR-132]] 정정 12)', () => {
  ...
  // ── 글자 ──────────────────────────────────────────────────────────────────────
  // **강조색은 테마의 메인 컬러다** ([[ADR-132]] 정정 23, 사용자 판정 — *"active 탭의 컬러가 왜
  ...
  // **활성과 비활성이 같은 색으로 보이면 안 된다.** 레테 실측에서 활성 글리프 (176,148,196) 와
  // 비활성 (184,156,189) 이 사실상 같은 색이었다 — 그 테마는 `textMuted` 자체가 연보라라, 강조색을
  // `text` 쪽으로 미는 것만으로는 비활성과 같은 자리에 도착한다([[ADR-132]] 정정 20).
  //
