// 안전영역 페이드의 **두 값**([[ADR-134]]) — 어디를 깎는가(결정 3)와 어떻게 깎는가(결정 2).
//
...
describe('페이드는 콘텐츠가 실제로 지나가는 자리에만 있다 ([[ADR-134]] 결정 3)', () => {
  ...
  // 설정 계열. 셸이 스크롤포트를 안전영역만큼 내리므로([[ADR-099]] 결정 6) 콘텐츠가 그 자리에
...
describe('마스크 곡선은 smoothstep² 이다 ([[ADR-134]] 결정 2 · 정정 2)', () => {
  /** 세지기 전 곡선. 정정 2 가 이것을 제곱했다. */
