...
// ── '다시 시작' 이 필수 프롭이 됐다 ────────────────────────────────────────────────
//
// 웹은 기본값이 `window.location.reload()` 였다. **RN 에는 그 짝이 없다** — 번들을 다시 실행하는
// 것은 OTA 런타임의 일이고(`expo-updates` 의 `reloadAsync`), 그 도입은 [[ADR-128]] 결정 7 이 별도
// ADR 로 미뤄 둔 작업이다. 없는 기본값을 지어내면(예: 상태만 초기화해 다시 렌더) 같은 예외로 즉시
// 되돌아오는 버튼이 되어 [[ADR-065]] 결정 5 가 세운 *"그 하나가 분명해진다"* 를 깬다. 그래서 이
// 컴포넌트는 재시작 수단을 **호출부에서 받는다** — OTA 런타임이 붙는 순간 그 자리 한 곳만 바뀐다.
