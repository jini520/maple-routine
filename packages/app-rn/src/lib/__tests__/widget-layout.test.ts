// 좌표 배치의 **검증과 해석**([[ADR-146]] 결정 2 · 정정 1). 배치를 손으로 적기로 한 이상 그
...
/** [[ADR-146]] 정정 13 의 기본 배치 앞부분 — 실제로 쓸 좌표를 그대로 쓴다. */
...
describe('validateWidgetLayout — 손으로 적은 좌표를 지킨다 ([[ADR-146]] 결정 2)', () => {
  it('유효한 배치는 빈 배열이다', () => {
  ...
  it("w < 4 인 타일은 h: 'auto' 를 쓸 수 없다 ([[ADR-146]] 정정 1)", () => {
...
describe('resolveWidgetPositions — 좌표를 절대 위치로 ([[ADR-146]] 결정 2)', () => {
  ...
  // 캐릭터 4명이면 `55 + 45 × 4 = 235` 인데 최소 높이가 76 이라 초과분이 160 이다([[ADR-146]] 정정 1).
