// 떠 있는 바의 **치수**([[ADR-132]] 정정 30). 바를 렌더하지 않고 볼 수 있어서 따로 있다 —
...
describe('바의 세로는 창 폭에서 나온다 ([[ADR-132]] 정정 30)', () => {
...
describe('큰 화면에서는 폭이 상한에서 멈춘다 ([[ADR-132]] 정정 30)', () => {
...
// 이 값이 바의 실제 높이와 어긋나면 콘텐츠가 바 뒤로 들어가거나(작으면) 바닥에 빈 띠가 남는다
// (크면). 예전에는 상수 두 벌이 그것을 약속으로만 지켰다(`FLOATING_BAR_SPACE_PX === 72`).
