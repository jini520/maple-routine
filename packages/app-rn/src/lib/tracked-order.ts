/**
 * 「저장 순서가 곧 표시 순서다」([[ADR-143]] 결정 3) — 추적 목록(`trackedCharacters`)의 배열
 * 순서를 화면 목록에 얹는다.
 *
 * ## 왜 core 스토어를 안 고치고 화면 쪽에서 얹는가
 *
 * 컨텐츠·보스 스토어는 `sortByCachedLevel`(레벨 내림차순, [[ADR-017]] 결정 2)로 목록을 정렬하고
 * **Capacitor 앱이 그 정렬로 산다.** 거기서 «저장 배열 순서» 로 바꾸면 웹뷰 앱의 화면 순서까지
 * 함께 바뀌어 이 개편의 적용 범위(RN 만)를 넘는다. 그래서 core 는 **안정된 기준 순서를 그대로
 * 내고**, RN 이 그 배열을 이 함수 하나로 다시 세운다([[ADR-143]] 결정 3 의 구현 지시, 2026-08-17).
 *
