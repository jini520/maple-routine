/**
 * core 가 쓰는 **Vite 전용 `import.meta` 확장의 타입 선언만**.
 *
 * ## 왜 필요한가
 *
 * `tsc` 는 상대 import 를 `paths` 로 못 돌리므로(그리고 번들러 치환도 모르므로) `@core/...` 를 타입
 * 검사하면 core 의 **원본**까지 따라 들어간다. 그 안에서 Vite 의 컴파일 타임 API 를 만나면 이 패키지엔
 * `vite/client` 타입이 없어 `Property '…' does not exist on type 'ImportMeta'` 로 죽는다.
 *
 ...
 *
 * 에셋 목록이 빌드 타임 `import.meta.glob` 에서 **커밋된 생성물**로 바뀌면서 core 에 glob 사용처가
 * 0이 됐다. 선언을 남겨 두면 *"쓰면 타입은 통과하고 런타임에 죽는"* 함정이 그대로 살아 있으므로
 * 함께 지운다 — 이제 core 에서 `import.meta.glob` 을 쓰면 **tsc 가 먼저 막는다.**
 * 그 자리를 지키던 전수 목록 테스트(`src/__tests__/core-shims.test.ts`)는 "0이어야 한다"로 뒤집혔다.
 */
