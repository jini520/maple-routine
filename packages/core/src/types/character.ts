...
export interface CharacterPickerEntry {
  ...
  world?: string
  /**
   * 캐릭터 카드 2줄의 «레벨 + 직업»([[ADR-144]] 결정 2). 출처는 `character/list` 이고 `world` 와
   * 같은 이유로 옵셔널이다 — 모르면 그 자리를 비우고 레벨만 그린다([[ADR-101]] 결정 1).
   */
  jobClass?: string
  /**
...
export interface CharacterBasicProfile {
  ...
  world?: string
  // 캐릭터 카드 2줄의 «레벨 + 직업»([[ADR-144]] 결정 2). **값의 출처는 `character/basic` 이 아니라
