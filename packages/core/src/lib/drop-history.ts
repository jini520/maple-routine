...
export interface DropHistoryLine {
   ...
   * 떼어 준다. `connector` 는 상자명 뒤 연결부("를 열어 ")로, 조사가 붙어 있어 화면이 한국어 문법을
   * 계산할 필요가 없다.
   */
...
/**
 * 기록 한 건을 한 줄 문장으로 만든다(사용자 지정 형식, 2026-07-31):
 * `지내우시님이 가디언 엔젤 슬라임(카오스)에서 가디언 엔젤링을 획득하였습니다.`
 *
 * 아이콘·난이도 배지·2단 레이아웃을 쓰지 않고 문장 하나로 두는 것이 요점이다 — 한 기록이 목록에서
 * 큰 비중을 차지하지 않게 하려는 것(사용자 요청). **고가 아이템만** 화면이 `item` 조각을 골드로
 * 감싸 꾸민다. 그래서 이 함수는 `item` 을 따로 반환한다(문자열 하나로 합쳐 주면 강조할 수 없다).
 *
 * `characterName` 이 없으면(캐시 미스) 이름 부분을 비운다 — ocid 는 사용자에게 뜻이 없는 값이라
 * 대신 넣지 않는다.
 */
 ...
 * - **달력 주 경과**로 센다 — "그 사이 실제로 처치한 주만" 이 더 정확해 보이지만 백필 안 된 과거
 *   주는 처치 여부를 알 수 없어 셀 수도 뺄 수도 없는 주가 생긴다.
 * - 고가 여부는 화면 배지들과 같은 술어(`isValuableDrop(itemName)`)를 쓴다 — 상자 개봉 결과는
 *   상자명이 아니라 나온 아이템 이름으로 판정된다는 뜻이고, 그게 기존 강조 연출의 규약이다.
 ...
 *
 * **0단계(이번 주 획득)가 기쁨의 기준점이다.** 이게 없으면 아래 단계가 슬픔이 아니라 그냥 흐린 UI로
 * 읽힌다.
 *
 * ⚠️ **경계값은 아직 사용자 확인 전이다** — 실제 물욕템 체감 주기가 아니라 구현자가 잡은 기본값이다
 ...
 *
 * **단계마다 문구가 풀이다**([[ADR-146]] 정정 6·10) — 처음엔 마지막 단계만 여럿이었는데, today 의
 ...
 *
 * 단계마다 풀 크기가 달라 "마지막 단계 개수" 상수로는 모자란다([[ADR-146]] 정정 6). 인덱스를 감싸므로
