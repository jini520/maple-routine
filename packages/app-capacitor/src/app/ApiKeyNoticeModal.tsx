...
//
// ADR-116 결정 1: 429(호출 한도 초과)가 이 사슬을 **그대로** 탄다 — 원인은 달라도 처방이 같기
// 때문이다(저장된 키로는 앞으로 갈 수 없고, 사용자가 새 키를 넣어야 한다). 그래서 새 알림 UI 를
// 만들지 않고 **문구만** 갈린다.
//
// **닫을 수 없다**: `onClose` 가 no-op 이라 오버레이를 눌러도 닫히지 않고, 취소 버튼도 없다.
// 두 원인 모두 그 상태에서는 어느 화면도 제 기능을 못 하므로 **닫아서 돌아갈 곳이 없다** — 닫기를
// 주면 아무것도 안 되는 화면에 사용자를 되돌려 보내는 셈이다. 진행 동작 하나만 둔다(진행 중 배경
// 탭을 막는 `UpdatePromptModal` 과 같은 방식).
type NoticeCopy = {
...
const NOTICE_COPY: Record<ApiKeyNoticeKind, NoticeCopy> = {
  ...
  // 갈리는 것이 의도다 — 토스트는 `truncate` 라 한 줄이 상한이지만 모달은 줄바꿈이 되므로,
  // "자리마다 담을 수 있는 만큼" 기준이 여기서는 담는 쪽으로 나온다(ADR-114 결정 4). 문구가
  // 키의 단계를 판정하지 않고 안내만 하는 것은 ADR-114 결정 1 그대로다.
  rateLimited: {
    icon: Gauge,
    title: '호출 한도를 초과했습니다',
