...
export default tseslint.config(
  ...
  // 글자 배수 클램프는 atom 둘이 쥔다([[ADR-152]] 결정 4 · 정정 1). `react-native` 에서 직접
          ...
          paths: [
            {
              ...
              message:
                "글자는 'components/atoms/Text/Text', 입력 칸은 'components/atoms/TextInput/TextInput' 에서 가져올 것. 시스템 글자 크기 클램프가 거기 있다([[ADR-152]] 결정 4 · 정정 1). 칸에 묶여 못 커지는 자리는 `fixed` 프롭.",
            },
          ],
        },
      ],
  ...
  // 예외. ① 클램프를 실제로 거는 atom 둘([[ADR-152]] 정정 1) ② 테스트 — 픽스처가 그리는 글자는 제품 UI 가 아니고,
