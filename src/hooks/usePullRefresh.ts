/**
 * 당김 인디케이터의 켜짐을 내는 훅. 사용자가 당긴 회차에만 참이다.
 *
 * `status === 'loading'` 을 그대로 쓰면 안 된다. 그 값은 지금 조회 중인가 이지 사용자가
 * 당겼는가 가 아니고, 조회를 시작하는 자리가 당김 말고도 둘 더 있다(화면 마운트 하이드레이션 ·
 * 헤더 새로고침 버튼). 그래서 탭을 옮기기만 해도 인디케이터가 프로그램적으로 열린다.
 *
 * 잃는 것은 없다. 헤더 버튼은 자기 표시를 이미 갖고(아이콘이 돌고 `조회 중…` 이 뜬다) 자동
 * 조회는 원래 조용해야 하는 것이다.
 *
 * **부르는 것은 `ScreenScroll` 하나다.** 화면이 직접 부르면 인디케이터 조립이 화면마다 한 벌씩
 * 생기고, 그중 하나가 테마 색을 빠뜨려도 아무도 모른다. 화면은 셸에 `onRefresh` 로 함수만 준다.
 *
 * 스토어가 아니라 화면 로컬이다. 방금 이 화면에서 당겼는가 는 그 마운트에만 뜻이 있다. 스토어에
