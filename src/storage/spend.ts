...
export const SPEND_CATEGORIES = [
  '컨텐츠',
  // 갈래 이름이 바뀌었다. 기존 기록은 `db.ts` 의 마이그레이션이 옮긴다. 값 자체가 이름이라
  // 안 옮기면 고아가 된다.
  '이벤트·BM',
  '버프',
  '아이템 구매',
...
export interface SpendRecord {
  ...
  form: string | null
  /**
   * 아이템 구매의 종류. 게임 인벤토리 탭 이름 그대로다. 이 값 하나가 수량과 관세를 함께
   * 가른다. 장비는 하나를 사고 관세가 붙으며, 소비·기타는 여럿을 사고 월드 간 거래가 안 되어
   * 관세가 없다.
   *
   * `form` 과 갈라 둔 것은 그 칸이 카탈로그의 `forms` 를 뜻하기 때문이다. 한 칸이 두 뜻을
   * 들면 지금 이건 어느 쪽이냐 를 묻는 코드가 생긴다.
   *
