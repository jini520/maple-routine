/**
 * 저장소 포트 — `storage/*` 가 플랫폼 플러그인 대신 이 인터페이스에만 의존한다([[ADR-128]]).
 *
 * [[ADR-003]]·[[ADR-005]] 가 그은 어댑터 경계는 원래 "`features/*` 가 저장소를 직접 만지지 않는다"
 ...
 * ([[ADR-128]] 결정 4 — 그것이 `features/` 39개가 무수정으로 사는 유일한 조건이다).
 *
...
export interface PreferencesPort {
   ...
   * 범위와 용량을 계산한다([[ADR-052]]·[[ADR-058]]). 빠지면 설정의 「캐시 삭제」·「계정 데이터
   * 삭제」가 죽는다.
   */
  keys(): Promise<string[]>
}
 ...
 * 타임아웃([[ADR-117]] 결정 5)은 전부 `db.ts` 에 남는다. 이 포트는 **플러그인 호출 그 자체**만 맡는다.
 */
