...
// `db.ts` 는 이 어댑터가 들어오며 한 글자도 안 고쳤다.
...
describe('db.ts 와 맞물리는가', () => {
  ...
  it('옛 스키마 DB 에는 빠진 컬럼을 더한다', async () => {
    ...
    expect(executed.filter((statement) => /^ALTER TABLE \S+ ADD COLUMN/.test(statement))).toEqual([
      ...
      'ALTER TABLE boss_party_settings ADD COLUMN drop_shares_total INTEGER',
  ...
  it('컬럼이 이미 있으면 더하지 않는다', async () => {
    ...
    /**
     * 여기도 개수를 안 박는다. 대신 성질을 본다. 데이터 이관은 전부 `WHERE` 를 갖고, 그것이 이미
     * 옮겨진 뒤에는 걸리는 행이 없다는 근거다.
     */
