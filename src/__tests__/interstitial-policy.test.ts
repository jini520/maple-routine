// 전면광고 정책 가드 — [[ADR-150]].
//
// **`app-rn` 에는 전면광고가 없다.** [[ADR-128]] 이 RN 으로 간 동인 자체가 광고 인벤토리였고
// (웹뷰에는 카드 *사이*에 광고를 넣을 자리가 원리적으로 없어 전면광고 말고 선택지가 없었다),
// 그 제약이 사라졌으므로 먼저 걷었다. 인라인 광고는 후속이다.
//
// ## 왜 «없음» 을 테스트하나
//
// [[ADR-131]] sticky 가드·[[ADR-129]] 글롭 가드와 같은 이유다 — 지켜야 할 코드가 없는 결정이라
// 회귀가 «기능이 깨지는» 모양이 아니라 **«없기로 한 것이 슬그머니 돌아오는»** 모양으로 온다.
// `packages/core` 에는 `maybeShowTabSwitchAd` 가 그대로 살아 있고(`app-capacitor` 가 쓴다),
...
const FORBIDDEN: Array<{ pattern: RegExp; what: string }> = [
  ...
  { pattern: /\bshouldGateAd\b/, what: '[[ADR-132]] 결정 9 의 광고 게이트 판정' },
]

describe('[[ADR-150]] app-rn 에 전면광고가 없다', () => {
