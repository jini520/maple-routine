...
//
// ① [[ADR-129]] 가 목록을 커밋된 생성물로 옮기면서 그 사슬의 **첫 고리가 사라졌다** — 글롭 사용처가
//    0이 되고, 치환 다섯이 필요 없어졌으며, `ImportMeta.glob` 선언도 함께 지웠다. 그래서 「전수
//    목록」이 **「0이어야 한다」로 뒤집혔다**: 감시 대상이 없어진 것이 아니라 *"없는 상태가
//    유지되는가"* 가 감시 대상이 됐다.
//
// ② [[ADR-155]] 가 `core/` 를 `src/` 로 녹이면서 **치환 기구 자체가 사라졌다**(`core-shims.js` ·
