 ...
 *
 * 어댑터가 플랫폼 플러그인을 직접 import 하면 어댑터를 프레임워크 없는 패키지로 옮길 수 없다.
 * 여기서 뒤집는 것이 그 방향 하나뿐이고, 밖으로 나가는 `native/*` 함수 시그니처는 안 바뀐다.
 *
 * 포트는 플랫폼을 모른다. 어댑터가 그 판정을 든다. 이 인터페이스를 부르는 쪽이 아는 것은
 * 부르면 알아서 된다 뿐이고, 그래서 `isSupported()`·`prepareInterstitial(): boolean` 처럼
 * 결과로 능력을 말하는 자리는 있어도 지금 어느 플랫폼인가 를 묻는 자리는 없다.
 *
 * 포트 구현은 앱이 부팅 시 주입한다. 주입 전에 네이티브를 건드리면 조용히 넘어가지 않고
 * 던진다. no-op 으로 두면 이 플랫폼엔 그 기능이 없다 와 포트가 없다 가 구분되지 않아, 스플래시가
 * 안 걷히거나 광고가 안 뜨는 것이 정상 동작처럼 보인다.
 */
 ...
 * 자기 포트가 있다.
 *
 * 타입 두 개를 import 하는 자리가 여기뿐이다. 둘 다 `import type` 이라 런타임 의존은 0 이다.
 */
 ...
 * 손끝에 내는 짧은 두드림.
 *
 * 부르는 쪽이 아는 것은 **무슨 일이 일어났는가**이고, 그것을 어느 API 로 내는지는 어댑터가
 * 든다. 두 플랫폼이 서로 다른 자리를 쓰는데(안드로이드의 `impactAsync`·`selectionAsync` 는
 * `Vibrator` 로 흉내 낸 것이라 시스템 촉각 설정을 무시한다) 그 갈림이 호출부로 새어 나오면
 * 화면마다 다시 갈린다.
 *
 * **세기를 고르는 인자가 없다.** 고를 수 있게 두면 자리마다 다른 세기가 붙어 앱 전체의 촉감이
 * 갈린다. 메서드가 둘인 것은 세기가 둘이어서가 아니라 **일어난 일이 둘**이어서다.
 */
export interface HapticsPort {
  /** 눌러서 어딘가로 갔다. 진동 장치가 없거나 시스템이 껐으면 거절한다(호출부가 삼킨다). */
  tap(): Promise<void>
  /** 고른 값이 바뀌었다. 세그먼트 셋만 부른다. 거절하는 조건은 `tap` 과 같다. */
  select(): Promise<void>
}
...
export type LiveUpdateCheckResult =
   ...
   */
  /**
   * `minNativeVersion` 은 선택이다. `expo-updates` 의 `runtimeVersion` 은 fingerprint 해시라
   * 사용자에게 보여 줄 이름이 아니다. 그래서 이 값을 비우고 모달은 그 줄을 안 그린다. 없는
   * 숫자를 지어내지 않는다.
   */
 ...
 * Live Update (OTA).
 *
 * 경계는 프로토콜이 아니라 행위다. 이 인터페이스가 @capgo 의 모양을 그대로 드러내면
 * (`httpGet`·`download({url, checksum})`·`applyBundle(id)`) `expo-updates` 에 짝이 없다.
 ...
 *
 * 프로토콜이 바뀌어도 사용자가 보는 것은 안 바뀐다는 것이 이 경계의 목적이다.
 */
export interface LiveUpdatePort {
   ...
   *
   * 어댑터가 갖는 것은 그것이 빌드 시점 값이기 때문이다. core 에서 `import.meta.env` 를 읽는
   * 한 줄은 RN 에서 모듈을 평가하는 순간 죽는다.
   */
  getChannel(): string
