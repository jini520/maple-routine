 ...
 * - `isNewerVersion` — 순수 버전 비교. 스토어의 완료 안내 판정([[ADR-126]] 결정 4)이 쓴다.
 * - `applyLiveUpdate` — **적용 순서**([[ADR-117]] 결정 1). 아래 주석 참고.
 */

/**
 * `x.y.z` 세 자리를 비교해 **후보가 더 새것인가**를 답한다.
 *
 * 세 자리가 아니거나 숫자가 아니면 `false` 다 — [[ADR-024]] 가 늦게 발견한 버그가 정확히 여기였고
 ...
 *
 * **받는 것과 적용하는 것이 갈려 있다는 것이 [[ADR-027]] 결정 4 다** — 받아도 자동으로 적용되지
 ...
 *
 * **순서는 닫기 → 커버 → 적용이다([[ADR-117]] 결정 1).** 커버가 닫기 **뒤**인 것이 요점이다 —
 ...
 * 한 줄뿐이라, 이 함수는 [[ADR-137]] 에서도 그대로 남았다.
 *
 * SQLite 커넥션을 먼저 닫는 이유([[ADR-008]] 세 번째 정정): 안 닫으면 리로드로 JS 쪽 캐시만
 * 초기화되고 네이티브 커넥션은 stale 하게 남아, 재로드 후 첫 쿼리가 응답 없이 멈춘다.
 * `closeBossProfitDb` 는 던지지 않고 5초 안에 끝난다([[ADR-117]] 결정 5) — 여기서 또 감싸지 않는다.
