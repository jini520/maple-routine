/**
 * 캐릭터의 메소 획득량을 읽는 여섯(스킬 다섯 + 챌린저스).
 *
 * `character/basic` 에는 메획이 없다. 캐릭터 속성이 필요하면 추가 호출 전에 이 응답부터 확인
 * 하는 규칙을 따라 확인했고, 그래서 여섯을 새로 부른다.
 *
 * 부르는 계기는 시트에서 캐릭터를 고를 때 하나다. 시트를 열 때마다가 아니다. 여섯은 병렬이고,
 * 사용자 조작 하나에 6건이라 초당 500건 예산에 닿지 않는다.
 *
