...
// ── 무엇을 두지 않기로 했는가 ─────────────────────────────────────────────────────
//
// ① **스크림이 없다**(사용자 지정). 뒤를 어둡게 덮으면 ⓐ 피커 모달·키 안내 모달과
...
// ② **`border-panel-border` 가 아니라 `border-border` 다.** 그 토큰은 가 **스크림 위
...
// (머리 **값 규칙의 자리**). 이 컴포넌트는 받은 것을 그리기만 한다. 저장소도
// 네트워크도 모르고, **얼굴 하나 때문에 `character/basic` 을 부르지 않는다**(가
// 산 **안 열어 본 계정의 비용 0** 을 도로 내주는 일이다).
//
// ── RN 사정 셋 ────────────────────────────────────────────────────────────────────
//
// ⓐ **`Modal` 을 쓰는 이유**는 과 같다. RN 의
...
//  `opacity-0` 으로 기다린다. 이 없앤 **모르는 사실을 그리는 프레임** 과 같은 종류다.
