...
import { screenEffectScale } from '../drop-effect-layout'
...
describe('DropEffectOverlay: 구조', () => {
  ...
  // 검은 배경이 **순검정 사각형으로 그대로 보인다**. 눈으로만 잡히는 종류라 값이
  // 아니라 **어디에 걸렸는지**를 고정한다.
...
// 있고, 매핑 없는 아이템은 영영 안 그려진다.
  ...
  //
  // 엔진이 붙은 뒤로 첫 탭은 `end` 재생을 시작하고 그것이 끝나야 `onClose` 가 온다.
  ...
  // 배치된다. `width="100%"` 가 그 크기로 굳으면 화면 아래 46dp 가 안 칠해지고 그 자리로 뒤의
