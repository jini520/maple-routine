 ...
 *
 * 컴포넌트에서 떼어낸 것은 여기서 지켜야 할 것들이 전부 **눈에 안 보이는 순서**이기 때문이다.
 * 8프레임 시점에 아이템이 뜬다 · pre 가 끝나면 loop 로 넘어가 무한 반복한다 · 닫기는 end 를 한 번
 * 재생하고 끝난다. 화면 없이 검사할 수 있는 형태가 아니면 회귀를 못 잡는다.
 *
