/**
 * 드롭 연출 재생 상태 기계. 프레임 전이만 갖는 순수 함수들.
 *
 * 컴포넌트에서 떼어낸 것은 여기서 지켜야 할 것이 전부 눈에 안 보이는 순서이기 때문이다.
 * 8프레임 시점에 아이템이 뜬다 · pre 가 끝나면 loop 로 넘어가 무한 반복한다 · 닫기는 end 를
 * 한 번 재생하고 끝난다. 화면 없이 검사할 수 있는 형태가 아니면 회귀를 못 잡는다.
 *
 * 프레임을 실제로 그리는 일과 시간을 흘려보내는 일은 컴포넌트가 한다.
 */
...
/**
 * 아이템 등장 트리거. 시간이 아니라 ScreenEff 프레임 인덱스다. 버스트가 최대로 벌어지는
 * 그림에 묶여 있어 fps 배율이 바뀌어도 이 값은 그대로다.
 */
...
/**
 * `dt` 밀리초만큼 흘려보낸 다음 상태. **입력을 바꾸지 않는다.**
 *
 ...
 *
 * 그 낭비는 재생 첫머리의 불균등으로 나온다. 상태는 제때 진행하는데 렌더가 밀려 frame 0 이
 * 82ms 서 있고 frame 2 가 25ms 만에 지나갔다(기대는 둘 다 44ms). 누적 시간은 여기서 일부러
 * 뺀다. 그것 때문에 매번 달라졌다 가 되면 거르는 의미가 없다.
 */
