 ...
 *
 * ① `applyMesoKey`. 앱 키패드의 타건. 그 키패드가 있는 이유는 OS 키보드를 안 부르기
 *    위해서이고, 안 부르면 `KeyboardAvoidingView` 도 시트 높이 보정도 필요 없어진다. 그 이득은
 *    그 화면이 키보드를 한 번도 안 부를 때만 있다. 지금 그런 화면은 `DropPricePad` 하나다.
 * ② `parseMesoText`. OS 숫자 키보드가 넣은 글자. 가계부의 지출·수입 시트는 내용·시세 칸 때문에
 *    어차피 키보드를 부르므로 그 위에 앱 키패드를 또 두지 않는다.
 *
