/**
 * 금액 칸. 자릿수 전체가 주 표기이고 억/만은 보조 줄이다.
 *
 * 억/만으로 접어 보여주면 한 자를 칠 때마다 `3억` → `32억` → `3억 2,000만` 처럼 단위가
 * 갈아엎여 지금 무엇을 치고 있는지 안 읽힌다. 원시 표기는 왼쪽으로 자라기만 하므로 흔들림이
 * 없고 앱의 다른 금액 표기와도 같다. 억/만 환산은 자릿수를 눈으로 세지 않게 해 주는 값이라
 * 작게 아래에 남긴다.
 */
...
export function MesoAmountField(props: {
   ...
   *
   * 메포·캐시는 자릿수가 작아 `+100만 ~ +100억` 칩이 쓸모없고 억/만 환산도 그 단위에서는 틀린
   * 말이다. 그쪽 칩 값을 아직 안 정했으므로 잘못된 칩을 세우는 대신 안 세운다.
   */
  mesoHelpers?: boolean
  /**
   ...
   * 그 화면은 키보드를 한 번도 안 부르는 것이 이득이다.
   *
   * 켜는 자리는 가계부의 지출·수입 시트 둘이다. 내용 칸 때문에 어차피 키보드가 뜨므로 안
   * 불러서 아끼는 것이 없다.
   */
  editable?: boolean
  /**
   * 단위를 고르는 것. 금액에 속하는 축이라 여기 산다.
   *
   * 억/만 보조 줄과 같은 줄에 왼쪽으로 놓인다. 넘기는 곳은 지출 시트의 기타 하나이고, 안
   * 넘기면 그 줄은 전과 한 픽셀도 안 다르다. 드롭 판매가는 통화가 하나뿐이라 안 넘긴다.
   */
  unitPicker?: React.ReactNode
}): React.JSX.Element {
  const mesoHelpers = props.mesoHelpers ?? true
  const amountClass = `text-32 font-bold tracking-[-.03em] ${
    props.meso === 0 ? 'text-text-disabled' : 'text-text'
  }`
  return (
    <View testID={`${props.amountTestID}-field`}>
      {/* 초기화는 금액 왼쪽이다. 키패드 자리를 안 뺏고(⌫ 는 한 자씩 지우는 별개 동작이라
          남는다) 고칠 대상인 숫자 바로 옆이라 겨냥이 자명하다. 값이 0 이면 지울 것이 없으므로
          자리만 지킨다. 없애면 금액이 좌우로 흔들린다. */}
      <View className="mt-5 flex-row items-center justify-end gap-1.5 border-b border-border pb-1.5">
        <Pressable
          ...
          // RN 에 `visibility` 가 없어 투명도로 대신하고, 투명한 버튼이 눌리지 않도록 터치를
          // 함께 끈다.
      ...
      {/* 다섯 개가 390px 한 줄에 들어가도록 여백·글자를 한 단계 줄인다. `flex-wrap` 은
          안전장치다. 더 좁은 기기에서는 넘치는 대신 줄을 바꾼다. 값은 `lib/meso-quick-adds`
          가 들고, 아이템 분배 계산기도 같은 눈금을 쓴다. */}
