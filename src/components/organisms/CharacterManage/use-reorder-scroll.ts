// 끌기 중 **자동 스크롤이 만질 스크롤 뷰**([[ADR-144]] 결정 5) — 화면의 `ScreenScroll` 이다.
//
// ── 왜 화면까지 올라가는가 ──────────────────────────────────────────────────────────
//
// [[ADR-131]] 이 고정 영역을 없앤 뒤로 이 화면에는 자기 스크롤을 가진 조각이 없다. 두 층도 CTA 도
...
//
// 자리를 컨트롤러(`useCharacterManage`)에 두는 이유는 [[ADR-144]] 결정 1 과 같다: 설정 하위 페이지와
