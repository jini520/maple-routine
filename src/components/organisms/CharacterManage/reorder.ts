 ...
 *
 * 끌기는 자동 테스트가 원리적으로 못 본다. 제스처는 네이티브가 인식하고, jest 의 렌더러는
 * 레이아웃을 계산하지 않아 행이 어디까지 내려갔는가 라는 사실 자체가 없다. 그래서 이 파일에
 * 손가락 좌표 → 결과 사이의 판단을 전부 모으고 화면 쪽 코드는 좌표를 넣고 나온 값을 그리기만
 * 한다.
 *
 * 끌기와 접근성 액션이 같은 함수를 부른다. 끌기는 스크린리더로 조작할 수 없어 위로·아래로
 * 옮기기 액션이 짝으로 서는데, 그 둘이 각자 배열을 만들면 언젠가 갈라지므로 `moveOcid` 하나만
 * 둔다. 액션은 `to = index ± 1`, 끌기는 `to = resolveDropIndex(…)` 로 같은 문에 들어온다.
 */
...
/**
 * 끈 거리(px)를 몇 번째 칸에 놓이는가 로 바꾸는 계산.
 *
 * 행 높이는 얼굴(40px)이 정해 전부 같으므로 칸 간격 하나로 충분하다. 반올림이라 반 칸을
 * 넘어야 자리를 내준다. 그보다 예민하면 손을 조금만 떨어도 목록이 흔들린다.
 *
 * `pitchPx` 가 0 인 것은 아직 재지 않았다 는 뜻이라(첫 프레임) 제자리로 답한다. 나누면
 * `Infinity` 가 되어 끝 칸으로 튄다.
 */
 ...
 *
 * 깊이에 비례한다(경계에서 0, 끝에서 최대). 계단으로 두면 구간에 살짝 걸치기만 해도 최대
 * 속도로 튀어 놓을 자리를 지나쳐 버린다. 화면 밖으로 나간 손가락도 최대에서 멈춘다.
 */
