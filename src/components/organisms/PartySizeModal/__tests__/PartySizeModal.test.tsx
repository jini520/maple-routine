...
import { ILLUSTRATION_MASK_HERO } from '../../../../constants/style/illustration-card'
...
describe('PartySizeModal', () => {
  ...
  // 페이드 끝점이 카드와 다르다(`ILLUSTRATION_MASK_HERO` — 42%/82%). 같은 값을 쓰면 넓고 낮은
  // 히어로에서 그림이 너무 일찍 끊긴다.
  it('베일은 카드가 아니라 히어로 정지점을 쓴다', async () => {
    const { getByTestId } = await renderOverlay(<PartySizeModal {...props()} />)

    // 웹 마스크(`… 42%, transparent 82%`)의 정지점 + 네이티브 전용 끝점 1.
    const stops = [...ILLUSTRATION_MASK_HERO.matchAll(/(\d+)%/g)].map(([, v]) => Number(v) / 100)
