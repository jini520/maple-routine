// 에러 바운더리.
//
// 다시 시작을 누르면 주입한 `onRestart` 를 부른다. RN 에는 `window.location.reload` 짝이 없어
// 기본값이 없다. 테스트 전용이던 프롭이 여기서는 계약이라 이 케이스가 그것을 지킨다.
//
// 폴백이 뜨면 스플래시를 내린다. `jest.mock` 팩토리는 호이스팅돼 스코프 밖 변수를 못 읽는다.
// `mock` 접두 이름만 예외다.
