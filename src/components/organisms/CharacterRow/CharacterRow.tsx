...
//                                       기준이라 성질이 다르다)
//
// **모르면 그 자리를 비운다** — 레벨을 모르면 직업만, 직업을 모르면 레벨만,
// 둘 다 모르면 **2줄 자체를 그리지 않는다**(빈 줄을 남기면 행 높이만 들쭉날쭉해진다). 조회 불가는
// 그 둘보다 먼저 알아야 할 사실이라 2줄을 통째로 대체한다.
//
// ── 웹 그리드에서 가져온 것 셋 ─────────────────────────────────────────────────────
//
// ① **얼굴 크롭**은 기법 그대로이고, 표와 크기(36px)는 **`lib/face-crop` 한 곳**에서
//    온다 — 드롭다운 행·보스 수익 아바타·초상화 레일이 이미 쓰던 그 표다(아래).
// ② **엠블럼은 번들 에셋 id 라 `{ uri }` 로 감싸지 않는다** — 얼굴만 원격 URI 다.
// ③ **`w-auto` 의 짝이 `naturalAspectStyle` 이다** — RN 은 이름을 부르지 않은 축에
