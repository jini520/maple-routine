...
export function InputCardHost(): React.JSX.Element | null {
  ...
  return (
    /*
      `seed` 가 바뀌면 카드가 씨앗을 **다시 심는다**. 카드는 친 값과 스테퍼 수를 자기가 드는데,
      잇따라 열면 앞 아이템의 값이 그대로 남기 때문이다.

      **`key` 로 다시 세우지 않는다.** 다시 세우면 칸이 `autoFocus` 를 다시 걸어 키보드가 닫혔다
      열린다(사용자 지적). 그 깜빡임이 `다음` 을 누를 때마다 났다.
