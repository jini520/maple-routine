              ...
              */}
              <View testID="input-card-split-fees" className="flex-row items-stretch gap-3">
                {usesRatio ? (
                  ...
                  //
                  // **높이 132 를 못박는다.** 132 는 `비율` 칸이 제 내용으로 서는 높이다(여백 11 +
                  // 머리 23 + 간격 8 + 트랙 44 + 간격 8 + 합 26 + 여백 12). 안 못박으면 이 칸이
                  // 84 라, 세그먼트를 누를 때마다 아래 버튼 줄이 그 차이만큼 뛴다. `비율` 칸은 안
                  // 못박는다. 제 내용대로 서게 두어야 글꼴 크기가 커져도 안이 안 잘린다.
                  //
                  // 오른쪽 수수료 칸은 110 이라 줄 높이를 이 칸이 정한다(시뮬레이터 실측).
                  <View
                    testID="input-card-party"
                    className="h-[132px] flex-1 gap-2.5 rounded-[12px] bg-bg px-3 pb-3 pt-[11px]"
                  >
