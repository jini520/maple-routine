          ...
          {props.share !== undefined && (
            <View className="mt-3 gap-2.5">
              ...
              {/*
                ...
                **줄 높이를 비율 칸에 못박는다.** 카드에서 내용대로 키가 변하는 자리가 여기뿐이라
                `기본` 과 `비율` 을 오갈 때도 수수료 줄이 하나로 줄 때도 카드가 안 움직인다.
                157 은 비율 칸의 높이다 - 여백 11 + 머리 24 + 간격 16 + 본문 94 + 여백 12.

                `items-start` 다. `stretch` 면 `기본` 의 인원 상자까지 157 로 늘어나 안이 텅 빈다.
              */}
              <View testID="input-card-split-fees" className="h-[157px] flex-row items-start gap-3">
                {usesRatio ? (
                  ...
                  <View className="flex-1 gap-2.5 rounded-[12px] bg-bg px-3 pb-3 pt-[11px]">
                    {/*
                      **머리 줄만** 비율 쪽과 같은 높이다(24). 안 맞추면 `파티 인원` 과 `분배 비율`
                      이 서로 다른 높이에 선다(사용자 지정). 상자 자체는 제 크기로 두고, 카드가
                      안 움직이는 것은 바깥 줄이 진다.
                    */}
                    <View
                    ...
                    <View testID="input-card-party-body" className="h-9 items-center justify-center">
                      <PartySizeStepper
                        size="bare"
                ...
                {props.fees !== undefined && (
                  <View className="h-full flex-1 justify-center gap-3.5">
