...
// 디렉터리에 있을 뿐 **일부러 디자인을 공유하지 않는 사이**라([[ADR-060]] 결정 5), 한 파일에 두면
// 스냅샷 이름도 섞인다.
//
// **문구는 한 글자도 손대지 않았다.** 이 저장소는 에러·불가 문구를 전수 조사해 어미까지 통일한
// 이력이 있다([[ADR-062]] 결정 5) — 여기서 다듬으면 그 작업이 조용히 되돌아간다.
...
describe('UnavailableNotice', () => {
  ...
  // [[ADR-068]] 결정 1 — `notCollected` 는 넷째 얼굴이다. "영구히 확인할 수 없다"(정보 톤)와 같은
  // 말을 하면 거짓말이 되므로 중립 톤 + Clock 이고, **시각을 암시하는 표현을 쓰지 않는다**
  // ([[ADR-067]] 트레이드오프 — 집계 시각은 브래킷만 실측됐다).
  ...
  // 고칠 수 있는 실패가 아니라 API 의 알려진 제약이라 액션을 두지 않는다([[ADR-060]] 결정 5).
