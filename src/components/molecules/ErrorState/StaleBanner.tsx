...
export function StaleBanner(props: StaleBannerProps): React.JSX.Element {
  return (
    <div
      ...
      role="alert"
      className="mb-3 flex items-center gap-2 rounded-[10px] bg-error-tint px-3 py-2.5"
    >
      <AlertTriangle className="h-4 w-4 shrink-0 text-error-ink" strokeWidth={2} aria-hidden="true" />
      <span className="min-w-0 flex-1 text-left text-xs text-text">{props.message}</span>
      {props.action !== undefined && (
        <button
          type="button"
          onClick={props.action.onClick}
          className="shrink-0 text-xs font-semibold text-primary-ink hover:text-primary-hover"
        >
          {props.action.label}
        </button>
      )}
