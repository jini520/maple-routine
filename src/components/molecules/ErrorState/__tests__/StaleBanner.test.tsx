...
// 섞이지 않게 파일을 나눈다).
//
// 문구·라벨은 전부 **호출부가 넘긴 값**이고 컴포넌트는 분기를 갖지 않는다([[ADR-114]] 결정 3 —
// molecule 이 `ScheduleSyncError` 를 알면 계층 의존 방향이 뒤집힌다, [[ADR-094]] 결정 2).
...
describe('StaleBanner', () => {
  ...
  // 받았고([[ADR-114]] 결정 3), 지금은 401 이 액션 없이 안내 모달로 간다([[ADR-115]] 결정 7).
  ...
  // [[ADR-114]] 결정 2·3: 재시도가 통하지 않는 실패(429·characterUnavailable·401)에는 액션이
