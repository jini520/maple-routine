 ...
 *
 * @see [[ADR-204]] 정정 3. 치수가 세 파일에 흩어져 있던 것을 모은 경위.
 */
...
/**
 * 레일 칸. 네 화면이 같은 한 벌을 쓴다([[ADR-161]] 결정 1).
 *
 * @see [[ADR-142]] 결정 2 · 정정 1. 상자가 정사각이 아닌 이유.
 */
export const PORTRAIT_RAIL = {
  ...
  centerY: 32,
  gap: 4,
  /** 얼굴 원의 지름. [[ADR-015]] 크롭이 기준으로 삼는 상자다. */
  faceSize: 40,
  /** stroke 중심선의 반지름. */
  ringR: 26,
  ringStroke: 3,
  ...
  emptyRingStroke: 1,
  /** 반원 둘일 때만 12시·6시에서 비운다. 온전한 원에서는 틈이 결손으로 읽힌다([[ADR-142]] 정정 3). */
  ringGapDeg: 5,
  /** 이름과 레벨이 함께 도는 호 하나의 반지름([[ADR-142]] 정정 2). */
  textR: 35,
  textFontSize: 8.5,
 ...
 * 보스 수익 아코디언 헤더의 칸.
 *
 * **`slot` 은 못 바꾼다.** 헤더 높이 64px 의 재료이고([[ADR-054]] 정정 6), 칸 수와 무관하게
