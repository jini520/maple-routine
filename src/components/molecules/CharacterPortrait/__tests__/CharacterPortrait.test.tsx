// 초상화 한 칸의 계약([[ADR-142]] · [[ADR-054]] · [[ADR-204]] 정정 1).
//
...
describe('CharacterPortrait · rail 규격의 빈 링', () => {
  // [[ADR-188]] 결정 2 — 얼굴 둘레 테두리([[ADR-161]] 결정 3)는 없앴다. 진행 링을 그리는 화면에서
  ...
  // 흐림 말고도 칸 하나만 보고 읽히는 절대 신호가 남아야 한다([[ADR-161]] 결정 3).
...
describe('CharacterPortrait · rail 규격의 얼굴과 선택', () => {
  // [[ADR-188]] 결정 1 — 얼굴 뒤 회색 판을 걷는다. 캐릭터 이미지가 투명 배경이라 그 회색이 그림
  ...
  // [[ADR-161]] 결정 2 — 세기를 0.45 에서 0.3 으로 올렸다. 칸이 여섯을 넘게 늘어서면 0.45 로는
...
// [[ADR-204]] 정정 1 로 보스 수익의 아바타가 이 부품으로 들어왔다. 아래는 그 화면의 계약이고,
// 링이 진행률의 **유일한** 표현이라([[ADR-054]] 정정 7 — `n/12` 글자 보류) 접근성 이름이 곧 그
// 정보다.
describe('CharacterPortrait · compact 규격', () => {
  ...
  // 두 탭이 같은 부품을 쓴다([[ADR-059]] 결정 7). 주기를 고정하면 한쪽 탭에서 거짓이 된다.
  ...
  // 색을 손으로 적지 않고 테마 정의에서 가져와 견준다([[ADR-006]] 규약).
