// 치수 표가 지키기로 한 것 하나 — **글자와 링이 겹치지 않는다**([[ADR-142]] 결정 2 · 정정 1).
//
...
describe('rail 규격은 겹치지 않는다', () => {
  ...
  // 치수를 한 벌로 합쳤으니 **죽은 여백이 돌아온다**([[ADR-161]] 결정 1). 링을 안 그리는 관리 화면에서
...
// [[ADR-054]] 정정 3 — 링은 얼굴 **바깥**에 여백을 두고 두른다. 슬롯 하나가 얼굴 상자이면서 링이
...
describe('compact 규격은 링이 얼굴 밖에 선다', () => {
  ...
  // [[ADR-054]] 정정 6 — 헤더 높이 64px(12 + 40 + 12)의 재료다. 슬롯이 커지면 헤더가 커진다.
