...
// 각 화면의 오프셋이다.
//
// ── ⚠️ 이 컴포넌트는 **지금 아무도 쓰지 않는다** ([[ADR-130]], step 4) ──────────────
//
// 아래 갈래가 `RefreshControl` 쪽으로 닫혔다. 표에 없던 사실 하나가 판정을 결정했다 —
// **안드로이드에는 당김 거리 신호 자체가 없다**(iOS 는 `bounces` 로 `contentOffset.y` 가 음수가
// 되지만 안드로이드 `ScrollView` 는 콘텐츠를 안 움직이고 글로우만 그린다). 커스텀 마크는 그
// 플랫폼에서 제스처 계층을 **새로 만드는** 일이 되고, 그것은 옮기는 작업이 아니다.
//
// **그래도 지우지 않는다**([[ADR-130]] 결정 5). 고아가 확정되는 것은 보스 스케줄러(step 5)·보스
// 수익(step 7)까지 같은 선택을 물려받은 뒤이고, 남겨 두면 이 결정을 뒤집는 비용이 「화면 배선 한
// 줄」로 유지된다. 그때 이 파일과 테스트를 함께 지운다.
//
// ── 갈래표 (판정의 근거로 남긴다) ─────────────────────────────────────────────────
//
...
// 사항이라 코드가 조용히 정할 일이 아니다). → 그 새 결정이 [[ADR-130]] 이다.
//
// ── RN 으로 옮기며 바뀐 것 셋 ─────────────────────────────────────────────────────
//
...
// "같은 마크가 그대로 이어진다"가 코드 위에서는 성립한다. **눈으로는 끝내 못 봤다** — step 4 가
// `RefreshControl` 을 고르면서 이 마크가 화면에 붙을 자리가 없어졌다.
