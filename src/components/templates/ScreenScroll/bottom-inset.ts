 ...
 * **바닥 인셋 하나**만 준다.
 *
 * 높이로 어림잡는 것은 가 명시적으로 금지했다(*"높이로 어림잡지 않는다"*).
 ...
 *
 * ## 하한이 깔린 뒤로 두 조각은 인셋 이 아니라 안전영역 을 나눈다
 *
 * 안드로이드 하단 안전영역에 하한(34)이 생기면서 위 표의 안드로이드 = 전부 막는다 가 두 뜻으로
 * 갈렸다. 막아야 하는 것은 **내비바가 실제로 차지하는 자리**(인셋)이지 하한이 더한 몫이 아니다.
 * 그 몫은 리듬일 뿐이라 의 지나가도 되는 여백 쪽에 든다.
 *
 * 하한을 `portBottomPx` 에 통째로 실었다면 제스처 기기의 스크롤포트가 34 위에서 끝나 위 대가의
