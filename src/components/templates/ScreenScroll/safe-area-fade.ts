 ...
 * 기기마다 다른 값이 된 뒤로 이 파생은 상수가 아니라 계산이다.
 */
 ...
 *  의 페이드는 콘텐츠가 실제로 지나가는 경계에 둔다).
 *
 * | | 값 | 0이 되는 경우 |
 * |---|---|---|
 * | 상단 | 상단 안전영역(`lib/safe-area.ts`. 안드로이드는 하한 48) | 헤더가 없는 화면(설정 계열). 셸이 스크롤포트를 그만큼 내려, 깎으면 콘텐츠의 첫 줄을 깎는다 |
 * | 하단 | 하단 안전영역(`lib/safe-area.ts`. 안드로이드는 하한 34) **+ 바의 절반**(탭 화면) | 안드로이드 3버튼 내비의 하위 페이지. 스크롤포트가 이미 인셋 위에서 끝난다(`bottom-inset.ts` 결정 19) |
 *
