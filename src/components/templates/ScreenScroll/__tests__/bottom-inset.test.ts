// 하단 인셋의 두 조각. 컴포넌트에서 떼어낸 이유는 둘이다. 판정이 플랫폼에 걸려 있어 렌더로
// 검사하려면 `Platform.OS` 를 조작해야 하고(그러면 무엇을 지키는 테스트인지가 흐려진다),
// 무엇보다 RN 이 못 가르는 자리라 그 대가가 이름을 갖고 드러나야 한다.
...
const 홈인디케이터 = 34
const 안드로이드_3버튼 = 48
/** 안드로이드 제스처 내비의 값. 45px @3.0. 하한 34 아래인 유일한 경우다. */
const 안드로이드_제스처 = 45 / 3
/**
 * 바의 몫은 이제 **인자**다 (기기 폭에 따라 64~81 이다). 여기서는 기준
...
describe('떠 있는 바가 있으면 **콘텐츠 끝**에 그 몫을 남긴다', () => {
  // 떠 있는 바는 화면 위에 있어 콘텐츠가 그 아래로 지나간다. 스크롤포트를 줄이면 떠 있는 의미가
  // 사라지므로(그냥 화면이 작아진다) 여백은 콘텐츠 쪽이다. 홈 인디케이터에 세운 지나가도 되는
  // 여백과 같은 형태다.
  ...
  // 탭 화면의 몫은 인셋이 아니라 하한이 깔린 안전영역이다. 여기가 어긋나면 마지막 카드가 캡슐
  // 뒤로 들어간다. 바는 같은 하한 위에 뜬다.
  ...
  // 상수 두 벌(여기와 `BottomBar.tsx`)이 같은 값이라는 약속을 테스트가 대신 지키던 자리다.
  // 몫이 기기마다 달라지면서 그 약속은 구조가 됐다. 둘 다 `resolveBottomBarMetrics` 를 본다.
  // 그래서 여기서 물을 것은 값이 아니라 받은 값이 그대로 흐르는가 다.
  it('바의 몫은 상수가 아니라 받은 값이다', () => {
...
describe('탭바가 없으면(하위 페이지) 플랫폼이 가른다', () => {
  // 홈 인디케이터는 **지나가도 된다**. 콘텐츠 여백으로 넣어야 스크롤 가능한
  ...
  // 3버튼 내비 뒤로 콘텐츠가 지나가면 안 된다. `react-native-safe-area-context` 는 3버튼과
  // 제스처의 구분을 주지 않는다. 높이로 어림잡는 것은 금지이므로 보수적인 쪽으로 고정한다.
  // 대가는 제스처 내비 기기에서 지나가도 될 자리를 안 쓰는 것이다.
  ...
  // 하한이 깔리면 안드로이드의 전부가 두 뜻으로 갈린다. 인셋만큼은 내비바가 실제로 차지하는
  // 자리라 막고, 하한이 더한 몫은 리듬일 뿐이라 지나가도 된다. 하한을 `portBottomPx` 에 통째로
  // 실었다면 제스처 기기의 스크롤포트가 34 위에서 끝나 바닥의 배경색 띠가 그만큼 커진다.
  ...
  // 두 조각 중 하나만 쓴다가 이 함수의 옛 계약이었고, 지금은 합이 안전영역이다. 안드로이드
  // 제스처에서만 둘 다 0 이 아니다. 합이 어긋나면 같은 인셋을 두 번 비우거나 하한이 더한 몫이
  // 사라진다.
  it.each([
    ['ios', 홈인디케이터],
    ['android', 안드로이드_3버튼],
