// 하단 인셋의 두 조각. 컴포넌트에서 떼어낸 이유는 둘이다. 판정이 플랫폼에
...
describe('떠 있는 바가 있으면 **콘텐츠 끝**에 그 몫을 남긴다', () => {
  // ** 로 뜻이 바뀐 자리다.** 옛 탭바는 화면 상자 **밖**이라 둘 다 0 이면 됐지만(탭
...
describe('탭바가 없으면(하위 페이지) 플랫폼이 가른다', () => {
  ...
  // 3버튼 내비 뒤로 콘텐츠가 지나가면 안 된다. 웹뷰는 `tappableElement` 인셋으로
  ...
  // 내비바가 실제로 차지하는 자리라 막고, 하한이 **더한 몫**은 리듬일 뿐이라
