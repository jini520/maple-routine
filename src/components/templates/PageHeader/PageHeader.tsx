...
//
// 웹에서 이 헤더는 `fixed inset-x-0 top-0` + **실측 높이 spacer** 였다(
// ). 그 형태가 필요했던 이유는 하나다. 화면 전체가 문서 스크롤 하나를 공유하니
...
//    흐름 안에 있으면 뺄 자리가 없다.
// ② **실측(`useMeasuredHeight`)이 없다.** spacer 가 없으니 맞출 대상이 없다. 가 이슈
...
//  오히려 이 금지한 "첫 프레임에 spacer 0" 을 되살렸을 것이다.)
...
//
//  다만 뒤로 **상태바 밑은 지나간다.** 헤더가 올라가면 그 자리에 아무 배경이 없다.
...
//
// **상수 몫 `1rem` 은 옮기지 않는다**(사용자 판정 2026-08-16). 웹에서 그 16 이 하던
