...
// 하나까지 그대로 낸다"* 였는데, RN 에서는 그 마크업이 통째로 바뀌므로(`PageHeader.tsx`)
// 이 파일이 지키는 것도 갈린다.
//
// | 지키던 것 | 여기서는 |
// |---|---|
...
// ★ **딱 안전영역만큼이다.** 상수 몫
  ...
  //
  // 헤더가 불투명하면 벽지 위에 검은 띠가 남는다. 배경 없는 테마에서는 보이는 그림이 바뀌지
  // 않는다. 내비게이션 테마가 화면을 같은 `bg` 로 칠하므로 뒤에 같은 색이 있다. 그래서 이
  // 검사는 색이 무엇인가 가 아니라 칠하지 않는가 다.
...
// `fixed` + 실측 spacer 로 두면 그 spacer 가 헤더보다 한 프레임 늦게
  // 갱신되는 결함을 고치느라 measure/observe 두 effect 를 두었다. RN 에서
...
// 경계 페이드는 두지 않는다. 실기기에서 띠가 엉뚱한 자리에 보였다. 남기는 것은 없다는 사실
// 하나다.
describe('경계 페이드', () => {
