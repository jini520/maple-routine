/**
 * 시스템 글자 크기(OS 배수)를 `[1.0, 1.235]` 로 자르는 **산수 한 조각**.
 *
 ...
 * 이 값을 옮기려면 그 결정을 먼저 열 것.
 */
export const FONT_SCALE_MIN = 1

/**
 * 상한. iOS 배수표의 **XXL 칸**이다(임의의 숫자가 아니다).
 *
 * 다음 칸 1.353(XXXL)은 행 높이가 76 으로 고정된 today 격자에서 위험하다.
 */
...
export type Clamped<P> = Omit<P, 'allowFontScaling' | 'maxFontSizeMultiplier'> & {
  /** 상자가 글자를 못 따라가는 자리. 시스템 글자 크기를 아예 안 따른다. */
  fixed?: boolean
}
 ...
 * @param fixed 칸에 묶여 글자를 못 키우는 자리인가
 */
