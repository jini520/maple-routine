 ...
 * ([[ADR-066]] · [[ADR-199]]).
 *
 * **수익을 가리키는 자리는 전부 이것 하나를 쓴다**([[ADR-066]] 결정 1). 하단 탭바 · 총 수익 헤드라인
 * 엠블럼 · 빈 상태 · 가계부 수입 줄이 그렇다. 사용자가 고른 그림이 lucide 에 없어 직접 그렸고,
 * lucide `coins` 로 대체하면 안 된다. 그것도 동전 더미라 이 아이콘과 뜻이 겹친다.
 *
 * 겹침을 `clipPath`·`mask` 가 아니라 **좌표로** 표현한다([[ADR-066]] 결정 4). 한 화면에 여러 번
 ...
 * 채우면 동전 사이의 단이 면에 묻혀 그림이 뭉개진다([[ADR-132]] 정정 25).
 *
 * @example
