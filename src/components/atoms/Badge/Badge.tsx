...
// `tabular-nums` `shrink-0` 같은 **레이아웃 유틸까지 조합에 포함**해 센 것이었다. 그것들을
// 걷어내고 외형만 보면 조합이 10종으로 줄고, 그중 둘이 각각 3곳으로 당시 세운 기준("같은 조합이
// 3곳 이상 반복되면 그때 만든다")을 통과한다.
//
...
export function Badge({ tone, className, ...rest }: BadgeProps): React.JSX.Element {
  ...
  return <Text className={className === undefined ? base : `${base} ${className}`} {...rest} />
}
