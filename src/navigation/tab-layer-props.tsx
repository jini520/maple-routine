// 층 안의 탭 내비게이터 셋이 공유하는 props ([[ADR-167]] 결정 3 · [[ADR-193]]).
//
 ...
 *
 * ## `screenLayout` 이 **여기** 있어야 하는 이유 ([[ADR-193]])
 *
 * 안드로이드는 화면을 **불투명**하게 칠하고(전환 중 아래 화면이 비치는 것을 막으려고) 벽지를
 * 화면마다 들려 보낸다([[ADR-134]] 정정 5). 그런데 그 불투명 배경은 `NavigationContainer` 테마라
 * **모든** 내비게이터에 걸리고, 벽지는 `screenLayout` 을 준 내비게이터에만 걸린다.
 *
 * [[ADR-167]] 이 탭 위에 층 스택을 끼우면서 `screenLayout` 을 **그 스택에만** 뒀고, 이 탭들이 자기
