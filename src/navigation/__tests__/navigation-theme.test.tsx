// 내비게이션 테마. 라이브러리가 자기가 칠하는 자리에 쓸 색.
//
// 여기서 지키는 것은 하나다. 배경 이미지가 있는 테마에서 화면 배경을 비우는가. 안 비우면
// 벽지가 통째로 사라지고, 그때 증상이 어둡게 깔림 이 아니라 아예 안 보임 이라 원인을 짚기
// 어렵다. 그 자리를 값으로 고정한다.
...
// 벽지 배선의 **반대쪽 갈래**. 화면이 벽지를 들지 않는 플랫폼(iOS, 이 러너의 기본)에서는
// `ScreenBackdrop` 이 뷰를 하나도 늘리지 않고 자식을 그대로 통과시킨다. 여기서 벽지를 그리면
// 셸의 벽지와 **두 겹**이 되고, 그것이 이 없앤 **그리는 곳이 둘** 이다.
describe('화면이 벽지를 들지 않는 플랫폼', () => {
