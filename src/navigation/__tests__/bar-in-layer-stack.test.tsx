// 떠 있는 바가 **화면 안에서** 규칙대로 도는가. 규칙 자체는 `bar-groups.test.ts` 가
...
// 다른 탭으로 갈 수 있었다. `zIndex` 로는 못 고친다(부모가 다르다). 지금은 펼침판이 `LayerStack` 의
...
describe('← 는 **한 층 내려온 자리**로 되돌린다 (결정 4)', () => {
 ...
 // 분기가 안전망인 근거라 여기 남긴다. 규칙 자체는 `bar-groups.test.ts` 가 상태를 직접 만들어
 // 고정한다.
