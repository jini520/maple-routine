...
//
// 층을 스택이 들면 상태는 react-navigation 것이므로 이 함수들은 무엇을 할지(`BarIntent`)를
// 돌려준다. 규칙이 순수 함수라 판정을 물으려고 렌더가 필요하지 않다.
//
...
describe('← 는 하위 행에만 선다 (결정 3)', () => {
  ...
  // 하위 행에 있다는 것이 곧 스택 깊이 ≥ 1 이다. 기록이 없는데 하위 행에 있다 는 상태가 만들어질
  // 길이 없어, ← 가 페이지는 그대로 두고 그룹 행만 올리는 안전망으로 떨어지지 않는다.
