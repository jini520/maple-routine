// 층은 **진짜 스택 한 단**이다 — [[ADR-167]] · 이슈 #240.
//
// ## 무엇을 검사하나
//
...
describe('그룹 진입이 스택 한 단을 만든다 ([[ADR-167]] 결정 1)', () => {
...
describe('같은 층의 옆걸음은 쌓이지 않는다 ([[ADR-167]] 결정 3)', () => {
  ...
  // **이 설계의 축**([[ADR-132]] 결정 4). 둘 다 하위가 없어 같은 층이다.
  ...
  // 그때 층이 쌓이지 않고 **올라가면서 옆걸음해야** 한다([[ADR-167]] 결정 5 의 `switchGroupPage`).
...
describe('마지막으로 보던 하위를 기억한다 ([[ADR-167]] 결정 4)', () => {
...
describe('층은 하위 페이지와 같은 방식으로 열린다 ([[ADR-120]] 결정 5·6)', () => {
  it('전환과 제스처가 켜져 있다', () => {
