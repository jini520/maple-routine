 ...
 *
 * 딥링크(`linking`)를 두지 말 것. 설정을 두면 이 표가 문서에서 동작으로 바뀌어 없던 진입
 * 경로가 생긴다.
 */
...
export type TabParamList = {
   ...
   * 캐릭터 선택하러 가기, 컨텐츠·보스 스케줄러의 빈 상태 CTA.
   *
   * 파라미터가 스택에 남는다. 탭을 떠났다 돌아오면 그대로 살아 있으므로 화면이 `setParams` 로
   * 지운다(`SettingsScreen`).
   */
...
export type RootStackParamList = {
   ...
   *
   * 이름이 `Tabs` 가 아닌 것은 이 자리가 탭 내비게이터가 아니라 스택이고 탭이 그 스택의 각 단
   * 안에 있기 때문이다. 하위 페이지는 이것 위로 밀려 들어와 `Main` 통째를 밀어낸다. 바가 그
   * 안에 있으므로 탭바가 아래 화면과 한 덩어리로 밀려 나간다.
   */
   ...
   *
   * 이 자리가 유틸리티의 구조를 정한다. 도구는 유틸리티 화면 안의 카드가 아니라 루트 스택에
...
/** 대조용 경로 이름표. `origin: 'rn'` 이면 옛 앱에 없던 화면이다. */
  readonly path: string
  /** 그 경로가 그리던 화면. 두 행이 같은 값을 가질 수 있다. */
 ...
 * 보인다. 이 앱에서 스택이 2단이 되는 자리는 여기뿐이다.
 */
export const ROUTE_TABLE: readonly RouteRow[] = [
  ...
  // 여기부터 RN 에서 새로 생긴 화면. `path` 는 대조용이 아니라 이름표다. 탭 셋은 전부 진짜
 ...
 * 처음 서 있는 탭. 표의 `/` 행과 갈렸다.
 *
 * 타입이 `TabRouteName` 이 아니라 `keyof GroupLayerParamList` 다. 앱은 탭 아홉 중 하나가
 * 아니라 그룹 층의 첫 화면에서 시작한다. 하위 층은 push 로만 열리므로, 여기에 하위 페이지를
 * 적을 수 있으면 앱을 켰는데 스택이 한 단 깊은 상태가 표현돼 버린다.
 *
