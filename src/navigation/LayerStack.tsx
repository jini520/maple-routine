...
/**
 * 층 안의 화면들은 탭이다. 그래서 옆걸음에 전환이 없고 서로 언마운트하지 않는다. 바는 이
 * 내비게이터들이 그리지 않는다(`tabBar` 가 아무것도 안 낸다). 층 스택의 `layout` 이 한 벌만 그린다.
 *
 * `backBehavior="none"` 이다. `"history"` 는 모든 탭 전환을 쌓는다. 층을 오르내리는 일은
 * 바깥 스택이 진다.
 */
 ...
 * 자리 자체가 없다.
 *
 * 바가 `layout` 안에 있는 것은 `layout` 이 내비게이터의 내용 전체를 감싸기 때문이다. 화면
  ...
  const barNavigation: BarNavigation = useMemo(
    () => ({
      openLayer: (layer: LayerRouteName, target: TabRouteName) => {
        // 같은 층이 스택에 두 번 서지 않는다. 이미 아래에 있으면 그리로 되돌아간다.
        // react-navigation 7 부터 `navigate` 는 되돌아가지 않고 한 단 더 쌓으므로(그 몫이
        // `popTo` 로 갈라졌다) 갈래를 여기서 정한다.
        //
