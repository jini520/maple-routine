 ...
 *
 * 다크는 원색이 판보다 어두워 활성이 비활성보다 흐려진다(레테에서 활성 원색 L0.60 대
 ...
 * 재질이 배경을 굴절시키며 채도까지 함께 빼 준다. iOS 에서 (246,245,245) 가 바 (254,243,249)
 * 옆에서 무채색 렌즈로 읽히는 이유다.
 *
 ...
 * 유리는 tint 로 내린 만큼을 재질의 하이라이트로 도로 올린다. 그래서 상수는 그 합을 재서
 * 정한다. 0.98 에서 엔젤릭버스터가 `#F5F5F5` 로, iOS 실기기와 한 자리 안에서 만난다.
 *
