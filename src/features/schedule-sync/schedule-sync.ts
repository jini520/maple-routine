...
// 네 섹션(daily/weekly/weeklyBoss/monthlyBoss) 중 하나라도 stale 이면(리셋 이후 미접속) 과거
// 날짜 조회로 항목 단위 선채움을 시도한다. daily/weekly 는 character 범위 항목 유무로 판정한다.
...
// 이미 조회한 날짜를 다시 부를 가치가 있는가. 원장은 그 날짜에 각 섹션의 내용이 있었는지만
// 기억하므로(값은 기억하지 않는다), 지금 비어 있는 섹션 중 하나라도 그 날짜에 있었다면 값을
// 가져오러 다시 부른다. 하나도 없었다면 불러 봐야 같은 0건이라 건너뛴다. 보스 0건 캐릭터가
// 매번 13일을 소진하던 고리가 여기서 끊긴다.
...
// 조회가 실패해도 그 날짜만 건너뛰고 계속한다.
//
// 조회와 병합이 갈렸다. 날짜는 한꺼번에 나가고 접는 것은 그대로 날짜 순 폴드다. 순서가
// 결과를 정하는 것은 조회가 아니라 병합이라 `resolved` 중단을 포함해 아래 루프의 의미는
// 한 글자도 안 바뀐다.
//
// 그 갈림이 만드는 새 사실 하나. 폴드가 일찍 멈춰도 뒷 날짜 응답은 이미 손에 있다. 버리면
...
async function fillMissingSections(
  ...
  // 이미 조회한 날짜를 다시 부르지 않는다. 보스가 0건인 캐릭터(특수 월드·저레벨)는 과거 날짜도
  // 0건이라 이 루프가 13일을 다 쓰고도 해결하지 못하고, 상태가 안 변해 매 동기화 14회 호출이
  // 영구 반복된다.
...
// 스케줄 동기화가 실제로 도는 회차에 그 대상 캐릭터의 character/basic 을 함께 받아 캐시를
// 갱신한다. 이 갱신의 상시 경로가 피커 하나뿐이면 레벨·외형이 피커를 마지막으로 연 시점의
// 스냅샷으로 굳는다.
//
...
async function syncOneCharacter(
  ...
  // 첫 캐릭터를 프리플라이트로 먼저 호출해 401/403·429처럼 모든 캐릭터에 똑같이 적용되는 전역
  // 실패인지 확인한다. 전역 실패면 나머지는 API 를 더 호출하지 않고 캐시 폴백만 한다. 아니면
  // 나머지 캐릭터는 서로 기다리지 않고 병렬로 호출한다. 서비스 단계 키(초당 500건)라 병렬
  // 호출이 한도와 충돌하지 않는다. 병렬 구간에서 개별 캐릭터가 401/429 를 반환해도 이미 발사된
  // 형제 호출은 못 막으므로 그 캐릭터만 개별 폴백한다.
  //
  // ocids 로 지정된 캐릭터만 동기화한다. 계정의 전체 캐릭터를 대상으로 하면 추적 대상이 아닌
  // 캐릭터까지 호출해 로딩이 느려진다.
async function runSyncRound(
  ocids: string[],
  onProgress?: (completed: number, total: number) => void,
): Promise<CharacterScheduleSync[]> {
  // 여기서부터 실제 네트워크가 나간다. 성공이 아니라 시도를 표시하므로 실패해도 표시한다.
  // 오프라인에서 탭을 옮길 때마다 실패 호출이 반복되지 않게. 캐릭터별 신선도는 isSyncFresh 가
  // 따로 본다.
...
//
// 게이트는 이번 실행에서 시도함 AND 캐시가 10분 안 인데, 플래그는 호출이 시작될 때 서고
// 신선도는 호출이 끝나야 갱신된다. 그 사이에 다른 화면이 진입하면 시도함 = true · 신선함 =
// false 를 보고 같은 호출을 한 번 더 낸다. today 가 첫 화면이라 이 경로는 지배 경로다.
//
// 키는 회차 하나다. ocid 집합이 아니다. 스케줄러 셋과 today 가 같은 추적 목록을 보므로 집합이
// 늘 같고, 집합별 슬롯을 두면 목록이 조금 다른 조합에서 같은 캐릭터가 여전히 두 번 나간다.
//
// 다만 합류에는 자격이 있다. 진행 중인 회차가 요청 ocid 를 전부 덮어야 한다. 수동 모드 시드는
// 자기가 자기와 부딪쳐 그 창이 100% 였다. ocid 마다 회차를 동시에 내면 첫 호출이 슬롯을 잡고
// 나머지가 전부 거기 합류해, 추적 캐릭터 전원이 첫 캐릭터의 스케줄로 시드된다. 못 덮는 요청은
// 그 회차가 정산된 뒤에 자기 회차를 잇는다.
//
// 대가는 합류한 호출의 `onProgress` 가 안 불린다는 것이다. 진행률은 회차를 소유한 호출이 받는다.
let inFlightRound: { ocids: ReadonlySet<string>; promise: Promise<CharacterScheduleSync[]> } | null =
  null

// 호출부 여섯이 전부 결과를 내가 물어본 캐릭터들 로 취급해 map 하므로, 회차가 더 많이
// 물어봤더라도 초과분은 돌려주지 않는다. 순서는 회차 순서 그대로다. 표시 순서는 화면 셀렉터의 몫이다.
