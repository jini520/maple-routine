 ...
 *
 * 미조회 날짜는 한꺼번에 나간다. 하루씩 순서대로 물으면 그 직렬 구간이 그대로 캐릭터가 화면에
 * 뜨는 시간이 된다(최대 13 RTT). 날짜끼리는 서로를 모르고 각 날짜가 하는 일은 원장에 기록과
 * 완료를 봤는가 뿐이라, 순서를 없애도 답이 바뀌지 않는다. 바뀌는 것은 조기 종료가 아끼던
 * 호출뿐이고 그 대가는 서비스 단계 키의 초당 500건 예산 안에서 치른다.
 *
 * `todayState` 는 이미 오늘 응답을 손에 쥔 호출부(예열)가 넘긴다. 같은 호출을 두 번 하지
 * 않기 위해서다. 오늘 응답은 원장에 기록하지 않는다. 오늘은 아직 끝나지 않은 날이라 완료
...
export async function resolveCharacterEligibility(
  ...
  // `unavailable` 이 `completion` 을 이긴다. 위 `judgeFromLedger` 가 원장을 그 순서로 읽으므로,
  // 여기서 뒤집으면 같은 입력에 이번 회차와 다음 회차의 답이 갈린다. 003 은 그 ocid 는 어느
  // 날짜로도 조회 불가 라 실제로 섞일 일이 없고, 이 줄은 그 전제가 깨졌을 때의 안전망이다.
