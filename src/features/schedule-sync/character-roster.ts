...
// 조회 불가 항목은 레벨과 무관하게 **맨 뒤로** 보낸다. 고를 수 없는 후보가
...
// 캐시 우선 표시(Stale-While-Revalidate). 캐시가 있으면 즉시 그 값으로 첫
...
// 결정 1 (2026-07-29): 확인되지 않은 캐릭터는 목록에 넣지 않는다. 확인 경로는
// character-basic-cache/조회 원장 또는 character/basic 응답뿐이고, 그 값이 없는 character/list
// 응답으로 목록을 채우지 않는다.
//
// 가 그 결정의 **결정 2**(콜드 스타트에서는 ②·③의 중간 방출을 억제하고 완료 후 1회만)를
...
export async function getCharacterPickerRoster(
  ...
  await Promise.all(
    characters.map(async (character) => {
      ...
      try {
        // 캐시 쓰기까지 공유 경로 안이다. 온보딩 한 바퀴(프로브 → 예열 → 피커)가
        ...
        // 결정 4 +: 조회 불가(400 OPENAPI00003)는 **추적 중일 때만** 남긴다.
  ...
  // 조회가 모두 끝난 뒤의 최종 방출. 콜드 스타트에선 방출이 이것뿐이고,
