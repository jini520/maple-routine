/**
 * 과거 기간 백필. 어떤 (캐릭터, 기간)을 조회할지 정하고, 조회해서 기록으로 남기고, 직전 기간
 * 총액과 더 뒤로 갈 수 있는지를 판단한다.
 *
 * 조회 원장(같은 날짜 재조회 금지)이 여기서 읽고 쓰인다.
 */
...
// 성공하면 markPeriodChecked 를 호출해 다음 방문부터 재조회하지 않게 하고, 실패하면 호출하지
// 않아 다음 방문 때 재시도된다. 이미 기록된 보스는 건드리지 않고 기록이 없는 조합만 기본값으로
// 채운다. 실시간으로 쌓인 기록이 base 이고 백필은 빠진 것만 채우는 delta 다.
//
// 반환값은 이번 시도의 결과다. null 이면 확인 완료, notCollected 면 아직 집계 전, failed 면
// 그 외 실패다.
//
// 조회 불가(구간 밖) 대상은 여기 들어오지 않는다. 호출부가 걸러낸다. 이 함수가 그 대상을
// markPeriodChecked 로 굳히면 조회해서 0건을 봤다 와 조회 불가라 굳혔다 가 같은 기록이 되어
// confirmedEmpty 가 outOfRange 로 격하된다.
export async function backfillTarget(target: BackfillTarget, now: Date): Promise<PeriodQueryOutcome | null> {
  ...
  try {
    ...
    // `selectBossProfitBosses` 로 그룹(content_name)당 실제 처치 난이도만 골라야 한다. 그러지
    // 않으면 등록 난이도와 실제 처치 난이도가 다를 때 둘 다 완료로 잡혀 같은 보스를 두 번
    // 기록한다. 과거 기간 백필이므로 미완료 placeholder 는 기록 대상에서 제외한다.
    ...
    // 백필로 만드는 delta 행에도 월드를 박는다. 그 시점 캐시의 월드를 쓰는데, 리프 이전 주는
    // API 가 400 을 주므로 백필 자체가 불가능하고 백필로 채워지는 리프 이후 주는 현재 월드가
    // 정답이라 실질 부정확이 없다.
    ...
    //  - notCollected(00009): 실패가 아니라 아직 이다. 재시도 유도 문구를 띄우지 않는다.
    //  - periodOutOfRange(00004): 우리 계산상 조회 구간 안인데 API 가 거부한 것. 월드 리프
    //    이전·휴면 등 그 캐릭터·날짜에 고유한 사정이라 조회할 수 없다 가 맞다.
