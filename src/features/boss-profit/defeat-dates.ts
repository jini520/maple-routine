 ...
 * 보스를 며칟날 잡았는지 캐내는 모듈.
 *
 * `boss_profit_records` 의 `period_key` 는 주·달이라 날짜를 못 든다. 스케줄러 API 도 직접
 * 안 알려 준다. `date=D` 응답은 D 시점의 완료 현황이라 주간 보스가 완료로 보이면 D 또는 그
 * 이전이다.
 *
 * **답은 두 날의 차이에 있다.** D−1 이 미완료이고 D 가 완료이면 그 보스는 D 에 잡혔다. 그래서
 * 하는 일이 한 날짜 조회가 아니라 **그 기간의 날짜들을 훑어 뒤집힌 지점 찾기**다.
 *
 * 비용이 보스 수가 아니라 날짜 수다. 응답 하나가 그날 보스 전부를 담으므로 한 주를 훑으면 그 주의
 * 모든 보스가 한꺼번에 풀린다. 캐릭터당 주 7회가 상한이고 날짜끼리 서로를 몰라 한꺼번에 나간다.
 *
 ...
 * 가진 관측만으로 다 풀렸다 · 키가 없다. 정상 상태(전부 캐 놓은 뒤)가 그 둘째라, 화면을 오갈
 * 때마다 호출이 나가지 않는다.
 *
 * 키가 없어도 채울 수 있는 것은 채운다. 소거법과 리셋 당일은 조회가 필요 없다.
 */
export async function resolveDefeatDates(ocids: readonly string[], now: Date): Promise<number> {
  // 두 화면이 같은 순간에 부를 수 있다. 원장은 차례로 부를 때만 겹침을 막으므로, 동시에 도는
  // 것은 여기서 하나로 접는다. 아니면 같은 날짜가 두 번 나간다.
  if (inFlight !== null) {
    return inFlight
  }
