...
// 행 타입의 정의 위치는 rows.ts 지만 공개 경로는 그대로 둔다. 화면과 테스트가
...
// 이 스냅샷에서 슬라이스하기 위한 용도다("로컬 우선 캐싱").
...
// 시점이 달라 신뢰도가 낮다. world는 정렬에 참여하지 않는다.
...
//
// liveRows는 "이번 달을 보고 있을 때"만 채워진다(지난 달을 여는 loadPeriod 분기는 []를
// 넘긴다. 그 화면의 행은 기록이 원천이므로). 평소엔 진행 중인 주가 언제나 이번
...
async function buildWeeklySubtotalsForMonth(
  ...
  // 진행 중인 주의 금액을 liveRows에서 읽을 수 있는가(= 이번 달을 보고 있는가)..
  ...
  // 지난 주의 상태를 6상태로 판정하려면 **확인 기록**이 필요하다. 기록이 없는 주가
...
async function loadPeriod(
  ...
  // target별 조회는 서로 독립이라 병렬로 던진다. 직렬 await 이면 월간 탭에서
...
export const useBossProfitStore = create<BossProfitStore>()((rawSet, get) => {
  ...
  async refresh(ocids, options) {
    ...
    // `world` 컬럼을 새로 더했으므로 그전 기록에는 월드가 없다. 지금 아는 월드로
    ...
    // 미선택(null)은 '자동'으로 동작한다. 동작 기본값은 그대로다.
    ...
    // **재조회를 건너뛰는 진입**(skipSync)은 예외다. 이 단계가 곧 최종 화면이라
    // 여기서 기록하지 않으면 화면이 계산되지 않은 채로 선다(아래 autoRecordRows 호출).
    // 재조회 게이트의 판정값(syncedAt)도 이 단계에서 함께 모은다. 이 조회가
    ...
    const cachedByOcid = await Promise.all(
      ocids.map(async (ocid): Promise<CachedCharacterEntry> => {
        ...
        // 보여준다. selectBossProfitBosses가 그룹(같은 apiName)당 "실제로 처치한"
        ...
        // 써야 한다. 수동 모드는 사용자 멤버십을 병합해 표시한다.
    ...
    // 결정 1~3: 화면 진입 자동 재조회는 데이터가 신선하면 건너뛴다. 캐시가 없는 캐릭터는
    ...
    // 건너뛴 진입은 이 캐시 단계가 곧 최종 화면이다. 건너뛰는 것은 네트워크
    ...
    // 캐시 우선 표시(/017)가 계속 유지된다. 실시간 동기화가 성공하면 아래에서 다시
    // 최신 데이터로 덮어쓴다.
    latestSyncSnapshot = {
      ocids: [...ocids],
    ...
    // 제자리 새로고침은 캐시 우선 표시의 **화면 반영만** 건너뛴다. 이 단계가
    ...
    if (refreshInPlace) {
      ...
      // 건너뛰는 진입에는 이 값을 다시 채울 동기화 완료 단계가 없다. 행이 하나도 없어도
    ...
    // 레벨이 바뀌어 캐릭터 순서가 바뀌는 것은 의도된 결과다(정렬 규칙은 그대로).
    ...
    // 월간 행이 사라지면 그 달 기록을 찾지 못해 화면에서 금액이 사라졌다. 현재
    ...
    // 가능한 기간에서만 노출되므로(#30) 완전히 닫힌 과거 기간을 보는 동안 이 set이
    // 일어나도 화면에는 영향이 없다.
    ...
    // 동기화·자동 기록은 위에서 다 끝났다. 보던 기간(지난 달)의 화면 반영은
    // loadPeriod에 넘긴다. 그 함수가 이미 "과거 기간은 기록이 원천"을 알고 있어
