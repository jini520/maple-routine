// 완료 행의 **드롭 이관과 자동 기록**(로 store.ts 에서 분리).
//
// 왜 분리하나 — 이 루프는 지금까지 `refresh` 의 **동기화 완료 분기 안에만** 있어서 "동기화를
// 건너뛴다"가 곧 "기록을 건너뛴다"였다. 그 결합은 설계된 것이 아니라 코드
...
/**
 * /: 기록이 없는 완료 보스는 화면 진입 전에도 즉시 기본 파티원 수로 자동 기록한다.
 ...
 * 아래 루프에서 완료 행의 드롭 이관에 쓴다(자동 기록과 같은 순회를 쓴다).
 *
...
export async function autoRecordRows({
  ...
  for (const row of rows) {
    ...
    // 미완료 placeholder는 절대 자동 기록하지 않는다. 여기서 기록해버리면
