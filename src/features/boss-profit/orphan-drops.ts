/**
 * 잡지 않은 보스에 남은 드롭을 정리하는 순수 함수. 규칙은 하나다.
 *
 * > 설 자리도 처치 기록도 없는 드롭 그룹은 지운다.
 *
 * 고아가 생기는 경로가 셋인데(주간 한도를 채워 행이 사라짐 · 수동 추적에서 뺌 · 주가 바뀌어 영영
 * 미처치로 굳음) 셋이 같은 모양이라 술어 하나로 덮는다. 경로마다 정리 코드를 쓰면 넷째 경로가 생길
 * 때 또 빠진다.
 *
 * 안 지우면 화면마다 다르게 보인다. 보스 수익에서는 사라지고, 가계부에는 안 잡히고, 드롭 히스토리와
 * today 위젯에는 남는다. 판매가가 같은 행에 붙어 있어 금액까지 그 상태가 된다.
 *
 * ⚠️ **지우는 조건은 미처치이지 추적 해제가 아니다.** 실제로 잡은 보스는 추적을 꺼도 화면에 계속
