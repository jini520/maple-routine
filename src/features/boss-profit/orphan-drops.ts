// **잡지 않은 보스에 남은 드롭 정리** ([[ADR-187]] 결정 5·6).
//
// 미완료 placeholder 행에도 드롭을 적을 수 있다([[ADR-032]] 배경 3 — 처치 직후 `complete_flag` 가
...
// 경로가 생길 때 또 빠진다([[ADR-055]] 결정 1 이 «선택 불가» 를 사유 하나로 모델링한 것과 같다).
//
...
//
// [[ADR-124]] 로 판매가가 같은 행에 붙어 있어 **금액까지** 그 상태가 된다.
//
...
export interface OrphanDropPlanInput {
   ...
   * ([[ADR-067]] 결정 7 이 자동 기록에 쓰는 술어와 같다).
   */
