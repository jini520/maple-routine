...
// 이벤트다. 카드가 아예 없으면 문구가 사라진 자리에 빈 칸이 남으므로 화면이 `ErrorState` 를
// 그린다.
//
// 문구는 그 `ErrorState` 의 제목과 같다. 같은 실패의 두 얼굴이 다른 말을 하면 안 된다.
//
// 중복 방지가 스케줄러 훅과 다른 것은 여기서 오는 신호가 문자열 상태(`periodState`)이기
// 때문이다. 그래서 기간 키를 가드로 쓰되 로딩이 시작되면 비운다. 그러지 않으면 다시 시도를
// 눌러 같은 기간이 또 실패했을 때 아무 반응이 없다.
