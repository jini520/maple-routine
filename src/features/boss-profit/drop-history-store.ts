...
// 드롭 획득 히스토리(전 기간) 상태(이슈 #54). **읽기 전용이다**. 삭제·수정은 드롭 입력
...
// 빈 배열로 바꾸면 "기록이 없습니다"라는 **거짓 빈 상태**가 된다. 실패는 실패로 알린다.
// 그래서 던지는 쪽인 `withSqliteTimeout` 을 쓴다. 이 파일에 있던 **같은 코드의 사본**을 걷었다
// (: 그 사본은 경주가 끝나도 타이머를 안 꺼서 조회마다 5초짜리 타이머를 남겼다).

// 저장 계층은 부재를 null 로, `RecordedDrop` 은 optional(undefined)로 표현한다. 여기서 한 번
// 정규화한다(`loadDropsByRowKey` 가 하는 것과 같은 변환). `recordedAt` 은 옮기지 않는다(
