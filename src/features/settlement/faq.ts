 ...
 *
 * `00:00 ~ 04:00` 은 **설명이지 판정이 아니다.** 앱은 이 시각으로 결산을 가르지 않는다 - 그 판정은
 * 서버가 밤마다 조회해서 한다. 안내 줄이 시각을 안 말하는 것과 여기가 말하는 것은 자리가 달라서다.
 * 줄은 상태를 알리고 시트는 설명한다.
 *
 * **끝 시각을 실측보다 넓게 적는다.** 서버 로그 여섯 밤은 02:00 에 끝났지만 그 표본에 매월 1일
 * 새벽이 없고, 좁게 적으면 02:10 에 아직 안 되는 사용자가 앱이 틀렸다고 읽는다. 넓게 적으면 일찍
 * 끝났다고 읽고 만다.
 */
