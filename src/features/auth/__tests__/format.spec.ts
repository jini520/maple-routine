...
describe('formatAuthError', () => {
  it('invalidApiKey를 사람이 읽을 문구로 바꾼다', () => {
    expect(formatAuthError({ kind: 'invalidApiKey' })).toBe('API 키가 유효하지 않습니다')
  })

  // 429는 원인만 말한다. 이 함수의 반환값은 전부 토스트 본문이라
  ...
  it('rateLimited를 사람이 읽을 문구로 바꾼다 (처방 없이 원인만)', () => {
    expect(formatAuthError({ kind: 'rateLimited' })).toBe('호출 한도를 초과했습니다')
  })

  it('network를 사람이 읽을 문구로 바꾼다', () => {
    expect(formatAuthError({ kind: 'network' })).toBe('네트워크 오류가 발생했습니다')
  })

  it('storageWriteFailed를 사람이 읽을 문구로 바꾼다', () => {
    expect(formatAuthError({ kind: 'storageWriteFailed' })).toBe(
