/**
 * 가계부의 오케스트레이션. 화면과 저장소 사이.
 *
 * 화면은 `storage/` 를 직접 안 부른다. 여기서 하는 일은 셋이다.
 *
 * ① 행의 신원을 만든다. `id` 와 `recordedAt`. 어댑터가 만들면 같은 입력에 같은 SQL 이 깨져
 *    테스트가 값을 못 박는다. 그래서 불순한 자리를 여기 하나로 모은다. `Math.random()` 과
 *    `new Date()` 는 이 파일에서만 부른다.
 * ② 시세를 기억한다. 메포 지출을 저장할 때 그 시세를 다음 입력의 기본값으로 남긴다.
 * ③ 여러 원천을 하루로 접는다. 캘린더 칸이 받는 `CalendarAmounts` 를 만든다. 원천이 넷이고
 *    (손입력 둘 · 보스 결정석 · 아이템 판매) 뒤의 둘은 여기서 못 고친다.
 */
...
/**
 * 행 하나의 신원. 자연키가 없어서 필요하다. 손입력은 같은 날 같은 것을 두 번이 정상이다.
 *
 * `crypto.randomUUID` 를 안 쓰는 것은 Hermes 에 없기 때문이다. 충돌만 안 나면 되는 로컬 키라
 * 시각 + 난수로 충분하다.
 */
 ...
 * 캐시가 낀 날은 이 값이 그날의 전부가 아니다. 그 사실은 고른 날의 상세가 따로 말한다.
 *
 * 수입과 지출이 같은 식이다. 그래야 수입 − 지출 이 한 자로 재인다.
 */
 ...
 *
 * 보스마다 한 줄이 아니라 이 요약이 한 줄이 된다. 한 줄이 한 기록 규칙에서 보스만 빠지는 것은
 * 그 규칙의 근거가 접으면 어느 쪽을 고치는지 못 고른다 였는데 보스 줄은 여기서 못 고치기
 * 때문이다. 접지 않으면 목요일 한 칸이 캐릭터 수 × 12줄이 된다.
 */
 ...
 *
 * 던지지 않는다. 날짜를 못 캔 것은 칸이 덜 채워진다 이지 화면이 죽을 일이 아니다.
 */
 ...
 *
 * 이름을 모르는 캐릭터는 안 든다. `ocid` 는 사용자에게 아무 뜻도 없는 문자열이고 알 수 없음 은
 * 있지도 않은 캐릭터를 목록에 만든다. 캐시는 캐릭터를 한 번이라도 연 뒤에 찬다.
 *
  ...
  const named = await Promise.all(
    ocids.map(async (ocid) => {
      ...
      return {
        ocid,
        name: profile?.name ?? '',
        // 레벨은 사냥 계산기가 쓴다. 지역을 ±20 으로 거르고 레벨 차이 페널티를 낸다. 캐시가 아직
        // 안 따뜻하면 `null` 이고 그때는 페널티 없이 계산하며 시트가 그 사실을 한 줄로 말한다.
 ...
 *
 * 수입과 지출은 테이블이 갈려 있어 한 목록에 세우려면 어느 쪽인지를 들고 다녀야 한다. 그
 * 표식이 `kind` 다.
 */
interface ManualDayRecordBase {
  /**
   * 그 기록에 붙은 캐릭터의 이름. 없으면 빈 문자열이다.
   *
...
/**
 * 결정석 줄. 누르면 그 자리에서 펼쳐진다.
 *
 * `bosses` 가 이 갈래에만 있는 것이 곧 판매 줄은 못 펼친다 다. 화면이 조건을 잘못 쓰면
 * 컴파일 단계에서 걸린다.
 */
export interface BossCrystalDayRecord extends AutoDayRecordBase {
  kind: 'bossCrystal'
  /** 그날 잡은 보스. 비어 있지 않다. 이 줄이 서는 조건이다. */
 ...
 * 손입력이고, 그것이 곧 여기서 고칠 수 있는가 의 답이다.
 */
...
/**
 * 줄에 적는 이름. 손입력은 항목이고 없으면 갈래 이름이다.
 *
 * 직접 입력에서 이름 칸을 비우는 것은 정상이라 이름 없는 기록이 실제로 생긴다. 그 줄을 비워
 * 두면 무엇인지 모르는 줄이 되는데 갈래는 언제나 있으므로 그것을 쓴다.
 *
 * 자동 줄은 캐릭터 · 갈래다. 이름을 모르면 갈래만 적는다. `ocid` 를 적으면 있지도 않은
 * 캐릭터를 만들어 낸다.
 */
/**
...
/**
 * 그날 상세의 합계 두 줄. 그날 줄들에서 바로 낸다.
 *
 * 칸 금액 표(`loadCalendarAmounts`)를 안 본다. 그 표는 지금 격자가 덮는 범위 것이고 고른 날은
 * 기간을 옮겨도 안 바뀌므로, 표를 보면 그 날이 범위 밖으로 나가는 순간 상세가 통째로 사라진다.
 * 그 날 기록이 없다 와 그 날이 범위 밖이다 가 같은 `undefined` 로 말해진다.
 *
 * 두 길은 같은 수를 낸다. 수입 = 손입력 + 결정석 + 판매, 지출 = `spendMesoOf`. 그 등식을
 * `records.spec` 이 붙들고 있다. 깨지면 칸에 적힌 수와 그 칸을 눌러 나온 수가 갈린다.
 *
 * 캐시로 낸 지출은 0 을 더한다. 환산을 안 하므로 메소 축에 얹을 값이 없고, 그 사실은 그 줄이
 * 원 으로 적히는 것으로 말한다.
 */
