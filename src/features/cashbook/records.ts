 ...
 *    하나로 모은다**. `Math.random()` 과 `new Date()` 는 이 파일에서만 부른다.
...
interface BossDaySummary {
   ...
   *
   * **마리당 금액을 안 든다**. 큰 것부터 정렬에만 쓰던 값인데 순서가 정규 순서로
 ...
 * 사냥 타이머 자동 수익은 폐기됐다(⛔, 2026-08-30).
 *
 * **날짜를 모르는 보스 기록은 안 든다**. 어느 칸에 얹으면 그것이 거짓 날짜가
...
/**
 * 가계부가 트는 **처치 날짜 캐기**. 캔 건수를 돌려준다. 0 이면 다시 읽을
 * 이유가 없다.
 *
 ...
 *
 * **이름을 모르는 캐릭터는 안 든다.** `ocid` 는 사용자에게 아무 뜻도 없는 문자열이고(
  ...
  const named = await Promise.all(
    ocids.map(async (ocid) => {
      ...
      return {
        ocid,
        name: profile?.name ?? '',
        // **레벨은 사냥 계산기가 쓴다**. 지역을 ±20 으로 거르고 레벨 차이
 ...
 *
 * 보스 수익 탭의 당김과 **같은 재조회**를 부른다. 두 하위 탭이 같은
 * 원천을 보므로 어느 탭에서 당겼나 로 결과가 달라지면 안 된다.
 *
 * **던지지 않는다.** 실패를 말하는 것은 그 스토어의 `error` 와 토스트다. 여기서
 ...
 * ). 가계부의 모든 조회가 이 파일을 지나므로 리비전만 예외로 두면 그 벽에
 ...
 *
 * 수입과 지출은 테이블이 갈려 있어 한 목록에 세우려면 어느 쪽인지를 들고
 * 다녀야 한다. 그 표식이 `kind` 다.
 */
...
/**
 * 그날 목록의 한 줄. **갈리는 기준은 테이블**이다. `income_records`·
...
export async function loadDayRecords(dateKey: string): Promise<DayRecord[]> {
  ...
  /**
   * 이름을 **한 번에** 찾는다. 손입력 줄과 보스 줄이 같은 캐릭터를 가리킬 수 있다(
 ...
 * 이름을 안 적는다. 캐시는 캐릭터를 한 번이라도 연 뒤에 찬다.
 */
  ...
  for (const summary of summaries) {
    ...
    if (summary.bossCount > 0) {
      rows.push({
        ...
        /**
         * **`weekly-bosses.json` 정규 순서**다. 앱에서 보스 무리가 서는 네 자리가
         ...
         * 그 SELECT 에는 `ORDER BY` 가 없고 앞의 주석은
     ...
     *
     * 종전에는 미입력만 있어도 세웠다. 가 가격 미입력이 정상 이라 정했으니 그 줄이
     ...
     * 있다**는 표시이고, 그 줄은 이미 판 것이 있어서 선 줄이다.
     */
...
export function recordTitleOf(entry: DayRecord): string {
  ...
  // **캐릭터가 붙어 있으면 이름이 앞에 선다**. 보스 줄이 이미 쓰던 어법
 ...
 * 사라진다. 그 날 기록이 없다와 그 날이 범위 밖이다가 같은 `undefined` 로 말해진다
 * (사용자 보고 2026-08-26: 8월 25일을 고른 채 7월로 옮기면 기록이 없어요가 떴다).
 *
 * **두 길은 같은 수를 낸다**. 수입 = 손입력 + 결정석 + 판매, 지출 = `spendMesoOf`. 그 등식을
 * `records.spec` 이 붙들고 있다(깨지면 칸에 적힌 수와 그 칸을 눌러 나온 수가 갈린다).
 *
 * 캐시로 낸 지출은 **0 을 더한다**. 환산을 안 하므로 메소 축에 얹을 값이 없다.
 ...
 *
 * 수입도 같다. 이벤트 보상이 캐시로 들어오는 자리가 있고, 환산을 안 하는
 * 것도 지출과 같은 이유다.
 */
...
export function recordCountLabelOf(entry: DayRecord): string | null {
  ...
  /**
   * 사냥은 **몇 재획을 돌았나**다(사용자 지정 2026-08-29). 보스 줄의 n마리와 **같은 자리·같은
   * 모양**이라 화면은 아무것도 안 가른다(그 칸이 하나뿐이다).
   *
   * **계산기로 적힌 행만** 그 수를 안다. 수동으로 적은 행에는 소재 줄이 없고,
   *  이전 행은 계산 입력 자체가 없다. 둘 다 칸이 안 선다.
   */
