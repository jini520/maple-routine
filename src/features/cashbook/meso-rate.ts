 ...
 *
 * 화면은 `nexon/` 도 `storage/` 도 직접 안 부른다. 여기서 하는 일은 넷이다. 키를 꺼내고 ·
 * 직업을 캐시에서 읽고 · 여섯을 부르고 · 성공하면 캐시에 남긴다.
 *
...
/**
 * 섀도어의 그리드를 세려면 직업 이름이 필요하다. 그 값은 `character/list` 가 캐시에 남겨 둔
 * 것이라 호출이 안 는다.
 *
 * 못 읽으면 `null` 이고 그 몫은 0 이 된다. 여기서 던지지 않는다. 직업을 몰라서 메획 전체를
 * 손입력으로 내리면 잃는 것이 더 크다.
 */
