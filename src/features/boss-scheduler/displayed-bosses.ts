 ...
 * 바꿔 달고, today 「남은 스케줄」은 이 값으로 «남은 것» 에서 뺀다(의 등식).
 */
...
/**
 * **캐릭터를 인자로 받는다** — 선택된 캐릭터의 카드 목록과 초상화 레일의 링이
 * **같은 함수**를 써야 «링이 세는 것 = 화면에 보이는 것» 이 구조로 보장된다. 레일은 선택되지 않은
 * 캐릭터도 세므로 «지금 선택된 캐릭터» 를 안에서 집을 수 없다.
 *
 * **순서는 `weekly-bosses.json` 정규 순서다** — 아래 `orderByReference` 가 **모드를 안
...
export function displayedBosses(
  ...
  // 한도는 **캐릭터의 주간 전체**로 판정한다 — 추적 목록이 아니라 동기화 결과다(결정 1
 ...
 * 이고, 이미 잡은 보스도 `false` 다(마감은 완료를 대체하지
 * 않는다).
 *
 ...
 * 네 벌이 된다.
 *
 * **완료는 여전히 자리를 안 바꾼다** — 이 함수가 보는 것은 이름과 난이도뿐이다.
 */
 ...
 * (— 무리가 비면 헤더도 함께 사라진다). 여기서 미리 걷어도 화면이 필터 후
 * 다시 걷어야 하므로 판정이 두 곳이 된다.
 *
 * **완료는 자리를 안 바꾼다** — 완료 여부로 가르는 정렬이 없는 것이 그 결정의
 * 모습이다. 무리 **안**의 순서는 두 모드 모두 `weekly-bosses.json` 정규 순서이고, 그것을 세우는
 * 자리는 `displayedBosses` 다.
 *
 * (2026-08-30 정정: 이 자리에 «자동 모드가 참조표 순서, 수동 모드가 추적 순서» 라고 적혀 있었는데
 * **정확히 반대**였다 — 정렬하는 쪽이 수동뿐이었다. 이 자동 모드도 같은 순서로 세우면서
