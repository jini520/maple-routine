...
export const RELEASE_NOTES: ReleaseNote[] = [
  {
    version: '1.0.8',
    // `chore(release): 1.0.8` 커밋 날짜다.
    date: '2026-09-01',
    // **한 줄이다**(사용자가 정한 문장, 2026-09-01). 이 릴리스는 핫픽스라 사용자가 겪은 문제가
    // 하나뿐이고, 시트 줄 배치는 그 문제를 고치면서 함께 손본 것이라 핵심 목록에 안 올린다
    // ([[ADR-126]] 결정 3 — 핵심은 데이터가 아니라 판단이다).
    highlights: ["사냥 수익 '챌린저스', 섀도어 '그리드' 스킬 미적용 수정"],
    // 메소 획득량이 낮게 잡히던 버그가 이 릴리스의 이유다. 시트 줄 배치는 같은 화면을 고치면서
    // 함께 바꾼 것이고 **1.0.7 에서 출시된 모양이 바뀌는 것**이라 적는다(이번 릴리스 안에서
    // 생겼다가 사라진 회귀가 아니다 — 1.0.7 이 세운 구분 그대로다).
    items: [
      {
        category: 'fix',
