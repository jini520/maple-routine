...
// 릴리스 노트의 진실 원천 한 벌. 개발 노트 화면과 `latest.json` 의 `notes` 가 여기서 갈라져
// 나간다. 노트를 두 곳에 쓰면 언젠가 반드시 갈라지고, 갈라진 순간 어느 쪽이 사실인지 알 방법이
// 없다.
//
// 최신이 먼저 오도록 버전 내림차순으로 손으로 유지한다. 화면은 이 배열을 그대로 그리고
// 정렬하지 않는다. 런타임 정렬은 잘못 쓴 파일을 조용히 통과시키므로 순서·중복·형식은 테스트가
// 강제한다(`__tests__/release-notes.test.ts`).
//
// 과거 버전의 근거는 GitHub 마일스톤이다. 그 버전 마일스톤에 닫힌 이슈가 저장소에 남아 있는
// 기록이고 그것을 사용자용 문장으로 옮긴 것이다. 날짜는 `chore(release)` 커밋 날짜다.
// - `1.0.1` 은 마일스톤이 없어 건너뛴다. 근거가 없는 버전을 지어내지 않는다. 그래서 목록에
//   1.0.2 다음이 1.0.0 인 빈틈이 있고 그것이 정상이다.
// - `1.0.3` 이후로는 배포 스크립트가 노트 없는 배포를 막으므로 빈 버전이 생길 수 없다.
//
...
export const RELEASE_NOTES: ReleaseNote[] = [
  {
    ...
    date: '2026-08-30',
    // 다섯 줄이다. `버그 수정.` 은 `items` 의 `fix` 를 한 줄로 뭉친 것이라 어떤 파생 규칙으로도
    // 안 나온다. `highlights` 를 `items` 에서 파생하지 않는 이유가 정확히 이런 줄이다.
    //
    // 첫 줄이 맨 위에 선다. 1.0.7 을 이미 받은 사람에게 이 번들이 다시 뜨고, 그때 새로 보는
    // 것이 그 한 줄뿐이다. 아래로 밀면 무엇이 달라졌는지 찾아 읽어야 한다.
    highlights: [
      "사냥 수익 '챌린저스', 섀도어 '그리드' 스킬 미적용 수정.",
      "'가계부' 기능 추가.",
      "'아이템 분배금 계산기' 기능 추가.",
      'today 페이지 디자인 개선.',
      '버그 수정.',
    ],
    // 근거는 v1.0.7 마일스톤의 닫힌 이슈 12건과 1.0.6 이후 머지된 PR #237~#277 이고, 그중
    // **화면에서 겪는 변화**만 사용자가 직접 골랐다(2026-08-30). 그래서 노트가 PR 수보다 훨씬
    // 짧다. 스물아홉 개 PR 의 대부분은 가계부 하나를 세우는 조각들이거나(#260~#276) 사용자가
    // 안 겪는 정리다(캐패시터 제거 · 테스트 러너 · ADR 배지).
    //
    // **이번 릴리스 안에서 생겼다가 같은 범위에서 고쳐진 회귀는 `fix` 로 적지 않는다**. 가계부
    // 시트의 키보드·한글 조합(#264) · `기타`를 메포/캐시로 저장(#267) · 다시 들어오면 안 읽던
    // 것(#276) · 공유 컨텐츠 체크박스 색(#277). 출시된 앱에서 아무도 겪은 적이 없어서, 적으면
    // 없던 문제를 있었다고 말하게 된다(1.0.6 이 세운 원칙 그대로다).
    //
    // **`requiresStoreUpdate` 가 하나도 없다.** 1.0.6 이후 늘어난 의존성은 `@gorhom/portal` 과
    // `zustand` 둘뿐이고 둘 다 네이티브 코드가 없는 순수 JS 라, 이 릴리스는 전부 OTA 로 간다
    // (사용자 결정, 2026-08-30).
    items: [
      {
        category: 'feature',
        text: '가계부 기능 추가. 수익 및 지출 내역을 기록할 수 있습니다.',
      },
      {
        category: 'feature',
        text: "'아이템 분배금 계산기' 유틸리티 기능 추가.",
      },
      {
        category: 'improvement',
        text: '보스 수익이 주단위 → 일단위로 기록되도록 수정.',
      },
      {
        category: 'improvement',
        text: 'today 페이지의 일부 위젯 디자인 개선.',
      },
      {
        category: 'fix',
        text: '페이지 이동 시 새로고침 인디케이터 영역이 줄어들지 않는 문제 수정.',
      },
      {
        category: 'fix',
        text: 'today 페이지의 글자 잘림 문제 수정.',
      },
      // 아래 둘은 1.0.7 이 출시된 뒤에 고친 것이고 같은 버전 번호로 다시 나간다. 위 주석의
      // 같은 범위에서 고쳐진 회귀는 안 적는다 에 걸리지 않는다. 그 규칙은 아무도 겪은 적 없는
      // 회귀의 이야기이고, 이 둘은 출시된 앱에서 실제로 겪었다.
      {
        category: 'fix',
        text: '사냥 수익 계산에 챌린저스 월드 버프와 섀도어 그리드 스킬의 메소 획득량이 반영되지 않던 문제 수정.',
      },
      {
        category: 'improvement',
        text: '수입 추가 사냥 시트의 지역·사냥터·소비 아이템 줄 배치 개선.',
      },
    ],
  },
  {
    ...
    date: '2026-08-19',
    // 네 줄이다. 화면 뼈대·캐릭터 선택·게임 데이터·그 아래 구조가 한 버전에 함께 바뀌어 뭉칠
    // 축이 넷이다. 마지막 줄은 `items` 첫 줄과 글자까지 같지만 파생이 아니다. 나머지 셋이
    // 무엇이 생겼나 인 자리에서 그 줄만 왜 이 릴리스인가 다.
    highlights: [
    ...
    // 아무도 겪은 적이 없어 **버그 수정으로 적지 않는다**. 없던 문제를 있었다고 말하게
  ...
  },
  {
    version: '1.0.5',
    date: '2026-08-12',
    // 긴급 수정 한 건뿐이라 핵심 목록도 한 줄이다("3~4줄"은 상한이지 채워야 할
  ...
  {
    version: '1.0.4',
    // 이 노트는 마일스톤 v1.0.4 의 닫힌 이슈를 근거로 릴리스 전에 미리 썼고, 배포하며 `date`
    // 를 작성일에서 `chore(release)` 커밋 날짜로 정정했다.
    date: '2026-08-11',
    // 업데이트 모달이 **받기 전에** 보여줄 핵심 목록(사용자가 직접 정한 3줄).
    ...
    items: [
      ...
      },
      {
        ...
        // 같은 안내의 **다른 마디**를 가리킨다. 위 `판매 가격` 항목은
      ...
      },
      {
        ...
        // 은 "통계 기능이 생기면 그쪽으로 옮긴다"까지만 정했고, 노트에 적는 순간
        // 그것은 계획이 아니라 약속이 된다.
        category: 'improvement',
      ...
      },
      {
        ...
        // 하나다. 안내 첫머리에 떨어뜨리면 그 마디를 다시 찾아야 한다.
        guideSectionId: 'card',
      },
      {
        category: 'improvement',
      ...
      {
        // 위 항목과 가른 이유가 그 자체다. 스와이프 백은 OTA 로 가지만 이 항목은
