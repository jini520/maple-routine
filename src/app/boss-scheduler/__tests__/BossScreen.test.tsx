...
// ADR-055 결정 9(이슈 #32): 표시 화면은 표시만 바꾼다 — 잠긴 항목을 추적 목록에서 자동으로 빼지 않는다.
describe('BossScreen — 요구 레벨 미달 표시 (ADR-055)', () => {
  function trackedBoss(boss: string, difficulty: string): MatchedBoss {
    return {
      apiName: boss,
      difficulty: difficulty as MatchedBoss['difficulty'],
      cycle: 'weekly',
      isRegistered: false,
      isComplete: false,
      ownComplete: false,
      matchedBossName: boss,
      portraitSlug: null,
      isSeasonBoss: false,
    }
  }

  it('레벨이 모자란 보스 카드에 "진행 불가"를 표시하되 목록에서 빼지 않는다', async () => {
    useTrackingModeStore.setState({ mode: 'manual' })
    mockStore({
      status: 'loaded',
      trackedOcids: ['ocid-1'],
      selectedOcid: 'ocid-1',
      // 유피테르는 노멀 295 요구 — 레벨 200 캐릭터는 진행할 수 없다.
      manualTrackedByOcid: { 'ocid-1': [{ contentName: '유피테르', kind: 'boss', difficulty: '노멀' }] },
      characters: [character({ ocid: 'ocid-1', level: 200, weeklyBosses: [trackedBoss('유피테르', '노멀')] })],
    })

    renderBossScreen()
    await screen.findByRole('combobox')

    expect(screen.getByText('유피테르')).toBeInTheDocument()
    expect(screen.getByText('진행 불가')).toBeInTheDocument()
  })

  it('레벨을 충족하면 "진행 불가"를 표시하지 않는다', async () => {
    useTrackingModeStore.setState({ mode: 'manual' })
    mockStore({
      status: 'loaded',
      trackedOcids: ['ocid-1'],
      selectedOcid: 'ocid-1',
      manualTrackedByOcid: { 'ocid-1': [{ contentName: '자쿰', kind: 'boss', difficulty: '카오스' }] },
      characters: [character({ ocid: 'ocid-1', level: 200, weeklyBosses: [trackedBoss('자쿰', '카오스')] })],
    })

    renderBossScreen()
    await screen.findByRole('combobox')

    expect(screen.queryByText('진행 불가')).not.toBeInTheDocument()
  })

  it('캐릭터 레벨을 모르면(캐시 미스) "진행 불가"를 표시하지 않는다 — 결정 5', async () => {
    useTrackingModeStore.setState({ mode: 'manual' })
    mockStore({
      status: 'loaded',
      trackedOcids: ['ocid-1'],
      selectedOcid: 'ocid-1',
      manualTrackedByOcid: { 'ocid-1': [{ contentName: '유피테르', kind: 'boss', difficulty: '노멀' }] },
      characters: [character({ ocid: 'ocid-1', level: null, weeklyBosses: [trackedBoss('유피테르', '노멀')] })],
    })
