 ...
 *
 * 상태는 `features/onboarding/store` 에 있다. 이 파일은 `status` 를 화면에 매핑하기만 한다.
 *
 * 단계 셸(`OnboardingStep`)은 옆 파일이고 **캐릭터 선택 단계만 스스로 두른다**. 그 단계는 CTA 를
 * 고정 바로 넘겨야 하는데 바의 활성 조건이 `useCharacterManage` 안에 있어, 그 훅을 부르는
 * 컴포넌트가 스크롤과 바 둘 다의 조상이어야 한다.
 *
