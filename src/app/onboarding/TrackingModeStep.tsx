 ...
 *
 * 카드 박스 없이 배경 위에 바로 놓이는 페이지 레이아웃(`ApiKeyForm` 과 같다)을 따르고, 바깥
 * 선택 카드 클래스는 설정 `ThemeSelector` 와 공유한다. 카드 안쪽(아이콘·설명·주의 박스)은 설정
 * `TrackingModeSelector` 와 같은 모양이어야 한다. 공용 카피 `features/tracking-mode/copy` 를
 * 함께 쓰므로 한쪽만 고치면 같은 선택지가 두 화면에서 다르게 생긴다.
 *
 * 설명·주의는 접지 않고 항상 보여준다. 고르기 전에 둘을 비교하는 화면이다. 기본 선택은 없으며
 * 사용자가 직접 고르기 전까지 계속하기 를 비활성화한다.
 */
