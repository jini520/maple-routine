...
describe('ContentScreen — 요구 레벨 미달 표시 (ADR-055)', () => {
  const GEARDRAK = '[일일 퀘스트] 기어드락 크로노스의 잔재 수집'

  it('레벨이 모자란 항목 카드에 "진행 불가"를 표시하되 목록에서 빼지 않는다', async () => {
    useTrackingModeStore.setState({ mode: 'manual' })
    mockStore({
      status: 'loaded',
      trackedOcids: ['ocid-1'],
      selectedOcid: 'ocid-1',
      // 기어드락 일일퀘는 295 요구 — 레벨 200 캐릭터는 진행할 수 없다.
      manualTrackedByOcid: { 'ocid-1': [{ contentName: GEARDRAK, kind: 'daily' }] },
      characters: [character({ ocid: 'ocid-1', level: 200, dailyContents: [] })],
    })
  ...
  it('레벨을 충족하면 "진행 불가"를 표시하지 않는다', async () => {
    ...
    mockStore({
      ...
      selectedOcid: 'ocid-1',
      manualTrackedByOcid: { 'ocid-1': [{ contentName: GEARDRAK, kind: 'daily' }] },
      characters: [character({ ocid: 'ocid-1', level: 300, dailyContents: [] })],
    })
  ...
  it('캐릭터 레벨을 모르면(캐시 미스) "진행 불가"를 표시하지 않는다 — 결정 5', async () => {
    ...
    mockStore({
      ...
      selectedOcid: 'ocid-1',
      manualTrackedByOcid: { 'ocid-1': [{ contentName: GEARDRAK, kind: 'daily' }] },
      characters: [character({ ocid: 'ocid-1', level: null, dailyContents: [] })],
    })
