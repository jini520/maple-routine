...
// ══ RN 으로 옮기며 갈린 것 다섯 ═══════════════════════════════════════════════════
//
// ① **셸이 통째로 바뀐다.** 웹은 `StackScreen scroll={false}` 안에 **자기 스크롤 상자**를 두고 그
//    안에 `sticky` 헤더를 넣었다 — 공용 `ScreenScroll` 의 `-mt-[var(--sa-top)]` 보정이 흐름 밖
//    `fixed` 헤더를 전제해 sticky 헤더와 겹쳤기 때문이다(실기기 계측: 31px). RN 에는 그 보정 자체가
//    없고 헤더는 **스크롤 뷰의 형제**라(`PageHeader` 파일 머리) 같은 그림이 공용 셸로 곧장 나온다.
