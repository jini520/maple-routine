 ...
 * ══ 못 옮긴 것 둘. 먼저 적는다 ═════════════════════════════════════════════════════
 *
 * ① **중첩 sticky**와 그에 딸린 셋(배지 sticky 레일 · stuck 헤더 하단 페이드 ·
 ...
 * | 073 | 목록을 `transform` 으로 내리고 인디케이터를 얹었다 | `RefreshControl` |
...
// `DeltaChip` 은 증감 표시를 통계 기능으로 옮길 때까지 쓰이지 않는다(2026-08-10)
    ...
    <View testID="page-header" className="z-10 px-4 pb-2" style={{ paddingTop: topSafeAreaPx }}>
      ...
      <View className="gap-4">
        {/* 히스토리 진입점은 탭 줄이 아니라 **제목 줄 우측**이고 아이콘이 아니라 글자다
            . 진입점 둘은 같은 어휘를 쓰고 `아이템 가격`(쓰기)이
            `히스토리`(읽기) **왼쪽**이다. 값을 매기는 쪽이 주마다 들르는 자리다. */}
        ...
        {!isPeriodLoading && characterGroups.length > 0 && (
          <View>
            ...
            {/* sticky 헤더 바닥 경계 = 카드 테두리 대체. 이 화면에 페이드가 없는
                이유가 이 한 줄이다. */}
  ...
  return (
    <BossProfitContextProvider value={bossProfitContext}>
      <View testID="screen-Profit" className="flex-1">
        ...
        {/* 총 수익 내역. 카드·보스 행과 같은 상자다. 셸 **바깥**에 두는 것은
