...
//  ② 구현 후 카드 금액은 **DB 기록에서만** 나온다 → 가릴 이유가 약해졌다.
//  ③ 실물을 띄워보니 라벨 배지("조회 불가")가 캐릭터명 폭을 먹어 **6자 이름부터 잘렸다**
//  (`내옆에최성일` → `내옆에…`). `n/12` 숫자 표기를 보류한 것과 같은 문제다.
// 그래서 라벨을 버리고 아이콘만 남긴다. 이름·금액·합계가 모두 온전하다. 원인 문구는 토스트가
// 담당하고 스크린리더에는 role="img" + aria-label로 전달한다.
...
// 팝오버는 **셸 바깥**(카드 루트)에 둔다. 셸은 펼침 상태에서 잘라내므로 안에 두면
// 잘린다. 고가 드롭 배지를 셸 바깥에 둔 것과 같은 이유다.
...
// 다투고(라벨 배지가 6자 이름을 잘라먹은 이유) 화면 폭에 따라 겹침이 생긴다.
//
...
// 이 두 값을 클래스가 아니라 값으로 적는 이유는 파일 머리 ③ 과 같다. 안 풀리는 클래스는 조용히
