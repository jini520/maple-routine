...
// | 갈래 | 왜 아닌가 / 왜인가 |
// |---|---|
// | 화면 안 절대 배치 | ** 가 막는다**. 펼친 카드 셸이 `overflow: clip` 이라 잘린다. 셸 밖 화면 루트까지 올리려면 컨텍스트로 좌표를 흘려야 하는데 그것이 곧 포털을 손으로 만드는 일이다 |
// | `BottomSheet` | 상호작용의 **모양이 바뀐다.** 이 상자는 "지금 이 줄의 내역"이라 트리거를 지목해야 하고(꼬리가 그 일을 한다), 게다가 보스 행은 때문에 이미 시트를 열어 둔 채 살아 있는 자리다. 시트 위 시트는 다른 물건이다 |
...
//  않는다" 이고, 실제로 이 없앤 "모르는 사실을 그리는 프레임"과 같은 종류다.
...
//  잃으므로 스크롤 컨테이너를 들었다. 별도 네이티브 윈도우는 아래 화면에
...
export function ItemRevenuePopover(props: {
   ...
   * 낱개로 못 꺼낸다(과 같은 구조적 한계). 목록 합으로 계산하면 그 경우
   * **합계가 카드 숫자와 어긋난다.**
   */
  crystalMeso: number
  itemMeso: number
            ...
            {listed.map((drop, index) => {
              ...
              return (
                ...
                >
                  ...
                  {/* **: 값이 없는 줄에 0 을 쓰지 않는다.** 미입력은 "0원에 팔았다"가
