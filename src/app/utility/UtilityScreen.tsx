 ...
 * 유틸리티. 도구 목록이다.
 *
 * 도구를 이 화면 안의 카드로 그리지 않고 하위 페이지로 미는 것이 결정이고, 그 값은 공짜로
 * 따라오는 전환 애니메이션과 iOS 가장자리 스와이프다.
 *
 * 목록은 2열 타일 격자다. 이름만 있고 설명은 없다. 도구 이름이 곧 설명이어야 한다는 뜻이고,
 * 그렇지 않은 이름은 이름 쪽을 고칠 일이다. 타일은 가로:세로 2:1 이다. 담는 것이 아이콘 하나와
 * 이름 한 줄뿐이라 정사각은 빈 자리가 더 컸다.
 *
 * 도구를 더할 때는 아래 타일을 형제로 하나 더 놓는다. 그리고 `routes.ts` 한 줄,
 * `RootNavigator` 의 `STACK_SCREENS` 한 줄, `docs/features/utility.md`. 넷뿐이다. 목록을 데이터
 * 배열로 돌리지 않는 것은 아직 하나뿐이기 때문이고, 셋쯤 되면 그때 돌린다.
 */
 ...
 * 타일 이름. 단어 단위로 줄바꿈한다.
 *
 * RN 의 `Text` 는 한글을 글자 단위로 끊는다(판매 분배금 계 / 산기). RN 에는 `word-break` 에
 * 해당하는 스타일이 없으므로 단어마다 `Text` 를 하나씩 두고 flex 아이템으로 감싼다. 줄바꿈이
 * 아이템 경계에서만 일어나므로 단어가 통째로 움직인다. 줄바꿈 문자를 박아 넣으면 넓은
 * 화면에서도 끊긴다.
 *
 * 가운데 정렬이다. 정렬은 `Text` 의 `text-center` 가 아니라 이 줄의 `justify-center` 가 한다.
 * 글자가 단어별 flex 아이템이라 각 줄을 미는 것이 그쪽 축이기 때문이다.
 */
      ...
      >
        ...
        <View className="flex-row flex-wrap gap-3 px-4 pb-4">
          <Pressable
            ...
            className="w-[48%]"
          >
            {/* 가로:세로 = 2:1, 안은 아이콘 → 이름 가로 배치. 납작한 타일에는 세로 쌓기보다
                가로가 맞는다. 세로면 둘 다 눌려 보인다. */}
