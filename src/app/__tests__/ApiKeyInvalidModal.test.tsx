...
const { confirmApiKeyInvalidMock, storeRef } = vi.hoisted(() => ({
  confirmApiKeyInvalidMock: vi.fn(),
  storeRef: { current: { apiKeyInvalidNotice: false } },
}))

vi.mock('../../features/onboarding/store', () => ({
  useOnboardingStore: () => ({
    apiKeyInvalidNotice: storeRef.current.apiKeyInvalidNotice,
    confirmApiKeyInvalid: confirmApiKeyInvalidMock,
  }),
}))
...
describe('ApiKeyInvalidModal (ADR-115 결정 10)', () => {
  beforeEach(() => {
    confirmApiKeyInvalidMock.mockClear()
    confirmApiKeyInvalidMock.mockResolvedValue(undefined)
    storeRef.current.apiKeyInvalidNotice = false
  })
  ...
  it('알림이 켜지면 원인과 다음에 일어날 일을 함께 말한다', () => {
    storeRef.current.apiKeyInvalidNotice = true
  ...
  it('확인을 눌러야 이동이 일어난다', () => {
    storeRef.current.apiKeyInvalidNotice = true
    render(<ApiKeyInvalidModal />)
    expect(confirmApiKeyInvalidMock).not.toHaveBeenCalled()

    fireEvent.click(screen.getByRole('button', { name: '확인' }))

    expect(confirmApiKeyInvalidMock).toHaveBeenCalledTimes(1)
  })
