/**
 * 당김 인디케이터의 «켜짐» — **사용자가 당긴 회차에만 참이다** ([[ADR-160]] 결정 1).
 *
 * ## 무엇이 문제였나
 *
 * [[ADR-130]] 결정 1 은 `refreshing = status === 'loading'` 으로 배선했다. 그 값은 «지금 조회
 * 중인가» 이지 «사용자가 당겼는가» 가 아니고, 조회를 시작하는 자리는 당김 말고도 둘 더 있다 —
 * **화면 마운트 하이드레이션**(`loadTrackedOcids` → `refresh(…, { auto: true })`, [[ADR-097]]
 * 결정 4 · [[ADR-101]] 결정 4)과 헤더 새로고침 버튼.
 *
 * 그래서 **탭을 옮기기만 해도 인디케이터가 프로그램적으로 열렸다.** 사용자 보고(2026-08-22)의
 * *"페이지 이동 시 당겨서 새로고침을 한 것처럼 동작한 뒤 상단이 비어 있는 상태로 멈춰 있다"* 가
 * 그것이다. [[ADR-130]] 이 그 대가를 적어 두긴 했지만(*"헤더 버튼으로 시작한 재조회에도 플랫폼
 * 인디케이터가 뜬다"*) 하이드레이션까지 같은 값을 쓴다는 것은 그때 세지 않은 자리다.
 *
 * ## 잃는 것이 없다
 *
 * 헤더 버튼은 **자기 표시를 이미 갖고 있다** — 아이콘이 돌고(`SPIN_ANIMATION`) 옆에 «조회 중...»
 * 이 뜬다([[ADR-141]] 결정 1). 그러니 플랫폼 인디케이터를 당김 전용으로 좁혀도 그 버튼의 되먹임은
 ...
 * [[ADR-096]] 결정 1), 두 스케줄러가 같은 스토어를 나눠 쓰는 자리에서는 한쪽의 당김이 다른 쪽의
 * 인디케이터를 연다.
 */
...
export function usePullRefresh(
  /**
   * 당김이 부르는 재조회 — 헤더 버튼이 부르는 것과 **같은 것**이어야 한다([[ADR-072]] 결정 2).
  ...
  return {
    ...
    onRefresh() {
      ...
      void run()
        ...
        // 세우고 토스트가 문구를 낸다([[ADR-063]]). 여기서 다시 말하면 같은 실패가 두 번 뜬다.
        .catch(() => undefined)
    },
  }
}
