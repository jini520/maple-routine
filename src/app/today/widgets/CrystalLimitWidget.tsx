 ...
 * 좁은 헤드라인 한 줄에 밀어 넣느라 합계(`46 / 180`)를 쓰지만, 여기서는 4x1 이
 ...
 * 을 실어 준다 — 여기에 숫자를 적으면 참조 데이터가 바뀌어도 화면만 옛 한도를 말한다.
 * **그래서 이 파일에는 한도 숫자가 한 번도 안 나온다**(주석 포함 — 테스트가 그것을 검사한다).
 *
 * ## 링은 «칸» 이 아니라 **연속 호**다
 *
 * 의 칸 링은 캐릭터당 주간 보스 한도(12)를 쪼갠 것이고, 그쪽은 칸을 셀 수 있어
 * 뜻이 있었다. 이 한도는 그 몇 배라 같은 방식으로 쪼개면 **읽을 수 없는 톱니**가 된다 — 비율을
 * 말하는 링은 연속 호라는의 갈래가 그대로 적용된다.
 *
 * ## 링 안은 두 줄이지만 **한 값**이다
 *
 ...
 * ) **어느 쪽인지 말하지 않는다**: 「판매 0개」라고 쓰면 없는 사실을 단정하고,
