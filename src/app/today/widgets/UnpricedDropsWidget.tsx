/**
 * 위젯 7. 가격을 아직 안 적은 드롭 건수를 그리는 타일.
 *
 * 이 타일이 옆 타일(위젯 4)의 `없음` 을 설명한다. 그쪽은 기록된 판매가만 줄 세우므로 값을 안 적은
 * 드롭이 순위에 아예 없다. 건수를 여기서만 말하는 것도 같은 이유다. 한 사실을 두 곳에서 말하지
 * 않는다.
 *
 * 2x2 만 아이템 이름을 보여 준다. `값을 적어야지` 보다 `그 연마석 얼마에 팔았지` 가 손을 움직이는
 * 문장이라, 이름 셋까지 세우고 나머지는 `외 N건` 으로 접는다. 타일은 목록이 아니라 요약이고 안에서
 * 스크롤하지 않는다.
 *
 * **0건이어도 타일은 남는다.** 좌표 배치라 자리를 빼면 빈 사각형이 남고, 다음 주에 다시 나타나면
 * 새 기능처럼 보인다.
 *
