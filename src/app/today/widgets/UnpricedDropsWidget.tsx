/**
 * 위젯 7 — **가격 미입력 드롭**([[ADR-147]] 결정 9 · 정정 5·13·14).
 *
 * ## 이 타일이 옆 타일의 «없음» 을 설명한다
 *
 * 위젯 4(최고가 아이템)는 **기록된 판매가**만 줄 세우므로, 값을 안 적은 드롭은 순위에 아예 없다
 * (값을 모르는 것을 가장 싼 것으로 단정하지 않는다 — [[ADR-147]] 결정 9). 그래서 위젯 4가 «가격이
 ...
 *
 * 「값을 적어야지」보다 **「그 연마석 얼마에 팔았지」** 가 손을 움직이는 문장이다. 건수만으로는 그
 * 문장이 안 나오므로 이름 셋까지 세우고 나머지는 «외 N건» 으로 접는다 — 타일은 목록이 아니라
 * 요약이고, 타일 안에서 스크롤하지 않는다([[ADR-147]] 결정 3).
 *
 * ## 0건이어도 타일은 남는다 ([[ADR-147]] 결정 5)
 *
...
/** 0건일 때 — 「0건」이 아니라 **끝났다는 사실**을 말한다(숫자 0은 할 일처럼 읽힌다). */
...
/**
 * 0건 — **타일은 남고 내용만 바뀐다**([[ADR-147]] 결정 5).
 *
