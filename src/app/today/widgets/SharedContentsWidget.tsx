/**
 * 위젯 9 — **계정 및 메이플 ID 공유 컨텐츠**(`4×auto` 정정 28~31).
 *
 * ## 계열은 **두 열**로 선다
 *
 ...
 * 것은 여전히 머리의 `?` 하나다. **채운 상자의 색은 `primary` 다** —
 ...
 * 공유 항목을 마지막 접속 월드 기준으로 돌려준다. 에픽던전 / 몬스터파크 /
 * 메이플 유니온으로 묶으면 **월드 라벨을 한 번도 안 쓰므로 틀린 말을 할 자리가 없다.** 한계가
 ...
 * 드래곤 퇴치)을 말줄임으로 밀어냈다. 의 완료 → CLEAR 는 이 위젯에서 죽고,
 ...
 *   분모를 앱이 지어내야 한다.
 * - 분자와 분모는 **붙여** 그린다(이 결정석 링에서 정한 것과 같은 이유 —
