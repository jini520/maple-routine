/**
 * 위젯 3. 이번 주 보스 수익을 결정석과 아이템 판매로 나눠 그리는 타일.
 *
 * 지키는 것 넷.
 *
 * ① **증감이 없다.** 그 자리를 채우는 것은 시간축 비교가 아니라 구성 비교라, 다른 기간을 조회하지
 *    않는다. 색은 링과 같은 짝이다(`primary` 결정석 · `third` 아이템).
 * ② **0 을 그리는 위젯은 이것뿐이다.** 기록이 없으면 옆의 한 줄이 그 사실을 지고, 그때는 스택 바와
 *    분해 금액도 함께 사라진다. 0/0 인 바는 분해할 것이 없는데 분해한 척이다.
 * ③ 금액은 `formatMesoShort` 로 접는다. 두 벌로 만들면 이 타일과 보스 수익 화면이 다르게 접는다.
 * ④ **카운트업을 안 쓴다.** 그쪽은 자릿수 전체를 굴리는 물건이라 접힌 표기(`12.0억`)와 짝이 안 맞는다.
 *
 * 기본 배치가 `4×auto` 인 것은 캐릭터 상한이 셋이라 자랄 폭이 좁아서다. 고정 3행이면 캐릭터가
 * 하나일 때 아래가 87px 빈다.
 *
...
function variantOf(w: number, h: WidgetHeight): Variant {
  ...
  // `'auto'` 는 **높이를 내용이 정하는** 4칸 타일이라 그리는 것은 4x3 과 같다.
