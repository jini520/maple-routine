...
// ④ **단위 메소는 1x1 만 예외**
//
// **네 크기를 전부 스냅샷으로 찍는다**. v1 배치가 쓰는 것은 2x1 하나뿐이라 나머지 셋은
...
describe('금액은 네 크기에 다 서고 단위만 1x1 에서 빠진다', () => {
  ...
  // 73×76 이라 물리적으로 안 들어간다. 유일하게 뺀 자리다.
...
describe('분배 표기', () => {
  it('4x2 는 **N인 분배** 를 단다. 총액보다 작은 이유를 그 자리에서 말한다', async () => {
    const { getByTestId } = await 위젯(크기['4x2'], 뷰모델({ topItem: 최고가(2, { splitLabel: '3인' }) }))

    expect(getByTestId('top-item-share')).toHaveTextContent('3인 분배')
  })

  it('단독이면 안 단다. 나눈 적 없는데 **1인 분배** 는 없는 사건을 말하는 것이다', async () => {
    const { queryByTestId } = await 위젯(크기['4x2'], 뷰모델({ topItem: 최고가(2, { splitLabel: null }) }))
  ...
  it.each(['2x2', '2x1', '1x1'] as const)('%s 은 자리가 없어 안 단다', async (size) => {
    const { queryByTestId } = await 위젯(크기[size], 뷰모델({ topItem: 최고가(0, { splitLabel: '3인' }) }))
