...
// ⑤ **물욕이라는 말을 쓰지 않는다**
//
// **세 크기를 전부 스냅샷으로 찍는다**. v1 배치가 쓰는 것은 4x1 하나뿐이라 나머지 둘은
// 아무도 안 부르는 렌더 분기이고, 그 분기를 지키는 것은 스냅샷뿐이다.

import { getValuableDroughtTier } from '../../../../lib/drop/drop-history'
...
describe('문구는 마운트당 한 번 고른다', () => {
  ...
  it('리렌더에도 안 바뀐다. 렌더마다 고르면 깜빡인다', async () => {
    ...
    //    (이것 없이는 **렌더마다 무작위** 로 되돌려도 테스트가 초록으로 남았다).
    await act(async () => {
      view.rerender(
        <ThemeProvider>
...
// 화면에 보이는 한국어는 `아이템 드롭`이다. 영문 식별자는 그대로 두므로 이 검사는
// **렌더 결과**에만 건다.
