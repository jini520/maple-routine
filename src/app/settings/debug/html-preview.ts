/**
 * 넥슨 공지 본문(HTML)을 블록으로 바꾸는 파서. **서버 파서의 복사본이다.**
 *
 * 제품에서는 이 일을 서버가 한다. 앱에 파서를 두면 넥슨이 마크업을 바꿀 때 그 수정이 OTA
 * 배포 축에 실리기 때문이고, 그 결정은 그대로다. 여기 복사본을 둔 이유는 하나뿐이다 -
 * **서버 배포 전에 파싱 결과를 눈으로 보려고.** 그래서 화면과 함께 지운다.
 *
 * ⚠️ **원본은 `maple-routine-server` 의 `src/html.ts` 다.** 그쪽을 고치면 이 복사본은 낡는다.
 * 낡은 채로 보면 «앱에서는 이렇게 보이는데 서버는 다르게 판다» 가 되므로, 파서를 손볼 때는
 * 이 파일을 다시 복사하거나 폴더째 지울 것.
 *
