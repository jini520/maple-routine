...
// ② `getByRole('heading')` 이 없다. RN 에 heading 역할이 없어 제목 글자로 본다. `제목이 하나뿐`
//    케이스도 그래서 `업데이트 카드가 자기 제목을 그리지 않는다` 로만 남는다.
// ③ OTA 상태는 포트를 갈아끼우는 것으로 만든다. 문구 열넷의 계약은 `AppUpdateSection` 테스트가
//    프롭을 직접 넣어 지킨다. 여기서 보는 것은 배선이다.
