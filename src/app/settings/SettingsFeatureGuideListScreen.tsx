/**
 * 설정 하위 페이지 `기능 설명`. 앱 기능을 기능 축으로 나열한 카탈로그이고 사용법 설명의 원천이다.
 *
 * 안내가 개발 노트 항목에만 붙어 버전 축으로 존재하면 지금 이 앱을 어떻게 쓰나 에 답할 자리가
 * 없다. 오래된 기능이 옛 버전 카드 안에 묻힌다. 여기가 원천이고 개발 노트는 링크만 건다.
 *
 * 탭은 하단 탭바와 같은 축이다. 이미 아는 구획이라 안내를 찾을 때 새로 배울 것이 없다.
 * `유틸리티` 만 그 축 밖이고 지금은 비어 있어 탭이 뜨지 않는다.
 *
...
import { BookOpenIcon, Card, ChevronRightIcon, Text } from '../../components/atoms'
