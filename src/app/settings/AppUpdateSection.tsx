...
//
// `UpdatePromptModal` 과 **같은 벽, 같은 처방**이다([[ADR-128]] 결정 7).
//
...
//    을 읽는데([[ADR-024]]) Metro·jest 에서 `import.meta.env` 는 `undefined` 다.
//
// 그래서 타입만 core 에서 가져오고(`import type` 은 컴파일에서 지워져 모듈이 평가되지 않는다)
// 상태 열넷과 필드 이름이 두 벌이 되지 않게 한다. **표시 상태는 하나도 지우지 않고 전부 적어
// 둔다** — 지금 도달하는 것은 호출부가 심는 `unsupported` 하나뿐이지만, 그 표가 곧 [[ADR-026]]·
// [[ADR-027]]·[[ADR-126]] 이 정한 계약이고 OTA 가 붙는 날 배선은 `state={useLiveUpdateStore()}`
