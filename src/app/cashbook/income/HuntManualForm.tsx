/**
 * 「사냥」 수동 입력 폼([[ADR-201]] 결정 1). **획득 메소를 사람이 친다.**
 *
 ...
 *
 * [[ADR-175]] 이전에 적힌 사냥 행도 이 폼으로 연다(결정 4). 그 행은 조각이 없어 **합계가 곧 획득
...
export function HuntManualForm(props: IncomeFormProps): React.JSX.Element {
  ...
  /**
   * 되살릴 입력([[ADR-201]] 결정 3). 수동으로 적힌 행일 때만 값이 있다.
   *
  ...
  /**
   * 친 획득 메소. **옛 행에서는 합계가 곧 이 값**이다([[ADR-201]] 결정 4) — 그 행은 조각이 없어
  ...
  return (
    <>
      ...
      <AmountFigure
        // 큰 숫자는 여기서도 **합계**다([[ADR-201]] 결정 1) — 사람이 치는 것은 획득 메소이지
      ...
      <SaveRow
        ...
        onSave={() =>
          void submit({
            ...
            category: '사냥',
            // **사냥터 이름 칸이 없다**([[ADR-201]] 결정 7). 새 기록은 비고, 옛 행의 이름은 그대로
