    ...
    <>
      ...
      */}
      ...
      {/*
        ...
        `flex` 를 `style` 로 주는 것은 비율이 값이기 때문이다. 클래스에 임의 값을 적으면 그
        수가 두 곳(둘의 합)으로 흩어져 왜 이 비율인가 가 안 읽힌다.
      */}
      <View testID="income-sheet-meso-line" className="flex-row items-start gap-3">
        {/*
          넓은 쪽이 켜는 칸이다. 라벨 다섯 글자와 체크박스 둘이 함께 서야 해서 값 칸보다
          자리가 더 든다. 값 쪽은 라벨과 숫자 하나뿐이라 좁아도 선다.
        */}
        <View
        ...
        {/*
          언제나 선다. 캐릭터를 안 골랐어도 그 자리는 있다. 안 세우면 캐릭터를 고르는 순간
          줄이 생겨 아래가 통째로 밀리고, 무엇보다 메획이 안 든다 는 사실을 화면이 말하지
          않는다. 캐릭터가 없으면 캐릭터 메획이 0 이고 켠 것이 없으면 0% 다.

          읽혔으면 못 친다. 치는 칸이 되는 것은 캐릭터를 골랐는데 못 읽었을 때뿐이다.
        */}
      ...
      {/* 못 친다. 앱이 세는 값이다. 큰 숫자(합계)와 다른 값이라 자기 줄을 갖는다.
          ...
          `≈` 를 붙인다. 이 수는 젠 주기·마릿수·레벨로 미리 세어 둔 값이지 실제로 받은 액수가
          아니다. 표식이 없으면 정산된 금액처럼 읽힌다. 0 에는 안 붙인다. 아직 어림할 것이
          없다. */}
