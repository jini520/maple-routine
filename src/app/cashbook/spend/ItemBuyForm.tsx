 ...
 *
 * 금액 칸은 늘 서고 장비에서만 라벨이 바뀐다. 하나를 사는 자리에 `단가` 라고 적으면 여러 개
 * 중 한 개 값이라는 뜻이 되어 거짓이다. 줄이 하나라 상태도 `testID` 도 하나다. 큰 숫자는
 * 양쪽 다 못 치는 합계다.
 *
 * 소비·기타에 관세가 없는 것은 월드 간 거래가 안 되기 때문이다. 있을 수 없는 것을 물을 수 있게
 * 두지 않는다. 통화는 언제나 메소다. 관세를 메소로 재므로 메포 칸이 없다.
 */
...
export function ItemBuyForm(props: SpendFormProps): React.JSX.Element {
  ...
  return (
    <>
      ...
      {!counts && (
        /*
         * 관세도 라벨–값 줄이다. 시트에서 고르는 것은 전부 이 모양인데 관세만 큰 숫자 밑의
         * 맨몸 체크박스였다. 장비에만 선다. 끄는 것이 아니라 줄 자체가 없다. 있는데 못 누르면
         * 왜 못 누르나 를 새로 묻게 된다.
         *
         * 더해지는 금액을 안 적는다. 큰 숫자가 그만큼 올라간다.
         */
      ...
      {counts && (
        // 스테퍼가 아니라 치는 칸이다. 주문서 300장을 스테퍼로 세면 300번을 누른다.
        //
        // 단위는 개다. 여기서 세는 것은 아이템이라 단위를 적을 수 있다. `기타` 는 자유 입력이라
        // 앱이 무엇을 세는지 몰라 그대로 비어 있다.
        <FieldRow label="수량">
          <AmountInput
      ...
      <AmountFigure
        // 어디서도 못 친다. 관세를 더한 값을 바로 그린다. 더해지는 금액을 따로 안 적는 것은
        // 그대로다.
        value={amount}
        unit="메소"
