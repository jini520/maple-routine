...
describe('항목 — 고르면 채워진다', () => {
  ...
  // [[ADR-202]] 결정 9 로 힌트 줄이 사라져 그 자리는 아예 안 그린다.
...
/** 「기타」는 큰 숫자가 합계라 **금액 칸**에 친다([[ADR-173]] 결정 17 · 라벨은 [[ADR-202]] 결정 7). */
...
/**
 * 금액을 치는 자리는 **라벨–값 줄**이다([[ADR-202]] 결정 1·2) — 큰 숫자는 어디서도 못 친다.
 *
 * 「아이템 구매」에서 그 줄은 늘 서고 라벨만 갈린다(장비면 `구매 비용`, 나머지는 `단가`).
...
describe('아이템 구매', () => {
  ...
  /**
   * **갈래를 옮기면 금액을 안 들고 간다**([[ADR-173]] 결정 13, 사용자 지정 2026-08-26).
   *
   * 큰 숫자는 언제나 **곧바로** 새 값이다 — [[ADR-202]] 결정 12 가 카운트업을 걷어, 값이 어떤
   * 이유로 바뀌든 중간값이 안 보인다.
   */
  ...
  /**
   * **친 구입가는 「구매 비용」 칸에 남는다**([[ADR-202]] 결정 1·2).
   *
   * 관세를 켜면 큰 숫자가 합계가 되고(더해지는 금액을 따로 안 적는 이유다 — [[ADR-173]] 결정 5)
   * 사용자가 친 값은 자기 칸에 그대로 서 있다. 결정 6 이 커서로 두 값을 오가게 하던 자리를 칸
   * 하나가 대신한다.
   */
...
describe('시세가 비어 있을 때', () => {
   ...
   *
   * 왜 0 인지를 말하던 힌트 줄은 [[ADR-202]] 결정 9 가 걷었다(사용자 지정 2026-09-02). 그 몫은
 ...
 */
describe('기타 — 금액 × 수량 ([[ADR-173]] 결정 17)', () => {
  ...
  // 큰 숫자는 **메소 축**이다([[ADR-173]] 결정 11) — 낸 메포는 금액 칸과 수량이 말한다.
  // 그것을 한 줄로 적던 힌트는 [[ADR-202]] 결정 9 가 걷었다.
  ...
  // 아이템 구매의 **장비**도 금액 칸에서 받는다([[ADR-202]] 결정 2) — 수량이 없어 곱할 것이
...
/**
 * **이름 칸의 라벨은 갈래가 정한다**([[ADR-170]] 정정 14 ② · [[ADR-202]] 결정 8) — 아이템 구매에서 그 칸이 묻는 것은
...
describe('직접 입력의 이름 칸 ([[ADR-170]] 정정 14 ②)', () => {
  ...
  it('기타는 「내용」 이다 — 수입 「기타」와 같은 낱말이다 ([[ADR-202]] 결정 8)', async () => {
    const view = await 그리기()
 ...
 */
describe('금액 줄의 단위 ([[ADR-170]] 정정 14 ④)', () => {
...
describe('「아이템 구매」의 종류 ([[ADR-173]] 정정 1)', () => {
   ...
   * ([[ADR-173]] 결정 12).
   *
   * [[ADR-173]] 결정 12 는 그 튐을 **굴리지 않고 갈아 끼우는 것**으로 다뤘는데, [[ADR-202]]
   * 결정 12 가 카운트업 자체를 걷어 이제 **모든 값 변화가 곧바로** 선다. 지켜야 할 것은 남았다:
   * 종류를 바꾼 뒤의 합계가 관세를 뗀 값이어야 한다.
   */
...
/**
 * 수정으로 열면 **그 기록의 값이 곧바로 선다** ([[ADR-202]] 결정 12).
 *
 ...
 */
describe('수정으로 열 때의 큰 숫자 ([[ADR-202]] 결정 12)', () => {
