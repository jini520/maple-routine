...
describe('갈래 칩', () => {
  ...
  /**
   * **칩은 고르는 화면에만 선다**(사용자 지정 2026-08-27).
   *
...
describe('항목 — 고르면 채워진다', () => {
  ...
  /**
   * **시세는 형태·단계를 고르기 전에도 선다**(사용자 지정 2026-08-26).
   *
  ...
  // 「−0」 은 «0 원짜리 지출» 로 읽히는데 사실은 «아직 안 골랐다» 다(의 태도).
  ...
  /**
   * **합계는 언제나 메소다**(사용자 지정 2026-08-26). 가계부의 축이 메소라
...
describe('아이템 구매', () => {
   ...
   * **친 구입가는 「구매 비용」 칸에 남는다**.
   *
   * 관세를 켜면 큰 숫자가 합계가 되고(더해지는 금액을 따로 안 적는 이유다 —)
...
describe('시세가 비어 있을 때', () => {
   ...
   *
   * 왜 0 인지를 말하던 힌트 줄은 가 걷었다(사용자 지정 2026-09-02). 그 몫은
 ...
 */
/**
 * 「기타」는 **단가 × 수량**이다(사용자 지정 2026-08-27).
 *
...
/**
 * **안 열린 묶음은 흐리게 두고 못 고른다**(사용자 선택 2026-08-27).
 *
...
/**
 * **직접 입력의 금액 뒤에 단위를 적는다** — 「기타」는 통화를 고르는 자리라
...
/**
 * **고를 것에 그림이 붙는다**(사용자 지정 2026-08-27).
 *
 * 그림이 있는 것만 붙는다 — 없는 것에 무언가를 지어내 놓지 않는다(의 태도).
...
describe('「아이템 구매」의 종류', () => {
   ...
   *
   *
   *  는 그 튐을 **굴리지 않고 갈아 끼우는 것**으로 다뤘는데
...
/**
 * **관세도 라벨–값 줄이다**(사용자 지정 2026-08-28).
 *
...
/**
 * 「아이템 구매」의 수량에는 **단위가 붙는다** (정정, 사용자 지정 2026-08-29).
 *
 * 결정 17 이 «수량에 단위를 안 적는다» 고 한 근거는 **「기타」가 자유 입력이라 앱이 무엇을 세는지
 * 모른다**는 것이었다. 아이템 구매에서 세는 것은 **아이템**이라 그 근거가 성립하지 않는다.
 */
