...
/**
 * 시트를 열고 갈래까지 고른다. 시트는 1차에서 갈래를 먼저 묻는다.
 *
 * `pick` 에 `null` 을 주면 고르는 단계에서 멈춘다. 1차 시트 자체를 보는 케이스가 그렇다.
 */
async function 그리기(
  overrides: Partial<React.ComponentProps<typeof IncomeSheet>> = {},
  pick: 'hunting' | 'sol_erda_fragment' | 'item_sale' | 'etc' | null = 'hunting',
) {
  const view = await 시트열기(overrides)
  if (pick !== null && overrides.editing === undefined) {
    await act(async () => {
      fireEvent.press(view.getByTestId(`income-sheet-category-${pick}`))
    })
  }
  return view
}
...
/**
 * 아이템 판매로 여는 시트. 1차에서 그 카드를 누르고 시작한다.
 */
async function 판매시트(
  overrides: Partial<React.ComponentProps<typeof IncomeSheet>> = {},
): Promise<Rendered> {
...
/**
 * 칸에 값을 넣는다. **칸이 어느 모양인지 보고 길을 고른다.**
 *
 * 치는 칸이 입력 카드로 옮겨가는 중이라 두 모양이 섞여 있다. 옮겨간 칸은 줄이 누르개라 카드를
 * 열고 쳐서 확인해야 하고, 아직 안 옮긴 칸(사냥 수동 입력)은 줄에서 바로 친다. 폼이 하나씩
 * 옮겨갈 때 이 도우미는 안 고쳐도 된다.
 */
async function 칸에치기(view: Rendered, testID: string, text: string): Promise<void> {
...
/**
 * 카드로 옮겨간 칸이 **줄에 적고 있는 글자**. 값이 비면 자리표시자가 그 자리에 선다.
 *
 * 그 줄은 이제 누르개라 `props.value` 가 없다. 보이는 글자로 재는 것이 사용자가 보는 것과 같다.
 */
function 줄글자(view: Rendered, testID: string): string {
  const 줄 = view.getByTestId(testID)
  // 아직 안 옮긴 칸은 줄이 곧 입력이다(`칸에치기` 와 같은 갈림).
...
describe('갈래', () => {
  it('1차 시트가 갈래 넷을 카드로 세운다', async () => {
    const view = await 그리기({}, null)

    expect(view.getByTestId('income-sheet-category-hunting')).toBeTruthy()
    expect(view.getByTestId('income-sheet-category-sol_erda_fragment')).toBeTruthy()
   ...
   */
  it('타일 넷이 저마다 게임 그림을 든다. 빈 자리가 없다', async () => {
    const view = await 그리기({}, null)
    ...
    expect(view.getByTestId('income-sheet-category-icon-sol_erda_fragment', 숨은것)).toBeTruthy()
...
describe('금액. OS 숫자 키보드다', () => {
   ...
   * 빠른 칩은 없다.
   *
   * 폼 안에 두면 저장과 이웃하고, 키보드 위로 내보내면 자리를 못 잡는다(라이브러리 슬롯).
   * 시트 마지막 자식으로 붙여도 마찬가지라 없는 편이 낫다.
   */
...
describe('판매 수수료', () => {
  ...
  /**
   * **친 대금은 그 자리에 남는다**. 요율을 껐다 켰다 해도 판매 대금이 안 부푼다.
   ...
   *
   * 갈래가 폼을 가르므로 옮기는 순간 언마운트된다. 지울 것을 손으로 세지 않는다.
   */
  /**
   * **갈래를 옮기는 길이 없어졌다.** 갈래는 1차 시트에서 한 번 고르고, 바꾸려면 시트를 닫고
   * 다시 연다. 그래서 `옮겨도 값을 들고 다닌다` 는 결함이 구조에서 사라졌다.
   *
...
describe('수정 모드', () => {
  ...
  // **제목이 고른 것** 이다. 수입은 고를 것이 갈래뿐이라 그것이 곧 제목이다.
  ...
  /**
   * **판매 대금이 정확히 되짚어진다**. 받는 돈 + 뗀 몫 이다. 요율만 들고 역산하면
 ...
 *
 * 서는 자리는 **`기타` 하나**다: 아이템 판매는 경매장이라 메소이고 사냥도 메소다.
 * 재는 규칙은 지출과 같다. 메포는 시세로 환산해 합계에 들고, 캐시는 안 든다.
 */
describe('통화', () => {
  ...
  it('캐시는 캐시 칸에 담기고 **환산하지 않는다** 고 말한다', async () => {
    ...
    // `캐시는 메소로 환산하지 않아요` 줄은 걷었다. 캐시가 메소 축에 안
  ...
  /**
   * 기타는 **금액 × 수량**이다. 지출 기타와 같은 식이다.
   *
   * 수량은 `income_records.quantity` 에 박힌다. 안 남기면 그 행을 다시 열 때 되짚을 길이
 ...
 * 원인이었다. 이제 갈래가 폼을 가르므로 옮기면 언마운트된다.
 */
describe('갈래마다 자기 폼', () => {
  ...
  /**
   * 갈래를 옮기는 길이 1차 시트로 옮겨 갔으므로, 값이 갈래를 넘어가는지는 **다시 연 시트가
...
describe('사냥 계산기', () => {
  ...
  /**
   * 사냥터 차례는 레벨 차이가 적은 순, 같으면 마릿수가 많은 순이다. 참조표 순서를 그대로
  ...
  it('조각을 한 번에 친다. 스테퍼 버튼이 없다', async () => {
    ...
    await 칸에치기(view, 'income-sheet-fragments', '83')
  ...
  it('사냥터를 안 고르면 조각을 적어도 저장이 안 된다', async () => {
    ...
    await 칸에치기(view, 'income-sheet-fragments', '1')
    await 칸에치기(view, 'income-sheet-fragment-price', '1000000')
    await 이름으로누르기(view, '저장')
  ...
  /**
   * **계산 입력을 함께 남긴다**. 없으면 수정 시트가 빈 계산기로 열려 만지는 순간
   * 금액이 덮인다.
   */
...
describe('사냥 수동 입력', () => {
  ...
  it('옛 행을 고쳐 저장해도 사냥터 이름이 안 지워진다 (결정 7)', async () => {
    ...
    // 옛 행은 캐릭터가 비어 있다. 사냥 기록은 캐릭터를 골라야 다시 저장된다.
    await 사슬고르기(view, 'ocid-1')
...
describe('조각 가격을 비우면 보관', () => {
  ...
  it('0 을 치면 커서가 빠져도 `0` 이 남고 0 으로 저장된다', async () => {
    ...
    await 밤의길3(view)
    await 칸에치기(view, 'income-sheet-fragments', '12')
    await 칸에치기(view, 'income-sheet-fragment-price', '000')
    await act(async () => {
      fireEvent(view.getByTestId('income-sheet-fragment-price'), 'blur')
    })

    expect(줄글자(view, 'income-sheet-fragment-price')).toBe('0')
    await 이름으로누르기(view, '저장')
    expect(onSave.mock.calls[0][0]).toMatchObject({
      mesoAmount: 21_168_000,
...
describe('사냥 기록의 캐릭터', () => {
  ...
  it('수동 입력은 획득 메소를 쳐도 캐릭터가 없으면 저장이 꺼진다', async () => {
    ...
    expect(view.getByLabelText('저장').props.accessibilityState?.disabled).toBe(true)
    await 사슬고르기(view, 'ocid-2')
    expect(view.getByLabelText('저장').props.accessibilityState?.disabled).not.toBe(true)
  })

  it('수동 입력의 캐릭터 목록에 `선택 안함` 이 없다', async () => {
    const view = await 그리기({}, 'hunting')
    await 이름으로누르기(view, '획득 메소 직접 입력')
  ...
  it('캐릭터 없이 적힌 옛 사냥 기록은 캐릭터가 빈 채 열리고 저장이 꺼져 있다', async () => {
    const view = await 그리기({ editing: 옛사냥행, onDelete: jest.fn() })
 ...
 */
describe('솔 에르다 조각 정산', () => {
  function 정산시트(overrides: Partial<React.ComponentProps<typeof IncomeSheet>> = {}): Promise<Rendered> {
    return 그리기(overrides, 'sol_erda_fragment')
  }

  const 정산기록: IncomeRecord = {
    ...옛사냥행,
    id: 'inc-settle',
    ocid: 'ocid-1',
  ...
  it('캐릭터를 고르기 전에는 보관을 안 읽고 저장이 꺼져 있다', async () => {
    const loadFragmentStorage = jest.fn(async () => 120)
    const view = await 정산시트({ loadFragmentStorage })

    expect(loadFragmentStorage).not.toHaveBeenCalled()
    expect(view.getByTestId('income-sheet-fragment-storage')).toHaveTextContent('-')
    expect(view.getByLabelText('저장').props.accessibilityState?.disabled).toBe(true)
  })

  /** 판 개수를 치면서 상한인 보관을 같은 줄에서 본다(사용자 지정 2026-09-16). */
  it('보관 중과 판 개수가 한 줄에 선다', async () => {
    const view = await 정산시트()

    const 줄 = view.getByTestId('income-sheet-settle-count-row')
    expect(flattenStyle(줄.props.style).flexDirection).toBe('row')
    expect(within(줄).getByTestId('income-sheet-fragment-storage')).toBeTruthy()
    expect(within(줄).getByTestId('income-sheet-settle-count')).toBeTruthy()
  })

  it('캐릭터를 고르면 그 캐릭터의 고른 날까지 보관을 센다', async () => {
    const loadFragmentStorage = jest.fn(async () => 120)
    const view = await 정산시트({ loadFragmentStorage })

    await 사슬고르기(view, 'ocid-1')

    expect(loadFragmentStorage).toHaveBeenLastCalledWith('ocid-1', '2026-08-23', undefined)
  ...
  it('판 개수가 보관을 넘기면 저장이 꺼진다. 보관만큼은 팔 수 있다', async () => {
    ...
    await 아이디로치기(view, 'income-sheet-settle-count', '121')
    expect(view.getByLabelText('저장').props.accessibilityState?.disabled).toBe(true)

    await 아이디로치기(view, 'income-sheet-settle-count', '120')
    expect(view.getByLabelText('저장').props.accessibilityState?.disabled).not.toBe(true)
  })

  it('판 개수나 개당 가격이 비면 저장이 꺼진다', async () => {
    const view = await 정산시트({ loadFragmentStorage: async () => 120 })
    await 사슬고르기(view, 'ocid-1')
    ...
    await 아이디로치기(view, 'income-sheet-settle-count', '')
    await 아이디로치기(view, 'income-sheet-settle-price', '8000000')
    expect(view.getByLabelText('저장').props.accessibilityState?.disabled).toBe(true)
  })
  ...
  it('날짜를 바꾸면 보관을 다시 센다', async () => {
    const loadFragmentStorage = jest.fn(async (_ocid: string, dateKey: string) =>
      dateKey === '2026-08-22' ? 30 : 120,
    )
    const view = await 정산시트({ loadFragmentStorage })
    await 사슬고르기(view, 'ocid-1')
  ...
  it('캐릭터를 바꾸면 그 캐릭터의 보관을 센다', async () => {
    const loadFragmentStorage = jest.fn(async (ocid: string) => (ocid === 'ocid-2' ? 40 : 80))
    const view = await 정산시트({ loadFragmentStorage })
    await 사슬고르기(view, 'ocid-1')
    expect(view.getByTestId('income-sheet-fragment-storage')).toHaveTextContent('80개')
  ...
  it('수정으로 열면 이 기록을 뺀 보관을 세고 판 개수와 단가를 되짚는다', async () => {
    const onSave = jest.fn()
    const loadFragmentStorage = jest.fn(async () => 50)
    const view = await 그리기({ editing: 정산기록, onDelete: jest.fn(), onSave, loadFragmentStorage })
...
describe('메소 획득량', () => {
  ...
  /**
   * **언제나 선다**. 캐릭터를 안 골랐어도 자리는 있다. 안 세우면 캐릭터를
  ...
  it('못 읽으면 **치는 칸**이 되고 기본값은 마지막 성공값이다 (결정 7)', async () => {
    ...
    expect(줄글자(view, 'income-sheet-meso-rate-input')).toBe('161')
  ...
  it('마지막 성공값도 없으면 빈 칸이고 0 으로 센다', async () => {
    ...
    // 줄은 언제나 **결과**를 적는다. 마지막 성공값이 없으면 그 결과가 0 이다.
    expect(줄글자(view, 'income-sheet-meso-rate-input')).toBe('0')
    // 치는 값은 캐릭터 메획이고 그쪽이 빈 칸이다.
    await 아이디로누르기(view, 'income-sheet-meso-rate-input')
  ...
  it('폴백 칸에 친 값이 계산에 든다', async () => {
    ...
    await 밤의길3(view)
    await 루디고르기(view)
    await 칸에치기(view, 'income-sheet-meso-rate-input', '100')
  ...
  /**
   * **아이템을 켜면 이 줄의 숫자가 오른다**. 게임 스탯창처럼 증가량 이고
  ...
  /**
   * **자른 숫자가 돈을 세지 않는다**. 줄에는 258% 가 적히지만
  ...
  it('줄은 결과를 적고 카드는 캐릭터 메획을 받는다', async () => {
    ...
    // 카드를 열면 결과가 아니라 **캐릭터 메획**이 씨앗으로 선다.
    await 아이디로누르기(view, 'income-sheet-meso-rate-input')
    expect(view.getByTestId('input-card-value').props.value).toBe('149')
    await 아이디로누르기(view, 'input-card-close')

    await 칸에치기(view, 'income-sheet-meso-rate-input', '100')
    // (100 + 100 + 50) − 100 = 150
    expect(줄글자(view, 'income-sheet-meso-rate-input')).toBe('150')
  ...
  /**
   * 줄을 하나 줄인다. 숫자와 그것을 움직이는 칩이 한 줄에 붙으므로 시트가 82vh 를 안 넘고
   * 읽기도 낫다.
   */
  /**
   * **켜는 자리와 세어진 값이 위아래로 갈린다**.
  ...
  it('못 읽어 치는 칸은 아이템을 켜도 폭이 그대로다', async () => {
    ...
    // 값 자리의 폭을 못박은 것은 줄이 아니라 그 자리다. 자릿수가 늘어도 왼쪽 덩어리가 안 밀린다.
    const 자리 = flattenStyle(view.getByTestId('income-sheet-meso-rate-slot').props.style)
    expect(자리.minWidth).toBe(56)
  })
