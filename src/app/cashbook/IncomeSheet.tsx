 ...
 * 정하던 자리가 규칙이 아니라 **기본값**이 됐다.
 *
 * ## 여기 서는 것은 **손입력 수익**뿐이다
 *
 * 보스 드롭은 이 시트로 안 들어온다 — 이미 보스 수익 탭이 기록하고, 두 곳에서
 ...
 * **수정 모드의 머리는 «고른 것»** 이다 — 수입은 고를 것이 갈래뿐이라
...
export interface IncomeSheetProps {
  ...
  /**
   * 마지막으로 쓴 메소마켓 시세 — 「기타」를 메포로 적을 때의 기본값이다.
   * 지출 시트와 **같은 계약**이라 화면이 한 값을 두 시트에 그대로 넘긴다.
   */
  lastPointRate: number | null
  /**
   * 캐릭터의 **메소 획득량**을 읽어 온다 — 시트는 `nexon/` 도 `storage/` 도
   * 모른다. 사냥 폼만 쓴다.
   */
  ...
  return (
    <BottomSheet testId="income-sheet" onClose={props.onClose}>
      <View className="gap-3 px-4 pb-2">
        ...
        {/* **새로 적을 때만 선다** — 수정 모드에서는 기록이 이미 정했다.
 ...
 *
 * `hunt` 가 `null` 인 행은 이전에 적힌 것이고 수동으로 연다(결정 4) — 조각이 없어
