/**
 * 컨텐츠 이름에서 지역 슬러그를 찾는 함수. 그 슬러그로 배경과 아이콘을 조회한다.
 *
 ...
 *
 * 공백을 지우고 견주는 것은 넥슨이 주는 이름과 표의 이름이 띄어쓰기만 다른 경우가 있어서다. 지우는
 * 것은 견줄 때뿐이고 돌려주는 값은 표의 원본이다.
 *
 * 앞부분 일치인 것은 넥슨 이름에 지역 뒤로 꼬리가 붙기 때문이다(`아르카나 - 무릉도원`). 주간 지역
 * 퀘스트만 전체 일치인 것은 그 표가 컨텐츠 이름을 통째로 들고 있어서다.
 */
