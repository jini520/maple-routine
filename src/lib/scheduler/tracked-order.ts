/**
 * 추적 목록의 저장 배열 순서를 화면 목록에 얹는 함수. 저장 순서가 곧 표시 순서다.
 *
 * 정렬을 스토어에서 안 고치고 여기서 얹는 것은, 그 정렬(레벨 내림차순)로 사는 소비자가 따로 있어서다.
 * 스토어는 안정된 기준 순서를 그대로 내고 화면 쪽이 다시 세운다. 플래그를 하나 더 만들지 않는 것이
 * 요점이다.
 *
 * ⚠️ **목록에 없는 항목을 버리지 말 것.** 저장 목록과 스토어 목록은 한순간 어긋난다(저장 직후 ·
 * 동기화가 두 번 커밋하는 사이). 그때 버리면 **캐릭터가 화면에서 통째로 사라진다.** 순서를 정하는
 * 함수가 목록의 크기를 바꾸면 안 되므로, 모르는 항목은 뒤에 원래 순서로 남긴다.
 */
