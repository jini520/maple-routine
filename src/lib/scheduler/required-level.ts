/**
 * 「이 캐릭터가 이 항목을 진행할 수 있는가」 — 요구 레벨 판정 **한 곳** ([[ADR-162]] 결정 1).
 *
 * ## 왜 한 곳인가
 *
 * 소비처가 다섯이다 — 컨텐츠 카드의 배지 · 보스 카드의 배지 · 컨텐츠 진행률의 분모 · 초상화 링 ·
 * today 「남은 스케줄」. 판정이 흩어지면 **같은 항목이 화면마다 다르게 세어진다**, 그리고 그것은
 * [[ADR-147]] 결정 8 이 금지하는 상태다(*"today 가 세는 «남은 것» 은 스케줄러 화면이 보여 주는
 * 것과 한 글자도 다르면 안 된다"*). 참조표 조회도 여기 모으는 이유가 같다.
 *
 * ## 이것은 잠금이 아니다
 *
 * [[ADR-055]] 정정 2 가 폐기한 것은 «수동 추적에서 고르지 못하게 막는 것» 이고, 여기서 하는 것은
 * «표시하고 집계에서 빼는 것» 이다. **막는 것은 아무것도 없다** — 축이 다르므로 그 정정을 되살리는
 * 것이 아니다(그 구분을 [[ADR-162]] 가 본문에 못 박았다).
 *
 * ## 두 «없음» 을 안 가른다
 *
 * | | 판정 | 왜 |
 * |---|---|---|
 * | 캐릭터 레벨을 모른다 | 진행 가능 | 모르는 것을 단정하지 않는다([[ADR-057]] 태도) |
