 ...
 * 대비비는 참고 수치다.
 *
 * 이 모듈은 대비를 관문으로 쓰지 않는다. 판단의 최우선은 전체 색감과 캐릭터의 컬러 컨셉이고
 * 대비는 그다음에 참고한다. 대비를 필수 기준으로 세우면 아름다운 선택이 번번이 면제로 밀려난다.
 *
 * 그래서 면제(waiver) 장치가 없다. 통과·실패가 없으니 면제할 것도 없다.
...
/**
 * accent 를 글자·아이콘으로 쓸 때의 가시성 하한.
 *
 * AA(4.5:1)를 겨냥하지 않는다. 그렇게 하면 머쉬맘 브랜드 주황(`#F58B0F`, 표면 대비 2.38:1)이
 * 짙은 갈색(`#A15800`)으로 눌려 탭·배지·링크 53곳의 색이 통째로 바뀐다. 색감이 최우선인데
 * 대비를 목표로 잡는 순간 브랜드 색이 먹힌다.
 *
 * 이 값은 목표가 아니라 안전망이다. `*-ink` 를 만든 이유는 렌의 창백한 하늘색(`#C9EEF2`)이
 * 글자로 1.24:1 이라 아예 안 보이던 사고 하나였다. 그 구간만 막고 그보다 위면 accent 원색을
 * 그대로 쓴다.
 */
 ...
 */
/**
 * 짙은 전경은 검정이 아니라 그 색의 진한 톤이다. 명도를 0.18 까지 누르면 파스텔 채움 위에
 * 사실상 검은 글씨가 앉아 어울리지 않는다. 0.38 + 채도 유지면 진한 올리브·진한 틸로 읽히면서
 * 대비도 5~7:1 나온다.
 */
 ...
 * 채움 위 전경색.
 *
 * 테마의 연한 색 하나를 만들어 모든 채움에 쓴다. 흰색도 검정도 아니고 채움색마다 다른 색조도
 * 아니다. 브랜드 색상(H)에서 뽑은 연한 톤이라 테마 전체가 한 톤으로 묶인다. 엔젤릭버스터라면
 * 아주 연한 핑크(`#FFECF6`), 머쉬맘이라면 연한 살구(`#FFEEE2`)다.
 *
 * 채움색마다 그 색조의 연한 톤을 만들면 테마가 여러 톤으로 흩어지고 채움에 따라 글자색이
 * 갈린다. 연한 색 은 채움이 아니라 테마에 맞춰야 한다.
 *
 * 짙은 전경으로 넘어가는 것은 그 연한 톤이 글자로서 사라질 때뿐이다. 그 경우엔 채움색의
 * 색조를 물려받은 짙은 색을 쓴다.
 */
 ...
 * 진행률 트랙.
 *
 * 표면 톤(`surface-2`)을 그대로 쓴다. 채움과 3:1 이 나올 때까지 명도를 벌리면 머쉬맘에서
 * 크림색 트랙이 어두운 올리브로 밀려 카드 인상이 망가진다. 색감이 우선이므로 트랙은 표면 톤을
 * 따르고, 채움과의 대비는 리포트가 수치로 알려준다.
 *
 * 토큰을 따로 두는 것은 값이 달라서가 아니라 역할이 달라서다. 특정 테마에서 진행률이 안
 * 읽히면 그 테마만 `track` 을 덮으면 되고 `surface-2` 를 쓰는 다른 자리는 안 건드린다.
 */
...
export type MediaScopeTokens = {
   ...
   *
   * 라이트 테마는 페이지 틴트를 그대로 쓴다. 어두운 카드 위의 옅은 칩이 잘 보인다. 다크
   * 테마에서는 그 틴트가 카드 안 시작 안함 배지(`surface-2`)와 너무 비슷해져 구분이 안 되므로
   * 진한 채움(`secondary` + `on-secondary`)을 쓴다.
   *
 ...
 * 미디어 스코프.
 *
 * 일러스트 카드 안은 바탕이 `surface` 가 아니라 `mediaSurface` 라서 틴트·잉크를 다시 계산해야
 * 한다. 카드 내부 배지가 AA 에 미달하던(레테 3.88:1) 원인이 이것이다. 커스텀 프로퍼티는
 * 선언된 요소에서 `var()` 가 해석되므로 스코프 안에 다시 선언해야 새 기준이 반영된다.
 */
export function deriveMediaScope(tokens: DerivedTheme, mode: ThemeMode): MediaScopeTokens {
  ...
  // 라이트는 페이지 틴트를 그대로 쓴다. 어두운 카드 위의 옅은 칩이 잘 보인다. 다크는 그
  // 틴트가 카드 안 시작 안함 배지(surface-2)와 너무 비슷해져 구분이 안 되므로 진한 채움을 쓴다.
  const complete =
    mode === 'light'
