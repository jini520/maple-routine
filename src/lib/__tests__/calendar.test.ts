// 격자를 만드는 규칙을 **입출력으로** 못 박는다. 이것이 화면 안에 있으면
// «어느 규칙이 이 배치를 만들었나» 를 화면을 띄워야만 볼 수 있다(과 같은 태도).
...
describe('WEEKDAY_LABELS', () => {
  // 게임의 목요일 리셋과 무관하다. 이 격자는 한국 달력이다.
  it('일요일에서 시작하는 일곱', () => {
...
// ══ 주간 격자 — 목요일 리셋 주 ══════════════════════════
//
// **이 앱에는 「주」가 둘이다.** 월간 격자의 주는 일요일에 시작하고 주간 보기의
