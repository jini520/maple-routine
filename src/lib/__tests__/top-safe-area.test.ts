...
describe('[[ADR-139]] 정정 1 — 안드로이드 상단 안전영역에는 하한이 있다', () => {
  // 값 자체가 결정이라 못 박는다. 이 숫자가 움직이면 제목 위치와 [[ADR-134]] 상단 페이드 길이가
  ...
  // 아래에서 시작하고, 그것은 [[ADR-139]] 결정 1 이 없앤 상태보다 나쁘다.
  ...
  // 이 경우가 [[ADR-134]] 결정 5 와 만난다 — 상단 페이드가 0이 아니게 되어 마스크가 걸린다.
