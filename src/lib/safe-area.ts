 ...
 * 안드로이드 상단 하한.
 *
 * 실기기 31.3 → 48. 인셋이 이미 48 이상인 기기(큰 컷아웃)에는 아무것도 안 더한다. 더하는
 * 상수였다면 그런 기기에서 제목이 화면 한참 아래에서 시작한다.
 *
 * 기기 하나를 보고 고른 값이다. 인셋이 40~48 인 기기에서는 더하는 양이 0 에 가까워지고, 그
 * 구간에서 다시 부족 보고가 오면 값이 아니라 정책을 다시 본다.
 */
 ...
 * 안드로이드 하단 하한.
 *
 * iOS 인셋과 같은 수라는 것이 이 값의 전부다. 위의 48 은 iOS(59) 아래에서 고른 값이지만
 * 여기는 고를 것이 없다. 홈 인디케이터가 34 라, 하한을 34 로 두면 안드로이드가 iOS 를
 * 따라잡고 iOS 는 정확히 하한에 앉아 한 픽셀도 안 바뀐다. 48 이었다면 그것은 두 플랫폼을
 * 맞추는 값이 아니라 안드로이드만 더 띄우는 값이다.
 *
