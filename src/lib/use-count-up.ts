 ...
 * 비어 있다, [[ADR-087]] 결정 8).
 *
 * **키에 탭·기간이 들어가는 것이 핵심이다.** 지난 주에서 이번 주로 이동하면 금액이 크게 바뀌지만
 * 그건 같은 값이 변한 것이 아니라 **다른 값을 보게 된 것**이라, 굴리면 아무 뜻도 없는 모션이 된다.
 * 키가 다르면 기억이 없으므로 자연히 굴리지 않는다 — 별도 분기가 필요 없다.
 */
...
export function useCountUp(identity: string, target: number): number {
  ...
  // 화면에 그려진 값. **rAF 콜백 안에서만** 쓴다(렌더 중 ref 쓰기 금지).
