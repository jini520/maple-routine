 ...
 * (브라우저 실측). 띄어쓰기만 줄바꿈 기준이 되게 하려면 그 두 지점을 막아야 한다.
 *
 ...
 */
/**
 * 아이템 드롭 가뭄에 대한 플레이어 반응을 단계로 옮긴 문구라 구현자가 톤을 다듬지 않는다.
 *
 * 단계마다 문구가 풀이다. 각 풀의 첫 항목이 인덱스를 주지 않은 호출이 받는 문구다.
 *
 * 무작위 선택은 이 파일이 하지 않는다. `Math.random()` 을 여기서 부르면 순수 함수가 아니게
 * 되고 테스트가 값을 고정할 수 없다. 화면이 인덱스를 골라 넘긴다(마운트당 한 번. 매 렌더마다
 * 고르면 리렌더 때 문구가 깜빡인다).
 */
