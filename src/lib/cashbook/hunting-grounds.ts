 ...
 * 자리를 세우면 고를 수 있는 것처럼 보인다.
 *
 * 두 끝을 **몬스터 레벨**로 잰다(`monsterLevelRangeOf`):
 *
 ...
 * 이쪽은 1, 접으면 0). 실제로 마주치는 몬스터가 저마다 1 씩 어긋나 있으므로 이쪽이 사실에 가깝다.
 */
 ...
 *
 * **거르지 않는다.** 지역이 목록에 섰으면 그 안의 맵은 전부 선다(창 밖의 맵도 있다 — 결정 6).
 * 여기서 또 걸러 내면 지역은 떴는데 안이 비어 있다 가 생긴다.
 *
