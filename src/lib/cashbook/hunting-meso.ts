...
/**
 * 사냥 효율 조각 다섯. 고르는 것은 몇 마리를 놓치나 다.
 *
 * 고정 퍼센트(100·95·90·85·80)는 맵을 모르는 값이다. 40마리에서 하나를 놓치는 것(97.5%)과
 * 22마리에서 하나를 놓치는 것(95.5%)은 같은 손해가 아니다. 그래서 고르는 축을 마릿수로 내리고
 * 퍼센트는 맵이 정하는 라벨로 만들었다(`efficiencyPercentOf`).
 *
 * 0 이 첫 조각이고 기본값이다. 다 잡는 것이 기준이다.
 */
...
/**
 * 세그먼트에 적히는 라벨(%). `(마릿수 − 놓침) / 마릿수` 를 소수 첫째자리에서 반올림한다.
 * 언제나 정수다.
 *
...
/** 실제로 잡는 마릿수. 사냥터 요약 줄이 이 숫자를 적는다. */
 ...
 *
 * 재획비는 합산 결과에 곱하는 것이라 둘 다 합연산으로 두면 안 된다. 유니온의 부만 켠 150% 에
 * 1.2 가 걸려 ×1.8 이 된다.
 *
 * `icon` 은 `assets/items/` 의 파일명이다. 칩이 글자가 아니라 그림이라 없으면 칩이 빈 채로
 * 뜬다. 에러가 아니다. 그 자리를 `IncomeSheet.test` 가 붙든다.
 *
...
/**
 * 화면에 적히는 메소 획득량(%). 켠 아이템까지 반영한 증가량이다.
 *
 * 게임 스탯창과 같은 뜻이라 기본 100% 를 뺀 값이고 소수점은 버린다.
 *
 *     floor( (100 + 가산 %합) × 곱셈 배율 ) − 100
 *
 * 메획 149 캐릭터가 유니온의 부와 재획비를 둘 다 켜면 `(100+149+50) × 1.2 = 358.8` → 258% 다.
 *
 * 셈에는 이 값을 안 쓴다. 돈을 세는 것은 내림 전의 값이고 여기서 자른 값은 화면에만 산다.
 */
