/**
 * 아이템 분배 계산기의 계산. 나누는 것은 몫이 아니라 **수수료**다.
 *
 * 파티가 먹은 아이템을 한 명이 팔아 나눠 줄 때 수수료가 두 번 떼인다(판매 한 번, 분배 한 번).
 * `정산 대상 ÷ 인원` 을 그대로 보내면 받는 사람만 분배 수수료를 물어 균등이 깨진다(10억·6인·3%/3%
 * 에서 480만 메소 차이).
 *
 * 그래서 나눗셈이 아니라 역산이다. 수수료를 두 번 거쳐도 여섯이 같아지려면 얼마를 보내야 하는가.
 *
 * ```
 * x(1 − d/100) = N − (n−1)x   →   x = 100N / (100n − d)
 * ```
 *
 * **결정석은 여기 안 들어온다.** 게임이 파티원 각자에게 직접 지급하고 그 몫은 보스 수익이 이미
 * 계산한다. 여기서 다시 세면 같은 돈을 두 번 센다.
 *
