...
// 사람은 안 물어 균등이 깨진다(10억·6인·3%/3% 에서 480만 메소 차이 — [[ADR-168]] 결정 2 의 표).
//
 ...
 *
 * **[[ADR-006]] 대상이다**(사용자 확인, 2026-08-23). 넥슨이 요율을 바꾸면 여기가 스탈이 된다 —
 * 앱이 추정해 채운 값이 아니다.
 */
export type FeePercent = 3 | 5

/**
 * 판매가 상한 — **정밀도에서 온 값이지 게임 규칙이 아니다**([[ADR-168]] 열린 질문).
 *
 ...
 * ([[ADR-168]] 열린 질문) 이 선택이 1 메소를 움직인다.
 */
...
/**
 * 파티원 한 명에게 **보낼 금액**. 이것이 화면이 내놓는 숫자 전부다([[ADR-168]] 결정 8).
 *
 * 남는 메소는 내림으로 먹은 사람에게 간다([[ADR-168]] 결정 5) — 앱이 결정석을 나눌 때 쓰는 규칙과
