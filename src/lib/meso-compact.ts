/**
 * 메소를 **한 단위로 줄여** 적는다 — 캘린더 칸처럼 좁은 자리용([[ADR-169]] 정정 1).
 *
 * `formatMesoUnits`(`lib/drop-price.ts`)와 목적이 다르다. 그쪽은 입력한 값을 **정확히** 되읽어
 * 주는 자리라 「1억 2345만 6789」처럼 단위를 다 적지만, 여기는 **화면 폭 ÷ 7** 안에 들어가야 하므로
 * 단위 하나만 남기고 유효숫자 넷으로 자른다. 두 함수는 서로를 대체하지 않는다.
 *
 * 만 미만을 「0만」 으로 뭉개지 않는 것은 [[ADR-057]] 의 태도다 — 적었는데 0 으로 보이면 «없음» 과
 * 구분이 사라진다.
 */
