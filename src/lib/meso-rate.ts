/**
 * 캐릭터의 **메소 획득량**을 「최대 세팅」으로 낸다([[ADR-177]]).
 *
 ...
 * ② **일시 버프가 섞인다.** 장비·어빌리티·심볼이 날짜별로 완전히 동일한 캐릭터에서도 그 값만
 *    161 → 273 → 156 → 161 로 출렁였다(실측 2026-08-28).
 *
 * ②는 ①과 별개라, 프리셋을 다 훑는 이 파서를 **환산값으로 교차검증하면 안 된다** — 어긋나는
 * 것이 정상이다.
 *
 * ── 세는 법 ────────────────────────────────────────────────────────────────────────
 *
 ...
 *
 * 수치와 규칙은 **전부 사용자 확정**이다([[ADR-006]]) — 100% 캡도, 그 캡이 잠재와 에디셔널의
 * **합**에 걸린다는 것도, 심볼이 그 캡 **밖**이라는 것도.
 */
import type {
 ...
 *
 * 실측으로 초과 사례를 본 적은 없다 — 표본에서 장비 메획의 최댓값이 정확히 100 이었다. 그래서
