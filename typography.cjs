...
// **줄 상자 계산 방식은 플랫폼마다 다르므로** 이 표가 필요하다.
//
...
// ## 줄 높이의 하한은 1.13em 이다 (쉼표가 나오는 자리)
//
// RN 은 줄 높이가 글꼴의 자연 줄 상자보다 낮으면 **모자란 만큼의 절반을 descent 에서 깎는다**
// (`CustomLineHeightSpan`). Pretendard 의 자연 상자가 1.193em, descent 가 0.241em 이고 쉼표는
// 기준선 아래로 0.206em 내려간다. 줄 높이가 1.13em 밑이면 **쉼표 꼬리가 잘린다**.
// 이 표의 가장 좁은 비율이 `chip` 의 1.17em 이라 지금은 어느 칸도 안 걸린다. 새 칸을 더할 때
// 1.13em 을 넘길 것.

/**
 * 앱이 실제로 쓰는 넷. 심은 글꼴의 굵기와 같다.
 *
 * **800(`extrabold`)은 표에 없고, 글꼴 파일도 안 심는다.** 안드로이드가 그 굵기에서 글자를 잰 폭보다
 * 넓게 그려, 마지막 음절이 다음 줄로 넘어가 상자에 가려 사라진다(`주간` 이 `주` 로 보였다.
 * 에뮬레이터에서 두 번 재현). 같은 조건에서 700 은 안 잘린다. 최대는 `bold` 다.
 */
const fontWeight = {
  normal: '400',
  medium: '500',
