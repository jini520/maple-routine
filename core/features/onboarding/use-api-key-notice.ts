 ...
 * 넘긴다([[ADR-115]] 결정 7 · [[ADR-116]] 결정 1). 원인은 둘이다:
 *
 ...
 * 갈 수 없고 사용자가 새 키를 넣어야 한다. 처방이 같으면 화면도 같아야 한다([[ADR-116]] 결정 1).
 ...
 * (처방이 "상태를 awaitingApiKey 로 되돌리는 것"이라서 — [[ADR-115]] 결정 2). `ScheduleSyncError` 는
 * 감지 쪽 어휘라 타입으로만 받는다.
 *
 * 여기에 자체 **멱등** 가드(ref)를 두지 않는다 — 동시 실패가 모달을 하나로 접는 것은
 * noticeApiKeyIssue() 안의 `apiKeyNotice !== null` 하나가 보장한다([[ADR-115]] 결정 6 · [[ADR-116]]
