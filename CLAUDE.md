...
- CRITICAL: 문서·주석·앱 문구를 **읽히게 쓸 것** ([[ADR-194]]). 셋을 지킨다. ① **코드에 실제로 있는 것의 이름**을 쓴다(그 말로 `src/` 를 검색해 안 나오면 바꾼다. `벽지`·`층`·`갈래` 같은 비유 금지). ② **결론만 쓰지 말고 이유를 같은 자리에** 쓴다. 한 문장에 개념을 셋 넣지 말고 문장을 나눈다. ③ **장식 기호를 쓰지 않는다** — `—`(문장을 끊거나 괄호로) · `«»`(없애고 볼드로) · `「」`(앱 문구는 곡선 따옴표 `‘’`, 문서는 백틱). 가운뎃점 `·` 은 그대로 쓴다
- CRITICAL: **소스 주석에 `[[ADR-NNN]]` 을 쓰지 말 것** ([[ADR-206]] 결정 1). 참조는 한 방향이다 — ADR 이 파일 경로·심볼을 가리키는 것은 결정이 코드를 관리하는 일이고, 그 반대에는 권한이 없다. 인용 대신 **이유를 그 자리에 한 줄로** 적고, 적을 이유가 없으면 주석째 지울 것. 찾는 길은 `docs/README.md` 가 기능→문서→소스로 낸다. 기존 4,359건은 감사가 그 파일에 닿을 때 함께 걷는다
- 주석은 JSDoc(`/** */`)으로 **역할만** 쓸 것 ([[ADR-206]] 결정 2~4 · [[ADR-194]] 정정 2). 태그는 `@param`(위치 인자) · `@example`(쓰는 법, 실제 호출부에서) · `@returns`(자명하지 않을 때) · `@see` 넷. `@use` 는 JSDoc 태그가 아니다. 경위·계측값·라이브러리 소스 인용은 안 적고, **어기면 버그가 나는 값만** 한 줄로 남긴다. 문장은 `~한다` 보다 **명사로 끝낼 것**(`무한 반복시키는 훅`)
- ADR 인용(`[[ADR-NNN]]`)은 `src/__tests__/adr-citation-policy.test.ts` 가 검사한다 — 실재하지 않는 번호, 🔗 없는 죽은 ADR 을 소스에서 인용하는 것, 문서에 없는 「결정 N」, 인덱스와 파일의 배지 불일치를 잡는다. 새 ADR 은 **`### 결정 N.` 제목 형식**으로 쓸 것(그래야 결정 번호까지 검사 대상이 된다)
