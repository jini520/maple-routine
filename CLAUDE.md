...
- CRITICAL: 문서는 **비유 대신 평이한 개발 용어**로 쓸 것 ([[ADR-194]]). 판단 기준은 하나 — 뜻을 풀어 다시 말해야 이해되는 문장이면 고친다. `든다`→`그린다/표시한다` · `갈래`→`종류/항목` · `선다`→`놓인다/표시된다` · `걷었다`→`삭제했다` · `못 박는다`→`고정한다` · `벽지`→`테마 배경` · `캔다`→`처리한다`. 전체 목록은 ADR-194 결정 2. 내용의 밀도(«무엇을 왜 그렇게 정했나» 를 자세히 적는 것)는 그대로 두고 **어휘만** 바꾼다
