...
// `src/assets/generated/*.ts` 로 낸다. **빌드 타임이 아니라 커밋 타임**에 하므로 번들러를 안 거치고도
// 같은 목록이 선다.
//
// 파일 안에는 평범한 ESM 에셋 import 만 있고, 그 값이 무엇이 되는지는 번들러가 정한다(Metro 는
// 에셋 id). 그래서 플랫폼별 생성물이 필요 없고, 갈리는 것은 값의 **타입 한 줄**뿐이다
// (`src/types/image-asset.ts` ↔ `.native.ts`).
//
 ...
 * 육안으로 같아도 안 맞는다. 슬러그 정규화를 **여기서 한 번** 한다
...
function renderFrames(group, framesByDir) {
  ...
  return [
    header(group, '순서가 곧 재생 순서다. 파일 이름 앞의 숫자로 정렬해 둔다(렉시코 정렬은 10 < 2).'),
    '',
    "import type { ImageAssetRef } from '../../types/image-asset'",
    '',
    ...imports,
...
export function renderGroup(group, assetsDir = ASSETS_DIR) {
  ...
  // 여러 디렉터리를 합칠 때 뒤가 이긴다. items → rings 를 한 맵에 넣는 순서다.
  const byKey = new Map()
