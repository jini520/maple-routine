#!/usr/bin/env node
// Live Update(OTA) 번들 배포 스크립트 (ADR-022).
//
// 사용법: node scripts/publish-live-update.mjs <x.y.z>
//
// 사전 준비: `gh auth login`으로 GitHub CLI 인증만 되어 있으면 된다(추가 계정 가입·결제 수단 불필요).
// 이 저장소(REPO)에 고정 릴리스 태그(RELEASE_TAG)를 하나 만들어 두고, 배포할 때마다
// 그 릴리스에 번들 zip을 추가하고 latest.json을 덮어쓴다. src/native/live-update.ts의
// LIVE_UPDATE_MANIFEST_URL이 이 릴리스의 latest.json을 가리킨다.
//
...
const RELEASE_TAG = 'live-update-latest'

const version = process.argv[2]
if (!version || !/^\d+\.\d+\.\d+$/.test(version)) {
  console.error('사용법: node scripts/publish-live-update.mjs <x.y.z>')
  process.exit(1)
}
