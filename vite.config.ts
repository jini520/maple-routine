...
// 대신한다(그 앱과 함께 사라졌다).
//
//   ① `npm test` — `core/` 의 vitest 실행. vitest 는 cwd 에서 설정을 찾으므로 루트에 하나가 있어야
//      한다. RN 쪽(`src/`)은 **jest** 로 돌므로 아래 `exclude` 로 뺀다 — 안 빼면 vitest 가 그것까지
...
const coreSrc = fileURLToPath(new URL('./core', import.meta.url))
...
export default defineConfig({
  ...
  test: {
    ...
    exclude: [...configDefaults.exclude, 'src/**', 'android/**', 'ios/**', 'dist/**'],
  },
})
